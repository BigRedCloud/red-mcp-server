import { convertBrcEduBlobToCsvText, resolveBrcEduBlobExtension, } from "./convertBlobToCsvText.js";
import { formatRedSyncFailureMessage, getRedBrcEduSyncEndpoint, getRedBrcEduSyncSecret, postCsvTextToRed, } from "./syncToRed.js";
...
export async function processBrcEduBlob(options) {
    ...
    try {
        ...
        logger.log("error", `Failed to read BRC Edu blob ${fileName}: ${safeProcessingError(error)}`);
        return "failed";
    }
    if (!csvText.trim()) {
