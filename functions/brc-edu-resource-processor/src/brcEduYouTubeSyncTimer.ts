...
import {
  RED_BRC_EDU_SYNC_SECRET_HEADER,
  RED_BRC_YOUTUBE_SYNC_ENDPOINT_ENV,
  RED_BRC_YOUTUBE_SYNC_SCHEDULE_ENV,
  RED_BRC_YOUTUBE_SYNC_SECRET_ENV,
} from "./constants.js";
...
async function postYouTubeSyncToRed(
  context: InvocationContext,
  source: "timer" | "webhook",
): Promise<void> {
  ...
  if (!endpoint || !secret) {
    context.error(
      "YouTube sync skipped: RED_BRC_YOUTUBE_SYNC_ENDPOINT or RED_BRC_YOUTUBE_SYNC_SECRET is not configured.",
    );
    return;
  }
  ...
  if (!response.ok) {
    ...
    context.error(
      `YouTube sync to Red failed (${response.status}): ${text.slice(0, 300)}`,
    );
    return;
  }
...
export async function brcEduYouTubeSyncTimer(
  _timer: Timer,
  context: InvocationContext,
): Promise<void> {
