...
import { adminRowsToSupportCsvText, buildWorkbookBufferFromAdminRows, parseWorkbookBufferToAdminRows, supportRowsFromAdminRows, validateWebinarAdminRows, } from "./brc_edu_workbook.js";
import { createAzureWorkbookBlobAccess, downloadWebinarWorkbookForAdmin, loadWebinarWorkbookForAdmin, saveWebinarWorkbookForAdmin, toSafeWorkbookStorageErrorMessage, } from "./brc_edu_workbook_store.js";
...
test("saveWebinarWorkbookForAdmin saves valid edited rows", async () => {
    ...
    }, access, new Date("2026-07-14T15:00:00.000Z"));
    assert.equal(result.ok, true);
    if (result.ok) {
...
test("saveWebinarWorkbookForAdmin creates archive before replacing latest", async () => {
    ...
    await saveWebinarWorkbookForAdmin({ rows: [sampleRow()], ifMatch: '"etag-initial"' }, access);
    assert.equal(uploads.length, 1);
...
test("saveWebinarWorkbookForAdmin returns 409 for stale ETag", async () => {
    ...
    const result = await saveWebinarWorkbookForAdmin({ rows: [sampleRow()], ifMatch: '"etag-stale"' }, access);
    assert.equal(result.ok, false);
    if (!result.ok) {
...
test("toSafeWorkbookStorageErrorMessage does not expose credentials", () => {
    const message = toSafeWorkbookStorageErrorMessage(new Error(`Upload failed: ${CONNECTION_STRING}`));
