...
import { BrcEduGraphError, downloadSupportCsvFromGraph, fetchGraphAccessToken, getBrcEduGraphConfig, } from "./brc_edu_graph.js";
const GRAPH_CONFIG = {
    tenantId: "tenant-123",
    clientId: "client-456",
...
function withGraphEnv(values, run) {
    const keys = [
...
test("fetchGraphAccessToken requests client credentials token", async () => {
    ...
    assert.equal(String(calls[0]?.input), "https://login.microsoftonline.com/tenant-123/oauth2/v2.0/token");
...
test("fetchGraphAccessToken throws safe error when token request fails", async () => {
    ...
    await assert.rejects(() => fetchGraphAccessToken(GRAPH_CONFIG, fetchImpl), (error) => {
...
test("downloadSupportCsvFromGraph throws safe error when download fails", async () => {
    ...
    await assert.rejects(() => downloadSupportCsvFromGraph(GRAPH_CONFIG, fetchImpl), (error) => {
        assert.ok(error instanceof BrcEduGraphError);
