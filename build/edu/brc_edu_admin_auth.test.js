...
test("getBrcEduAdminPublicUrl prefers BRC_EDU_ADMIN_PUBLIC_URL", () => {
    ...
    assert.equal(getBrcEduAdminPublicUrl(), "https://red.example.com/internal/brc-edu/resources/upload");
});
...
test("unauthenticated access redirects to Microsoft sign-in when Entra is configured", () => {
    ...
    if (!result.ok) {
        ...
        assert.match(result.redirectToLogin, /post_login_redirect_uri=%2Finternal%2Fbrc-edu%2Fresources%2Fupload/);
    }
});
...
test("buildEasyAuthLoginRedirectUrl encodes the return path", () => {
    assert.equal(buildEasyAuthLoginRedirectUrl("/internal/brc-edu/resources/upload"), "/.auth/login/aad?post_login_redirect_uri=%2Finternal%2Fbrc-edu%2Fresources%2Fupload");
});
