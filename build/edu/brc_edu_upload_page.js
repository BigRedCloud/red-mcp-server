...
import { renderYouTubeAdminSectionHtml, youtubeAdminSectionCss, YOUTUBE_ADMIN_REQUIRED_ELEMENT_IDS, youtubeSyncApiUrl, } from "../brc-edu/youtube/youtube-admin-page.js";
import { renderFreshdeskAdminSectionHtml, freshdeskAdminSectionCss, FRESHDESK_ADMIN_REQUIRED_ELEMENT_IDS, freshdeskSyncApiUrl, } from "../brc-edu/freshdesk/freshdesk-admin-page.js";
...
export function parseBrcEduAdminView(value) {
...
export function getAdminPageScriptReferencedElementIds(view = "overview") {
    if (view === "youtube") {
...
function adminPageUrl(auth, view = "overview") {
...
function renderAdminNav(auth, view) {
    const items = [
    ...
    return `
      <nav class="admin-nav" aria-label="Content administration">
        ...
        return `<a href="${href}" class="admin-nav-link${active ? " nav-active" : ""}"${active ? ' aria-current="page"' : ""}>${escapeHtml(item.label)}</a>`;
    })
        .join("")}
      </nav>`;
}
function renderOverviewSectionHtml(auth) {
...
export function renderBrcEduUploadPage(secretOrAuth, view = "overview") {
