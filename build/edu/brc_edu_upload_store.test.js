...
import { BRC_EDU_UPLOAD_MAX_BYTES, buildBrcEduBlobNames, contentTypeForUploadExtension, handleBrcEduResourceUpload, resolveUploadExtension, validateBrcEduAdminUploadSecret, } from "./brc_edu_upload_store.js";
...
test("handleBrcEduResourceUpload stores csv latest and archive blobs", async () => {
    ...
    assert.deepEqual(uploads.map((entry) => entry.blobName), [result.ok ? result.latestBlob : "", result.ok ? result.archiveBlob : ""]);
