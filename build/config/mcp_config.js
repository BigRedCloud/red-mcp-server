...
export const MANDATORY_ROUTING_INSTRUCTION = "MANDATORY ROUTING: For every new Red request, call brc_route_request with the user's complete original message before calling accounting tools. When a company is already connected and you have connectionRef from brc_confirm_company_connection, pass that same connectionRef silently on brc_route_request (and later transactional tools) so the routeToken binds to the verified connection across MCP session rotation. Do not show connectionRef to normal users. Transactional tools reject requests without a valid routeToken returned by the router. A routeToken does not replace preview-before-posting or user confirmation.";
