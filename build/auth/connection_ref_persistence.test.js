...
test("missing-ref failure followed by valid-ref retry succeeds", async () => {
    ...
    const retryScope = await prepareHttpToolSessionScope(uniqueId("retry-session"), new Map(), undefined, claimed.connectionRef);
    await runWithActiveConnectionRef(claimed.connectionRef, () => runWithHttpToolSession(retryScope, async () => {
        const credential = await getCredentialForCompanyAsync("Company C");
        assert.equal(credential.kind, "apiKey");
    ...
    const result = await handler({ companyName: "Company C", connectionRef: claimed.connectionRef }, { sessionId: uniqueId("handler-session") });
...
test("missing-ref failure does not delete the connection", async () => {
    ...
    const refStillValid = await getConnectionStore().getConnectionIdForRef(claimed.connectionRef);
...
test("ref remains valid after telemetry update", async () => {
    const { claimConnectionCodeForSession, getConnectionStore, prepareHttpToolSessionScope, runWithHttpToolSession, runWithActiveConnectionRef, listConnectedCompanyNames, } = await loadModules();
    ...
    const companies = await getConnectionStore().listConnectedCompanies(connectionId);
    assert.equal(companies.length, 1);
    assert.equal(await getConnectionStore().getConnectionIdForRef(claimed.connectionRef), connectionId);
    const scope = await prepareHttpToolSessionScope(uniqueId("after-tel"), new Map(), undefined, claimed.connectionRef);
    await runWithActiveConnectionRef(claimed.connectionRef, () => runWithHttpToolSession(scope, async () => {
        assert.deepEqual(listConnectedCompanyNames(), ["Company C"]);
    }));
});
...
test("list company contexts reloads through connectionRef", async () => {
    ...
    const result = await handler({ connectionRef: claimed.connectionRef }, { sessionId: uniqueId("contexts-session") });
...
test("list sales invoices reloads through connectionRef", async () => {
    ...
    const result = await handler({ companyName: "Company C", connectionRef: claimed.connectionRef }, { sessionId: uniqueId("invoice-session") });
...
test("separate connections cannot inherit Company B", async () => {
    ...
    const scopeB = await prepareHttpToolSessionScope(uniqueId("use-b"), new Map(), undefined, claimB.connectionRef);
    await runWithActiveConnectionRef(claimB.connectionRef, () => runWithHttpToolSession(scopeB, async () => {
...
test("true 401 removes only the affected company", async () => {
    const { brcFetch, setApiKeyForCompany, listConnectedCompanyNames, runWithSessionKeyStore } = await loadModules();
    ...
    try {
        await runWithSessionKeyStore(new Map(), async () => {
            ...
            const failed = (await brcFetch("Company Bad", "/v1/salesInvoices?page=1&pageSize=1"));
...
test("403, 404, 422, 500, timeout and malformed response preserve the connection", async () => {
    ...
    for (const testCase of cases) {
        const originalFetch = globalThis.fetch;
        globalThis.fetch = async () => new Response(testCase.body, { status: testCase.status });
        try {
            await runWithSessionKeyStore(new Map(), async () => {
                setApiKeyForCompany({
                ...
                await assert.rejects(() => brcFetch("Company Keep", "/v1/salesInvoices?page=1&pageSize=1"));
                assert.equal(listConnectedCompanyNames().includes("Company Keep"), true, `status ${testCase.label} must preserve the connection`);
            });
        }
        finally {
            globalThis.fetch = originalFetch;
