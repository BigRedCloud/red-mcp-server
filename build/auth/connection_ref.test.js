...
async function seedConnection(connectionId, companies) {
    const { getConnectionStore } = await loadModules();
    const store = getConnectionStore();
    await store.saveConnectedCompanies(connectionId, companies.map((companyName) => ({
        companyName,
...
test("confirm issues connectionRef and rotated Vibe session can reload companies with it", async () => {
    const { getConnectionStore, claimConnectionCodeForSession, runWithHttpToolSession, prepareHttpToolSessionScope, listConnectedCompanyNames, isConnectionRefFormat, } = await loadModules();
    const store = getConnectionStore();
    ...
    const scope = await prepareHttpToolSessionScope(rotatedSession, new Map(), undefined, result.connectionRef);
...
test("rotated session without connectionRef or stable client identity stays unbound", async () => {
    const { getConnectionStore, claimConnectionCodeForSession, prepareHttpToolSessionScope, listConnectedCompanyNames, runWithHttpToolSession, } = await loadModules();
    const store = getConnectionStore();
    const code = uniqueId("code");
    const connectionId = uniqueId("connection");
...
test("different connectionRef cannot access another connection", async () => {
    const { getConnectionStore, issueConnectionRef, prepareHttpToolSessionScope, listConnectedCompanyNames, runWithHttpToolSession, } = await loadModules();
    ...
    const scopeWrong = await prepareHttpToolSessionScope(uniqueId("session"), new Map(), undefined, refB.connectionRef);
    ...
    const scopeRight = await prepareHttpToolSessionScope(uniqueId("session-2"), new Map(), undefined, refA.connectionRef);
...
test("expired connectionRef fails safely", async () => {
    const { getConnectionStore, issueConnectionRef, prepareHttpToolSessionScope, } = await loadModules();
    const connectionId = uniqueId("connection");
...
test("audit log remains scoped to resolved connection across rotated MCP sessions", async () => {
    const { auditEntryMatchesScope, recordRedAuditEntry, getRedAuditLog, runWithHttpToolSession, prepareHttpToolSessionScope, claimConnectionCodeForSession, getConnectionStore, __resetRedAuditLogForTests, } = await loadModules();
    ...
    const scopeA = await prepareHttpToolSessionScope(sessionA, new Map(), undefined, claim.connectionRef);
    ...
    const scopeB = await prepareHttpToolSessionScope(sessionB, new Map(), undefined, claim.connectionRef);
