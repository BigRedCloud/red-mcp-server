...
test("confirm issues connectionRef and rotated Vibe session can reload companies with it", async () => {
    ...
    const result = await claimConnectionCodeForSession(confirmationCode, confirmSession);
...
test("audit log remains scoped to resolved connection across rotated MCP sessions", async () => {
    const { recordRedAuditEntry, getRedAuditLog, runWithHttpToolSession, prepareHttpToolSessionScope, claimConnectionCodeForSession, getConnectionStore, __resetRedAuditLogForTests, } = await loadModules();
