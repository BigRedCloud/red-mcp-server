...
export function formatConnectionExpiryForUser(expiresAt, timeZone = getDisplayTimezone()) {
    try {
...
function formatPartValue(parts, type) {
    return parts.find((part) => part.type === type)?.value;
}
export function formatUtcOffsetForTimezone(timeZone, date) {
    try {
        const offsetPart = formatPartValue(new Intl.DateTimeFormat("en-US", {
            timeZone,
            timeZoneName: "longOffset",
        }).formatToParts(date), "timeZoneName");
        if (!offsetPart) {
            return "UTC";
        }
...
export function formatExpiryTimezonePresentation(expiresAtMs, timeZone = getDisplayTimezone()) {
    ...
    const dayPeriod = (formatPartValue(timeParts, "dayPeriod") ?? "").toUpperCase();
    const expiryTimezoneAbbreviation = formatPartValue(timeParts, "timeZoneName") ?? timeZone;
...
export function formatExpiryTimeText(expiresAtMs, timeZone) {
...
export function buildConnectionExpiryMetadata(options) {
    ...
    const timezonePresentation = formatExpiryTimezonePresentation(expiresAtMs, options.timeZone);
