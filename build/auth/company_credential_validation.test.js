...
test("CSV resubmit removes stale stored company when new API key fails validation", async () => {
    resetCompanyCredentialValidator();
    resetValidationFetch();
    mockBrcValidationFetchForKeys(new Set(["key-b", "key-c", "key-d"]), new Set(["bad-key", "old-stale-key"]));
    const connectionId = uniqueId("connection");
    const store = getConnectionStore();
    ...
    assert.equal(outcome.failedCompanies.some((failure) => failure.companyName === "Company A"), true);
    const stored = await store.listConnectedCompanies(connectionId);
    assert.deepEqual(stored.map((company) => company.companyName).sort(), ["Company B", "Company C", "Company D"]);
});
...
test("validation logging omits API keys", async () => {
    ...
    const validationLogs = logs.filter((line) => line.includes("Red company credential validation:"));
