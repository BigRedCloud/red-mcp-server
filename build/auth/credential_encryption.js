...
function resolveEncryptionKey() {
    const fromEnv = process.env.RED_CONNECT_ENCRYPTION_KEY?.trim() ||
        process.env.RED_CONNECT_COSMOS_CONNECTION_STRING?.trim();
    if (!fromEnv) {
        throw new Error("RED_CONNECT_ENCRYPTION_KEY is required when using encrypted persistent connection storage.");
    }
    return createHash("sha256").update(fromEnv, "utf8").digest();
}
export function encryptCredentialSecret(plaintext) {
...
export function decryptCredentialSecret(ciphertext) {
    const key = resolveEncryptionKey();
...
export function encodeCredentialSecretForMemory(plaintext) {
    return `mem:${Buffer.from(plaintext, "utf8").toString("base64")}`;
}
export function decodeCredentialSecretFromMemory(encoded) {
