...
import { ensureConnectionStoreInitialized, getMcpSessionContext, resolveConnectionIdForActiveSessionWithMeta, runWithMcpSessionContext, } from "./connection_store.js";
import { ensureCredentialsForCurrentSession, normaliseCompanyName, resolveHttpClientKey, resolveSessionKeyStore, runWithActiveConnectionRef, runWithHttpClientKey, runWithHttpRequestSessionId, runWithSessionKeyStore, } from "../shared.js";
...
export function hashClientKeyForDiagnostics(clientKey) {
...
export function hashInstanceIdForDiagnostics() {
    const raw = process.env.WEBSITE_INSTANCE_ID?.trim() ||
...
export function hashSessionIdForClientKeyDiagnostics(sessionId) {
 ...
 */
export function resolveHttpClientKeyFromHeaders(headers, clientIp) {
    const identityParts = [];
...
export function buildHttpClientKeyFromRequest(req) {
    return resolveHttpClientKeyFromHeaders(req.headers, getClientIpFromRequest(req)).clientKey;
}
export function resolveHttpClientKeyFromRequest(req) {
    return resolveHttpClientKeyFromHeaders(req.headers, getClientIpFromRequest(req));
}
...
export function resolveHttpClientKeyDetailsFromExtra(extra) {
    const headers = extra?.requestInfo?.headers;
    if (!headers) {
        return undefined;
    }
...
export function registerSessionClientKey(sessionId, clientKeyOrResolution, inheritEligible = true) {
    const id = sessionId.trim();
    if (!id || !clientKeyOrResolution) {
        return;
    }
    const resolution = typeof clientKeyOrResolution === "string"
        ? {
...
export function getRegisteredSessionClientKey(sessionId) {
    return getRegisteredSessionClientKeyResolution(sessionId)?.clientKey;
}
export function getRegisteredSessionClientKeyResolution(sessionId) {
    const id = sessionId?.trim();
    if (!id) {
        return undefined;
    }
...
export function clearRegisteredSessionClientKey(sessionId) {
    sessionClientKeys.delete(sessionId.trim());
}
/** Test helper. */
export function clearSessionClientKeysForTests() {
 ...
 */
export function resolveClientKeyForToolSession(sessionId, extra) {
    return resolveClientKeyDetailsForToolSession(sessionId, extra)?.clientKey;
}
export function resolveClientKeyDetailsForToolSession(sessionId, extra) {
...
export function clientKeyForClaimInheritance(resolution) {
...
export async function prepareHttpToolSessionScope(sessionId, keyStore, clientKeyOrResolution, connectionRef) {
    await ensureConnectionStoreInitialized();
    const resolutionDetails = typeof clientKeyOrResolution === "string"
...
export async function runHttpToolSessionFromExtra(transportSessionId, keyStore, extra, fn, options) {
    ...
    const scope = await prepareHttpToolSessionScope(sessionId, store, clientKeyDetails, connectionRef);
