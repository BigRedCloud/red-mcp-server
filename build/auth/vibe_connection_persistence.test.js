...
import { CONNECTION_REF_INVALID_MESSAGE, CONNECTION_REF_NOT_PASSED_MESSAGE, } from "./connection_ref.js";
...
test("missing connectionRef error says Vibe did not pass connectionRef", async () => {
    const { runWithHttpRequestSessionId, getCredentialForCompany, } = await loadModules();
    const sessionId = uniqueId("vibe-session");
    assert.throws(() => {
        runWithHttpRequestSessionId(sessionId, () => getCredentialForCompany("Company A"));
    }, (error) => {
...
test("expired connectionRef still gives clear reconnect instruction", async () => {
    const { getConnectionStore, issueConnectionRef, prepareHttpToolSessionScope, runWithHttpToolSession, runWithActiveConnectionRef, getCredentialForCompanyAsync, } = await loadModules();
...
test("jsonResponse echoes activeConnectionRef without adding credential fields", async () => {
    ...
    const payload = runWithActiveConnectionRef(SAMPLE_REF, () => jsonResponse({
        Items: [{ id: 1 }],
        Count: 1,
        companyName: "Company A",
    }));
