...
test("GET public Freshdesk image route rejects invalid token without Azure details", async (t) => {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/public/brc-edu/freshdesk-images/1001/invalid.token.value`);
...
test("HEAD public Freshdesk image route is available without company credentials", async (t) => {
    const port = await getFreePort();
    await startTestServer(t, port);
    const response = await fetch(`http://127.0.0.1:${port}/public/brc-edu/freshdesk-images/1001/invalid.token.value`, { method: "HEAD" });
    assert.equal(response.status, 404);
    assert.equal(response.headers.get("x-content-type-options"), "nosniff");
});
test("help resource details tool guidance prefers instructionBlocks and prohibits Show Image", () => {
    ...
    assert.match(GET_HELP_RESOURCE_DETAILS_TOOL_DESCRIPTION, /Place each screenshot link immediately after the step/i);
