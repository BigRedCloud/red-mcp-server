...
import { buildExamplesText, buildGettingStartedText, buildSafetyText, customerDeploymentPolicyText, } from "./deployment_tools.js";
const INTERNAL_AUTH_FAILURE_PATTERN = /confirmed authentication failure|empty result or missing data|does not by itself mean the connection/i;
const FORBIDDEN_CUSTOMER_INTERNALS = /connectionRef|redconn_|rehydrat|HTTP status|session binding|mcp\.json|BRC_ALLOW_|Cursor/i;
test("getting-started separates one-time link from connection duration", () => {
    ...
    assert.match(text, new RegExp(`remains available in this Red session for ${duration.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`, "i"));
...
test("safety wording requires confirmation without overpromising identical previews", () => {
    ...
    assert.equal(/Nothing gets created, updated, or deleted without me showing you a plain-English preview first/i.test(text), false);
    assert.equal(INTERNAL_AUTH_FAILURE_PATTERN.test(text), false);
    assert.equal(FORBIDDEN_CUSTOMER_INTERNALS.test(text), false);
});
test("examples list only the three supported email document types when email is enabled", () => {
    const text = buildExamplesText();
    const capabilitiesEmailSection = /Supported email actions:[\s\S]*sales invoice[\s\S]*quote[\s\S]*customer statement/i.test(text) || /Email sending is not available in this Red session/i.test(text);
