...
- Sending supported customer emails: ${availability(capabilities.canSendEmails)}${capabilities.canSendEmails
        ? `
...
- Maximum records per batch request: ${capabilities.maxBatchItems}`
        : ""}
- Development / operator mode: ${capabilities.devModeActive ? "enabled" : "not enabled"}
...
export function buildGettingStartedText() {
...
export function buildExamplesText() {
    const capabilities = getCustomerDeploymentCapabilities();
...
export function buildSafetyText() {
    const sessionDuration = formatCredentialTtlForUser();
    const capabilities = getCustomerDeploymentCapabilities();
...
export function registerDeploymentTools(server) {
    server.tool("brc_getting_started", [
        ...
        "For a specific tutorial, screenshot, article, YouTube video, or webinar question, use brc_find_help_resources instead.",
