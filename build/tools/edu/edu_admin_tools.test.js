...
import { OPEN_EDU_ADMIN_TOOL_DESCRIPTION, OPEN_EDU_ADMIN_TOOL_NAME, OPEN_EDU_ADMIN_TRIGGER_PHRASES, buildOpenEduAdminMarkdownLink, registerEduAdminTools, selectOpenEduAdminToolForUserMessage, } from "./edu_admin_tools.js";
...
test("brc_open_edu_admin description includes required trigger phrases", () => {
    for (const phrase of OPEN_EDU_ADMIN_TRIGGER_PHRASES) {
        assert.ok(OPEN_EDU_ADMIN_TOOL_DESCRIPTION.includes(phrase), `Description should include trigger phrase: ${phrase}`);
    }
    ...
    assert.match(OPEN_EDU_ADMIN_TOOL_DESCRIPTION, /Do not ask whether the user means Big Red Cloud website login or connecting a company/i);
});
test("brc_open_edu_admin is selected for each required trigger phrase", () => {
    for (const phrase of OPEN_EDU_ADMIN_TRIGGER_PHRASES) {
        assert.equal(selectOpenEduAdminToolForUserMessage(phrase), OPEN_EDU_ADMIN_TOOL_NAME, `Expected selection for: ${phrase}`);
    }
    assert.equal(selectOpenEduAdminToolForUserMessage("Open Red's admin page"), OPEN_EDU_ADMIN_TOOL_NAME);
    assert.equal(selectOpenEduAdminToolForUserMessage("Please open the BRC Edu admin page now"), OPEN_EDU_ADMIN_TOOL_NAME);
    assert.equal(selectOpenEduAdminToolForUserMessage("connect my company"), null);
    assert.equal(selectOpenEduAdminToolForUserMessage("how do I add a customer?"), null);
});
...
test("brc_open_edu_admin returns only protected URL Markdown and never the admin secret", async () => {
    ...
    const expected = buildOpenEduAdminMarkdownLink("https://red.example.com/internal/brc-edu/resources/upload");
