 ...
 */
export function selectOpenEduAdminToolForUserMessage(message) {
    const normalised = message.trim().toLowerCase().replace(/\s+/g, " ");
    if (!normalised) {
        return null;
    }
    const phraseMatched = OPEN_EDU_ADMIN_TRIGGER_PHRASES.some((phrase) => normalised.includes(phrase.toLowerCase()));
    if (phraseMatched) {
        return OPEN_EDU_ADMIN_TOOL_NAME;
    }
    // Close equivalents Claude / hosts may paraphrase.
    const adminIntent = /\b(open|show|launch|go to|take me to)\b/.test(normalised) &&
    ...
    const manageIntent = /\bmanage\b/.test(normalised) &&
...
export function buildOpenEduAdminMarkdownLink(adminUrl = getBrcEduAdminPublicUrl()) {
    return `[Open Red's admin page](${adminUrl})`;
}
export function registerEduAdminTools(server) {
    server.tool(OPEN_EDU_ADMIN_TOOL_NAME, OPEN_EDU_ADMIN_TOOL_DESCRIPTION, {}, async () => {
