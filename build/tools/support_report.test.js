...
import { __resetInitiatingRequestsForTests, MAX_INITIATING_REQUEST_LENGTH, peekInitiatingRequestForRoute, runWithInitiatingRequest, sanitizeInitiatingRequest, } from "../audit/initiating_request.js";
import { buildRedSupportReport, buildSupportReportFilename, supportReportMcpResponse, } from "../audit/support_report.js";
import { wrapWriteToolHandler } from "../guards/write_confirmation.js";
import { issueActionRouteToken } from "../routing/route-token.js";
import { __resetRedAuditLogForTests, clearCredentialForCompany, getRedAuditLog, recordRedAuditEntry, setApiKeyForCompany, } from "../shared.js";
import { __resetBrcFailureTelemetryForTests, } from "../telemetry/brc_failure.js";
import { buildTelemetryCustomDimensions, runWithRedTelemetryContext, } from "../telemetry/identity.js";
...
function withStagingTelemetry(fn) {
...
function listedTechnical(companyName) {
    return getRedAuditLog({
        connectedCompanyNames: [companyName],
        includeTechnicalDetails: true,
    });
}
function listedCustomer(companyName) {
    return getRedAuditLog({
        connectedCompanyNames: [companyName],
    });
}
...
function captureAuditToolHandler(toolName) {
    let handler;
    registerAuditTools({
...
test("brc_route_request message is remembered on the issued route token", () => {
    ...
    assert.equal(peekInitiatingRequestForRoute(issued.payload.jti), "Change Quote id 999999999 reference to QA0001");
});
test("sanitizeInitiatingRequest redacts secrets and length-limits", () => {
    const sanitized = sanitizeInitiatingRequest("Change Quote apiKey=super-secret-company-key with redconn_abc123TOKEN and redroute_zzz.yy Authorization: Bearer abc");
    assert.ok(sanitized);
...
test("customer-facing audit output does not gain support-report technical fields", () => {
    ...
    for (const key of Object.keys(customerFacing[0])) {
        assert.ok(CUSTOMER_FACING_AUDIT_KEYS.has(key), `unexpected customer-facing field: ${key}`);
    }
...
test("support report includes success, confirmed Quote preflight failure, and routed request", async () => {
    ...
    await withStagingTelemetry(async () => {
        ...
        await runWithInitiatingRequest("Change Quote id 999999999 reference to QA0001", async () => {
        ...
        assert.equal(report.filename, "red-support-report-Company-C-20260817T140210Z.txt");
        ...
        assert.match(report.text, new RegExp(`telemetryConnectionSessionId: ${STAGING_CONNECTION_SESSION_ID}`));
        assert.match(report.text, new RegExp(`telemetryClientId: ${STAGING_TELEMETRY_CLIENT_ID}`));
        assert.equal(report.text.match(/telemetryConnectionSessionId: ([^\r\n]+)/)?.[1], azureDims["red.connection_session_id"]);
        assert.equal(report.text.match(/telemetryClientId: ([^\r\n]+)/)?.[1], azureDims["red.telemetry_client_id"]);
        assert.match(report.text, /Use telemetryConnectionSessionId to locate this Red session in Application Insights/);
        ...
        assert.equal(/Actual BRC request:\r?\nPUT \/v1\/quotes\/999999999/.test(report.text), false);
...
test("support report redacts credentials from initiating request and never stores secrets", async () => {
    ...
    await runWithInitiatingRequest("Change Quote 999999999 apiKey=super-secret-company-key token=access-token-value connectionRef=redconn_secretref routeToken=redroute_secrettok", async () => {
        await assert.rejects(() => invokeWrapped(wrapped, {
            companyName: "Company C",
            id: 999999999,
            reference: "QA0001",
...
test("brc_generate_support_report is scoped to the connected company and is not a BRC write", async () => {
    ...
    try {
        const result = (await handler({ companyName: "Company C" }));
...
test("preflight failure without a known BRC request is marked unavailable", () => {
    ...
    assert.match(report.text, /Intended write:\r?\nPUT \/v1\/quotes\/999999999/);
    assert.match(report.text, /Actual BRC request:\r?\nunavailable/);
    assert.equal(/Actual BRC request:\r?\nGET \/v1\/quotes\/999999999/.test(report.text), false);
