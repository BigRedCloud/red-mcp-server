...
export function registerAllocationResolverTools(server) {
    server.tool("brc_list_allocation_resolvers", [
        ...
        const data = await brcFetch(companyName, `/v1/allocationResolvers?${params.toString()}`);
    ...
    server.tool("brc_update_allocations", [
        ...
        confirmWrite: z
            .boolean()
            .optional()
            .describe("Must be true only after the user explicitly confirms posting the allocation update."),
        ...
        if (confirmWrite !== true) {
            return textResponse(buildAllocationSummary({
                companyName,
                bookTranId: payload.bookTranId,
                allocationResolvers: payload.allocationResolvers,
            }));
        }
        const data = await brcJsonRequest(companyName, "PUT", "/v1/allocationResolvers", payload);
        return jsonResponse(data);
    });
    server.tool("brc_list_allocated_transactions", [
        ...
        const params = new URLSearchParams();
        params.set("bookTranId", String(bookTranId));
        const data = await brcFetch(companyName, `/v1/allocationResolvers/allocated?${params.toString()}`);
        return jsonResponse(data);
    });
    server.tool("brc_delete_allocation_resolver", [
        ...
        confirmWrite: z
            .boolean()
            .optional()
            .describe("Must be true only after the user explicitly confirms reversing/deleting this allocation."),
        ...
        const data = await brcJsonRequest(companyName, "DELETE", `/v1/allocationResolvers/${encodeURIComponent(String(id))}`);
        return jsonResponse(data);
    });
}
