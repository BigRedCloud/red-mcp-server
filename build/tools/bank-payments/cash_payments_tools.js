...
export function registerCashPaymentTools(server) {
    ...
    server.tool("brc_create_cash_payment", "Creates a BRC cash payment. Use supplierId + ledger for supplier payments, bankAccountId + lodgement for bank lodgements, or analysisCategoryId + accountCode for analysed expenses. Analysis categories must be from the Cash Payments book (CP01-CP03) and accountCode must match the category.", {
        ...
        const payload = buildCashPaymentPayload({ ...args, entryDate, procDate });
        try {
            ...
            return jsonResponse({
                success: false,
                message: "BRC rejected the cash payment create request. Red sent the request to BRC rather than automatically blocking it because of financial year.",
                companyName,
                endpoint: "POST /v1/cashPayments",
    ...
    server.tool("brc_create_cash_receipt", "Creates a BRC cash receipt. Accepts either a raw payload object or common flat fields. entryDate/procDate default to today. VAT rate fields are only sent when the company's VAT on Cash Receipt setting is enabled.", {
        companyName: companyNameSchema,
        ...
        try {
            ...
            return jsonResponse({
                success: false,
                message: "BRC rejected the cash receipt create request. Red sent the request to BRC rather than automatically blocking it because of financial year.",
                companyName,
                endpoint: "POST /v1/cashReceipts",
                transactionDate: payload.procDate ?? payload.entryDate,
                payloadSent: payload,
    ...
    server.tool("brc_create_payment", "Creates a BRC payment from the Payments book. Use supplierId for supplier payments, or analysisCategoryId + accountCode for analysed bank payments. Analysis categories must be from the bank's Payments book (BP01-BP06) and accountCode must match the category.", {
        companyName: companyNameSchema,
        note: z.string(),
        entryDate: z.string().optional().describe("Entry date in ISO format. Defaults to today."),
        procDate: z.string().optional().describe("Processing date in ISO format. Defaults to entryDate."),
        ...
        const entryDate = args.entryDate ?? todayIsoDate();
        const procDate = args.procDate ?? entryDate;
        const payload = buildPaymentPayload({ ...args, entryDate, procDate });
        try {
            ...
            return jsonResponse({
                success: false,
                message: "BRC rejected the payment create request. Red sent the request to BRC rather than automatically blocking it because of financial year.",
                companyName,
                endpoint: "POST /v1/payments",
                transactionDate: payload.procDate ?? payload.entryDate,
                payloadSent: payload,
