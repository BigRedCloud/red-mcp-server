...
export const SALES_DOCUMENT_PRODUCT_ID_DESCRIPTION = "Do not invent productId values and do not use productId 0 or 1 as placeholders. productId 0 and 1 are treated as placeholders and are blocked at runtime before the draft preview and before posting. If a product line is needed, first call brc_list_products and use a real product from the connected company. If no suitable product exists, ask the user whether to create/select a product, or use a service/non-product line only if the endpoint supports it.";
export const SALES_DOCUMENT_SALES_VAT_CATEGORY_DESCRIPTION = "Sales invoices must use Sales VAT rates. Purchase/non-Sales VAT rates are blocked before draft and before posting, even if the VAT percentage matches.";
