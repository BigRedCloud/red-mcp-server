...
import { buildCashReceiptPayload, buildCustomerLikePayload, mergeCashReceiptUpdateFromCurrent, } from "./payloads_tools.js";
test("cash receipt single-rate VAT amount is the receipt total, not net", () => {
    ...
    const acEntries = payload.acEntries;
    ...
    const vatEntries = payload.vatEntries;
...
test("note-only merge keeps ledger 12.3 when builder would default it to 0", () => {
    ...
    const merged = mergeCashReceiptUpdateFromCurrent(built, current, requestedUpdates);
