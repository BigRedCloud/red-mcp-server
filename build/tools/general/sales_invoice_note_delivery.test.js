...
import { buildSalesInvoicePayload, normaliseDeliveryTo, resolveSalesDocumentNote, } from "./payloads_tools.js";
/**
...
test("invoice note defaults to the customer name when no note is provided", () => {
    const payload = buildSalesInvoicePayload(invoiceArgs({ customerName: "Acme Trading Ltd" }));
    assert.equal(payload.note, "Acme Trading Ltd");
});
test("explicit user note is preserved over the customer name", () => {
    const payload = buildSalesInvoicePayload(invoiceArgs({ customerName: "Acme Trading Ltd", note: "Deposit for March order" }));
    assert.equal(payload.note, "Deposit for March order");
});
test("product name / description is not used as the note by default", () => {
    const payload = buildSalesInvoicePayload(invoiceArgs({ customerName: "Acme Trading Ltd", description: "Blue Widget" }));
    assert.equal(payload.note, "Acme Trading Ltd");
...
test("explicit deliveryTo string is preserved as an array", () => {
    const payload = buildSalesInvoicePayload(invoiceArgs({ deliveryTo: "12 Main Street, Dublin" }));
    assert.deepEqual(payload.deliveryTo, ["12 Main Street, Dublin"]);
});
test("explicit deliveryTo array is preserved", () => {
    const payload = buildSalesInvoicePayload(invoiceArgs({ deliveryTo: ["Unit 4", "Industrial Estate", "Cork"] }));
