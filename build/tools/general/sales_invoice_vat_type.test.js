...
import { buildSalesInvoicePayload, buildSalesCreditNotePayload, resolveSalesInvoiceVatTypeId, } from "./payloads_tools.js";
...
test("3. VAT rate / percentage / totals are unchanged by the customer VAT type", () => {
    const domestic = buildSalesInvoicePayload(invoiceArgs({ customerVatType: DOMESTIC }));
    const foreign = buildSalesInvoicePayload(invoiceArgs({ customerVatType: FOREIGN_NON_EU }));
...
test("4. shared builder is unchanged for non-invoice-creation callers (credit notes default to Domestic)", () => {
    ...
    const creditNote = buildSalesCreditNotePayload(invoiceArgs());
...
test("resolveCustomerVatType reads the selected customer record", async () => {
    setCustomerVatTypeLoaderForTests(async (_company, customerId) => String(customerId) === "42"
