...
import { buildCustomerLikePayload, buildMissingCustomerLikeInformationResponse, collectMissingCustomerLikeFields, } from "./payloads_tools.js";
test("collectMissingCustomerLikeFields requires code and name", () => {
    ...
    assert.deepEqual(collectMissingCustomerLikeFields({ code: "ACME", name: "Acme Ltd" }), []);
});
test("buildMissingCustomerLikeInformationResponse asks without inventing data", () => {
    const response = buildMissingCustomerLikeInformationResponse(["code", "name"], 1);
...
test("buildCustomerLikePayload does not invent address or credit terms by default", () => {
    const payload = buildCustomerLikePayload({ code: "ACME", name: "Acme Ltd" }, 1);
