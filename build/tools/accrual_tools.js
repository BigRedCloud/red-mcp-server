...
export function registerAccrualTools(server) {
    server.tool("brc_list_accruals", [
    ...
    server.tool("brc_get_accrual", "Returns information about a single parent Accrual by id. Child accruals are not exposed by the public API.", {
        companyName: companyNameSchema,
        id: accrualIdSchema,
    }, async ({ companyName, id }) => {
        const data = await brcFetch(companyName, `/v1/accruals/${encodeURIComponent(String(id))}`);
        return jsonResponse(data);
    });
    server.tool("brc_create_accrual", [
        ...
        if (confirmWrite !== true) {
            return textResponse(buildAccrualSummary({
                companyName,
                action: "create",
                ...payload,
            }));
        }
        const data = await brcJsonRequest(companyName, "POST", "/v1/accruals", payload);
        return jsonResponse(data);
    });
    server.tool("brc_update_accrual", [
        ...
        const data = await brcJsonRequest(companyName, "PUT", `/v1/accruals/${encodeURIComponent(String(id))}`, payload);
        return jsonResponse(data);
    });
    server.tool("brc_delete_accrual", [
        ...
        const data = await brcJsonRequest(companyName, "DELETE", `/v1/accruals/${encodeURIComponent(String(id))}?${params.toString()}`);
        return jsonResponse(data);
    });
}
