...
import { buildSalesVatCategoryContext, enforceSalesVatCategoryOrThrow, setSalesVatCategoryContextLoaderForTests, } from "../../guards/sales_vat_category.js";
...
function analysisEntry(overrides) {
    return {
        id: 0,
        accountCode: "S01",
...
function validInvoice(lines, overrides = {}) {
...
function twoLineInvoice(overrides = {}) {
    return validInvoice([
        productLine({
            amountNet: 100,
            vat: 23,
...
test("missing productTrans returns a field error", () => {
    ...
    assert.ok(result.errors.some((error) => error.field === "productTrans"), `expected productTrans error, got ${JSON.stringify(result.errors)}`);
});
test("product line missing acEntries returns a field error", () => {
    const payload = twoLineInvoice();
    const lines = payload.productTrans;
    ...
    assert.ok(result.errors.some((error) => error.field === "productTrans.0.acEntries"), `expected productTrans.0.acEntries, got ${JSON.stringify(result.errors)}`);
});
test("missing productTrans.1.acEntries returns structured field error", () => {
    ...
    assert.ok(result.errors.some((error) => error.field === "productTrans.1.acEntries"), `expected productTrans.1.acEntries, got ${JSON.stringify(result.errors)}`);
});
...
test("analysis values not reconciling to amountNet are rejected", () => {
    ...
    assert.ok(result.errors.some((error) => error.field === "productTrans.0.acEntries" &&
...
test("line amount not equalling amountNet + VAT is rejected", () => {
    ...
    assert.ok(result.errors.some((error) => error.field === "productTrans.1.amount" &&
        /amountNet \+ vat/i.test(error.message)));
});
test("incorrect header totalNet is rejected", () => {
    const result = validateGeneratedReferenceSalesInvoicePayload(twoLineInvoice({ totalNet: 1 }));
    assert.equal(result.valid, false);
    if (result.valid) {
        return;
    }
...
test("incorrect header totalVAT is rejected", () => {
    const result = validateGeneratedReferenceSalesInvoicePayload(twoLineInvoice({ totalVAT: 1 }));
    assert.equal(result.valid, false);
    if (result.valid) {
        return;
    }
...
test("incorrect header total is rejected", () => {
    const result = validateGeneratedReferenceSalesInvoicePayload(twoLineInvoice({ total: 1, unpaid: 1 }));
    assert.equal(result.valid, false);
    if (result.valid) {
        return;
    }
...
function assertMissingLineFieldRejected(field) {
    const payload = twoLineInvoice();
    const lines = payload.productTrans;
    ...
    assert.ok(result.errors.some((error) => error.field.includes(field)), `expected an error mentioning ${field}, got ${JSON.stringify(result.errors)}`);
}
...
async function assertAcEntriesLineOneFailureDoesNotCallBrc(mutate) {
    let brcJsonRequestCalled = false;
    const payload = twoLineInvoice();
    mutate(payload.productTrans);
    let threw = false;
    let result;
    try {
    ...
    assert.equal(body.valid, false);
    assert.ok(Array.isArray(body.errors));
    assert.ok(body.errors.some((error) => error.field === "productTrans.1.acEntries"), `expected productTrans.1.acEntries in ${JSON.stringify(body.errors)}`);
}
...
test("write wrapper returns structured acEntries error before preview without calling handler", async () => {
    const { wrapWriteToolHandler } = await import("../../guards/write_confirmation.js");
    ...
    const body = parseResponseText(result);
    assert.equal(body.valid, false);
    assert.ok(body.errors.some((error) => error.field === "productTrans.1.acEntries"));
});
test("existing placeholder product ID guard still works on multi-line payloads", () => {
    const payload = twoLineInvoice();
    const lines = payload.productTrans;
    lines[1].productId = 1;
    assert.throws(() => enforceSalesProductLineProductIdOrThrow(payload), /placeholder productId 1/);
});
test("existing Sales VAT guard still works on multi-line payloads", async () => {
    ...
    try {
        ...
        await assert.rejects(async () => enforceSalesVatCategoryOrThrow("Test Co", payload), /Sales VAT|VAT category|purchase/i);
    }
    finally {
