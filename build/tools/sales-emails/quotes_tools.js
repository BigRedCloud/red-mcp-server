...
function quoteRecordValue(quote, ...keys) {
...
function isQuoteClosed(closedDate) {
 ...
 */
export function buildQuoteDeletePreview(quote, timestamp) {
    ...
    const preview = {
        ...
        customer: quoteRecordValue(quote, "customerOwnerName", "CustomerOwnerName", "customer") ??
            null,
        customerOwnerName: quoteRecordValue(quote, "customerOwnerName", "CustomerOwnerName") ?? null,
 ...
 */
export function extractCreatedQuoteId(response) {
...
function quoteCreateStatus(response) {
    if (response && typeof response === "object" && !Array.isArray(response)) {
        const record = response;
...
export function registerQuoteTools(server) {
    ...
    server.tool("brc_create_quote", `Creates a BRC quote using structured MCP fields. Requires a quote reference when quote references are manual or unknown. Do not use when Quotes reference setting is Unknown unless the user has provided a quote reference. Previews before posting include a Missing or not provided section for blank customer phone or email only — warnings only, do not invent values. Nothing is written to Big Red Cloud until you confirm. ${SALES_DOCUMENT_SALES_REP_REQUIRED_DESCRIPTION} ${SALES_DOCUMENT_ANALYSIS_CATEGORY_DESCRIPTION}`, quoteSchemaBase, async ({ companyName, confirmQuotesAutoGenerateInBrc: _confirmQuotesAutoGenerateInBrc, confirmCrAnalysisCategory, ...args }) => {
        ...
        try {
            ...
            return jsonResponse(buildQuoteCreateSuccessBody({
    ...
    server.tool("brc_update_quote", "Updates a BRC quote's manual reference only. Quote.note is not persisted by this update and is not accepted here. Loads the current quote, preserves all other fields (including timestamp, product lines, analysis entries, totals, customer, sales rep, dates, comments, and closed state), applies the new reference, then PUTs the full record. Manual quote references must be 6 characters or fewer because Big Red Cloud truncates longer references.", {
        companyName: companyNameSchema,
    ...
    server.tool("brc_delete_quote", "Deletes a BRC quote by id using timestamp confirmation. Loads the quote once to obtain its timestamp and to preview id, reference, customer, total, open or closed state, any linked sales invoice, and timestamp before asking for confirmation. A successful delete is the result; if a later lookup by quote id returns an unexpected error, treat that check as inconclusive rather than as a failed delete. Confirm remaining quotes from the quote list by quote id — quote references are not necessarily unique.", {
        companyName: companyNameSchema,
        id: z.union([z.string(), z.number()]).describe("Quote id."),
        ...
        const payloadPreview = buildQuoteDeletePreview(quote, timestamp);
        if (!isQuoteDeleteConfirmed({
            confirmDelete,
            confirmWrite: rest.confirmWrite,
        })) {
            return jsonResponse({
    ...
    server.tool("brc_generate_sales_invoice_from_quote", "Generates a sales invoice from a BRC quote. Preview-before-posting shows the exact POST /v1/quotes/generateSaleInvoice body (quoteId and optional entryDate/procDate only). Nothing is written until you confirm.", {
        companyName: companyNameSchema,
