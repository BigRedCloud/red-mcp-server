import { z } from "zod";
import { clearRedAuditLog, getRedAuditLog, RED_ACTIVITY_SCOPE_INSTRUCTION, textResponse, } from "../shared.js";
export function registerAuditTools(server) {
    server.tool("brc_list_audit_log", `Show a record of data changes (create, update, delete, batch, quote close/reopen, emails, etc.) made through this Red MCP server session. Read-only API calls are not logged. Use this as the source of truth for "what did I do today in Red?" style questions. ${RED_ACTIVITY_SCOPE_INSTRUCTION}`, {
        ...
        if (entries.length === 0) {
            return textResponse("No company changes have been recorded in this Red session yet. I can only see Red activity recorded for this current session/connection. For broader history, check Big Red Cloud directly.");
        }
        return textResponse(JSON.stringify({
            message: "Here is the Red audit log for this MCP server session. This is scoped to the current session/connection only. I can only see Red activity recorded for this current session/connection. For broader history, check Big Red Cloud directly.",
