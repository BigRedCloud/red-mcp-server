...
function addAccountToGroupedRows(groupedRows, account) {
...
function buildGroupedNominalReport(nominalAccounts) {
    ...
    return Array.from(groupedRows.values()).sort((a, b) => a.nominalCode.localeCompare(b.nominalCode, undefined, { numeric: true }));
}
function detectGroupedByField(nominalAccounts) {
    for (const account of nominalAccounts) {
...
export function registerNominalReportTools(server) {
    server.tool("brc_get_nom_ac_ledger_by_ids", "Gets nominal accounts for specific ids by calling GET /v1/nominalAccounts/{id} for each id.", {
        ...
        for (const id of idList) {
            try {
                const data = await brcFetch(companyName, `/v1/nominalAccounts/${encodeURIComponent(id)}`);
                items.push(data);
            }
            catch (error) {
                errors.push({
