import { z } from "zod";
import { brcFetch, brcJsonRequest, companyNameSchema, jsonResponse, textResponse, } from "../../shared.js";
...
function getTotals(accountTransactions) {
    return accountTransactions.reduce((totals, line) => ({
        totalDebit: totals.totalDebit + line.debit,
        totalCredit: totals.totalCredit + line.credit,
    }), { totalDebit: 0, totalCredit: 0 });
}
function validateBalancedJournal(accountTransactions) {
    const { totalDebit, totalCredit } = getTotals(accountTransactions);
    if (Number(totalDebit.toFixed(2)) !== Number(totalCredit.toFixed(2))) {
        throw new Error(`Nominal journal is not balanced. Total debit is ${totalDebit}, total credit is ${totalCredit}.`);
    }
}
...
export function registerNominalJournalBatchTools(server) {
    server.tool("brc_list_nominal_journal_batches", [
        ...
        filter: z
            .string()
            .optional()
            .describe("Optional OData filter. Filtering is allowed by entryDate, for example: entryDate ge 2024-01-01T00:00:00"),
        orderBy: z
            .string()
            .optional()
        ...
        const data = await brcFetch(companyName, `/v1/nominalJournalBatches${query}`);
        return jsonResponse(data);
    });
    server.tool("brc_get_nominal_journal_batch", "Returns information about a single Nominal Journal Batch by id.", {
        companyName: companyNameSchema,
        id: nominalJournalBatchIdSchema,
    }, async ({ companyName, id }) => {
        const data = await brcFetch(companyName, `/v1/nominalJournalBatches/${encodeURIComponent(String(id))}`);
        return jsonResponse(data);
    });
    server.tool("brc_create_nominal_journal_batch", [
        ...
        accountTransactions: nominalJournalBatchCreateSchema.shape.accountTransactions,
        confirmWrite: z
            .boolean()
            .optional()
            .describe("Must be true only after the user explicitly confirms creating this Nominal Journal Batch."),
    }, async ({ companyName, bookTranTypeId, entryDate, procDate, total, accountTransactions, confirmWrite, }) => {
        ...
        if (confirmWrite !== true) {
            return textResponse(buildJournalSummary({
                companyName,
                action: "create",
        ...
        const data = await brcJsonRequest(companyName, "POST", "/v1/nominalJournalBatches", payload);
        return jsonResponse(data);
    });
    server.tool("brc_update_nominal_journal_batch", [
        ...
        accountTransactions: nominalJournalBatchUpdateSchema.shape.accountTransactions,
        confirmWrite: z
            .boolean()
            .optional()
            .describe("Must be true only after the user explicitly confirms updating this Nominal Journal Batch."),
    }, async ({ companyName, id, bookTranTypeId, entryDate, procDate, total, timestamp, accountTransactions, confirmWrite, }) => {
        ...
        const data = await brcJsonRequest(companyName, "PUT", `/v1/nominalJournalBatches/${encodeURIComponent(String(id))}`, payload);
        return jsonResponse(data);
    });
    server.tool("brc_delete_nominal_journal_batch", [
        ...
        confirmWrite: z
            .boolean()
            .optional()
            .describe("Must be true only after the user explicitly confirms deleting this Nominal Journal Batch."),
        ...
        const data = await brcJsonRequest(companyName, "DELETE", `/v1/nominalJournalBatches/${encodeURIComponent(String(id))}?${params.toString()}`);
        return jsonResponse(data);
    });
}
