...
export function registerPrepaymentTools(server) {
    server.tool("brc_list_prepayments", [
        ...
        filter: z
            .string()
            .optional()
            .describe("Optional OData filter. Filtering is allowed by entryDate, for example: entryDate ge 2024-03-01T00:00:00."),
        orderBy: z
            .string()
            .optional()
    ...
    server.tool("brc_get_prepayment", "Returns information about a single parent Prepayment by id. Child prepayments are not exposed by the public API.", {
        companyName: companyNameSchema,
        id: prepaymentIdSchema,
    }, async ({ companyName, id }) => {
        const data = await brcFetch(companyName, `/v1/prepayments/${encodeURIComponent(String(id))}`);
        return jsonResponse(data);
    });
    server.tool("brc_create_prepayment", [
    ...
    }, async ({ companyName, acCode, entryDate, procDate, reference, total, firstDetail, secondDetail, confirmWrite, }) => {
        ...
        if (confirmWrite !== true) {
            return textResponse(buildPrepaymentSummary({
                companyName,
                action: "create",
                ...payload,
            }));
        }
        const data = await brcJsonRequest(companyName, "POST", "/v1/prepayments", payload);
        return jsonResponse(data);
    });
    server.tool("brc_update_prepayment", [
    ...
    }, async ({ companyName, id, acCode, entryDate, procDate, reference, total, timestamp, firstDetail, secondDetail, confirmWrite, }) => {
        ...
        const data = await brcJsonRequest(companyName, "PUT", `/v1/prepayments/${encodeURIComponent(String(id))}`, payload);
        return jsonResponse(data);
    });
    server.tool("brc_delete_prepayment", [
        ...
        const data = await brcJsonRequest(companyName, "DELETE", `/v1/prepayments/${encodeURIComponent(String(id))}?${params.toString()}`);
        return jsonResponse(data);
    });
}
