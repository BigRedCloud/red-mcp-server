...
import { assertSalesVatRatesOrThrow, buildSalesVatCategoryContext, classifyVatCategoryName, collectSalesLineVatRateIds, } from "./sales_vat_category.js";
/**
...
function invoicePayload(vatRateId) {
    return {
        customerId: 1,
...
test("2. a Purchases for Resale VAT rate is blocked on a sales invoice", () => {
    const context = buildSalesVatCategoryContext(CATEGORIES, RATES);
    assert.throws(() => assertSalesVatRatesOrThrow(invoicePayload(20), context), PURCHASE_MESSAGE);
});
test("3. a Purchases not for Resale VAT rate is blocked on a sales invoice", () => {
    const context = buildSalesVatCategoryContext(CATEGORIES, RATES);
    assert.throws(() => assertSalesVatRatesOrThrow(invoicePayload(30), context), PURCHASE_MESSAGE);
});
...
test("5. batch sales invoice items apply the same Sales VAT category validation", () => {
    ...
    assert.doesNotThrow(() => assertSalesVatRatesOrThrow({ customerId: 1, productId: 5023355, quantity: 1, unitPrice: 100, vatRateId: 10 }, context));
    // Flat/top-level batch item shape with a purchase rate is blocked.
    assert.throws(() => assertSalesVatRatesOrThrow({ customerId: 1, productId: 5023355, quantity: 1, unitPrice: 100, vatRateId: 20 }, context), PURCHASE_MESSAGE);
});
test("non-Sales (other) category is blocked with a clear non-Sales message", () => {
    ...
    const context = buildSalesVatCategoryContext(categories, rates);
    assert.throws(() => assertSalesVatRatesOrThrow(invoicePayload(90), context), /non-Sales VAT category/);
});
...
test("collectSalesLineVatRateIds reads productTrans and flat shapes", () => {
    assert.deepEqual(collectSalesLineVatRateIds({ productTrans: [{ vatRateId: 10 }, { vatRateId: 20 }] }), [10, 20]);
