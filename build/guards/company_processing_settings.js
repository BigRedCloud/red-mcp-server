 ...
 */
export function formatVatDiscrepancyTolerance(value) {
...
function deriveCashReceiptVatMode(vatOnCashReceiptsEnabled, useAllocations) {
...
function describeCashReceiptVatMode(mode) {
    switch (mode) {
        case "manual":
            return "manual";
        case "allocation":
...
export async function getCompanyProcessingSettings(companyName) {
    const raw = await brcFetch(companyName, "/v1/companySetupConfig/getCompanyOptions");
    const enableVocrReporting = asBoolean(findValue(raw, ["enableVOCRReporting"]));
    ...
    const cashReceiptVatMode = deriveCashReceiptVatMode(vatOnCashReceiptsEnabled, useAllocations);
    return {
        raw,
        ...
        grossPriceSalesInvoicingEnabled: asBoolean(findValue(raw, ["allowEntryOfGrossPriceInInvoicing"])),
        marginVatSchemeEnabled: asBoolean(findValue(raw, ["marginVatScheme"])),
        reverseChargeCreditInputEnabled: asBoolean(findValue(raw, ["creditInputForReverseChargeVAT"])),
        ...
        purchasesVatAnalysisType: asNumber(findValue(raw, ["purchasesVatAnalysisType"])),
        creditNoteJournalAgeingValue: asNumber(findValue(raw, ["creditNoteJournalAgeingValue"])),
        creditNoteJournalAgeingName: asString(findValue(raw, ["creditNoteJournalAgeingName"])),
         ...
         */
        customerPaymentTermsEnabled: asBoolean(findValue(raw, ["customerPaymentTermsEnabled"])),
        customerPaymentTermsBasis: asString(findValue(raw, ["customerPaymentTermsBasis"])),
        customerPaymentTermsDays: asNumber(findValue(raw, ["customerPaymentTermsDays"])),
        supplierPaymentTermsEnabled: asBoolean(findValue(raw, ["supplierPaymentTermsEnabled"])),
        supplierPaymentTermsBasis: asString(findValue(raw, ["supplierPaymentTermsBasis"])),
        supplierPaymentTermsDays: asNumber(findValue(raw, ["supplierPaymentTermsDays"])),
        defaultDebtorStatementMinimumBalance: asNumber(findValue(raw, [
...
export function formatCompanyProcessingSettings(settings) {
    return [
        "Company processing settings",
        "",
        `Nominal Ledger: ${yesNoUnknown(settings.nominalLedgerEnabled)}`,
        `Autogenerate nominal codes: ${yesNoUnknown(settings.autogenerateNominalCodes)}`,
        "",
        "VAT settings:",
        `- VAT on Cash Receipts: ${yesNoUnknown(settings.vatOnCashReceiptsEnabled)}`,
        `- Cash Receipt VAT mode: ${describeCashReceiptVatMode(settings.cashReceiptVatMode)}`,
        ...
        `- Credit Input for Reverse Charge VAT: ${yesNoUnknown(settings.reverseChargeCreditInputEnabled)}`,
        `- Sales VAT analysis type: ${numberOrUnknown(settings.salesVatAnalysisType)}`,
        `- Purchases VAT analysis type: ${numberOrUnknown(settings.purchasesVatAnalysisType)}`,
        "",
        "Transaction settings:",
        `- Gross Price Entry: ${yesNoUnknown(settings.grossPriceSalesInvoicingEnabled)}`,
        `- VAT discrepancy allowed: ${numberOrUnknown(settings.vatDiscrepancyAllowed)}`,
        `- Credit Note / Journal Ageing: ${textOrUnknown(settings.creditNoteJournalAgeingName)} (${numberOrUnknown(settings.creditNoteJournalAgeingValue)})`,
        `- Print OS Items Only: ${yesNoUnknown(settings.printOsItemsOnly)}`,
        "",
        "Payment terms:",
        `- Customer payment terms enabled: ${yesNoUnknown(settings.customerPaymentTermsEnabled)}`,
        `- Customer payment terms basis: ${textOrUnknown(settings.customerPaymentTermsBasis)}`,
        `- Customer payment terms days: ${numberOrUnknown(settings.customerPaymentTermsDays)}`,
        `- Supplier payment terms enabled: ${yesNoUnknown(settings.supplierPaymentTermsEnabled)}`,
        `- Supplier payment terms basis: ${textOrUnknown(settings.supplierPaymentTermsBasis)}`,
        `- Supplier payment terms days: ${numberOrUnknown(settings.supplierPaymentTermsDays)}`,
        "",
        "Statement settings:",
        `- Default debtor statement minimum balance: ${numberOrUnknown(settings.defaultDebtorStatementMinimumBalance)}`,
    ].join("\n");
}
export function getTransactionSafetyWarnings(settings, workflow) {
    const warnings = [];
    if (workflow === "sales_invoice" ||
        ...
        if (settings.vatDiscrepancyAllowed !== undefined) {
            warnings.push(`VAT discrepancy tolerance is ${formatVatDiscrepancyTolerance(settings.vatDiscrepancyAllowed)}. Check supplied VAT amounts against this tolerance where VAT is manually provided.`);
        }
    }
    if (workflow === "sales_invoice" || workflow === "sales_credit_note") {
        if (settings.grossPriceSalesInvoicingEnabled === true) {
            warnings.push("Gross Price Entry is enabled for sales invoicing. Before creating the invoice, confirm whether the prices entered are VAT-inclusive/gross or VAT-exclusive/net. Say 'gross' if the prices include VAT, or 'net' if the prices exclude VAT.");
        }
        if (settings.grossPriceSalesInvoicingEnabled === false) {
            warnings.push("Gross Price Entry is not enabled for sales invoicing. Treat line prices as net (VAT-exclusive) prices unless the user explicitly asks for VAT-inclusive calculation.");
        }
        if (settings.reverseChargeCreditInputEnabled === true) {
            warnings.push("Credit Input for Reverse Charge VAT is enabled. Before posting, check whether EU or reverse-charge VAT applies to this sale; if it does, confirm the correct VAT treatment with the user rather than assuming standard domestic VAT.");
        }
    }
    if (workflow === "purchase") {
        if (settings.reverseChargeCreditInputEnabled === true) {
            warnings.push("Credit Input for Reverse Charge VAT is enabled. Before posting, check whether EU or reverse-charge VAT applies to this purchase; if it does, confirm the correct VAT treatment with the user rather than assuming standard domestic VAT.");
        }
    }
    ...
    if (workflow === "statement") {
        if (settings.defaultDebtorStatementMinimumBalance === undefined) {
            warnings.push("Default debtor statement minimum balance is not exposed by the current company options endpoint. Do not assume a minimum balance unless the user provides one.");
        }
    }
    return warnings;
}
...
function hasManualCashReceiptVatDetails(payload) {
    const record = payloadRecord(payload);
    if (!record) {
        return false;
    }
...
function hasCashReceiptAllocationDetails(payload) {
    const record = payloadRecord(payload);
    if (!record) {
        return false;
    }
...
function readStatementMinBalance(payload) {
    return asNumber(readPayloadValue(payload, [
        "minBalance",
        "minimumBalance",
        "statementMinimumBalance",
        "defaultDebtorStatementMinimumBalance",
...
function enforceCashReceiptSettings(settings, payload) {
    ...
    if (settings.cashReceiptVatMode === "allocation") {
        if (!hasCashReceiptAllocationDetails(payload)) {
            throw preflightError("VAT on Cash Receipts is enabled and allocation mode is active. Please provide allocation details before posting this cash receipt, or update the setting in Big Red Cloud.");
        }
    }
}
