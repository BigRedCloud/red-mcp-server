...
import { buildSalesVatCategoryContext, setSalesVatCategoryContextLoaderForTests, } from "./sales_vat_category.js";
/**
...
test("placeholder productId 0 is blocked before the create handler runs", async () => {
    ...
    await assert.rejects(async () => wrapped({ productId: 0, quantity: 1, unitPrice: 100 }), /placeholder productId/);
...
test("placeholder productId 1 is blocked before the gen_ref handler runs", async () => {
    ...
    await assert.rejects(async () => wrapped({ payload: { productTrans: [{ productId: 1, quantity: 1, unitPrice: 100 }] } }), /placeholder productId/);
    assert.equal(handlerCalled, false);
});
