...
import { formatVatDiscrepancyTolerance, getTransactionSafetyWarnings, } from "./company_processing_settings.js";
...
test("gross price entry warning explains the gross/net action in plain English", () => {
    const warnings = getTransactionSafetyWarnings(settings({ grossPriceSalesInvoicingEnabled: true }), "sales_invoice");
...
test("VAT discrepancy warning includes a unit, not a bare number", () => {
    const warnings = getTransactionSafetyWarnings(settings({ vatDiscrepancyAllowed: 1 }), "sales_invoice");
...
test("reverse charge warning includes a clear next action about EU/reverse-charge VAT", () => {
    const warnings = getTransactionSafetyWarnings(settings({ reverseChargeCreditInputEnabled: true }), "sales_invoice");
    const reverseChargeWarning = warnings.find((w) => w.includes("Credit Input for Reverse Charge VAT is enabled"));
