...
function extractReferenceSettingsSection(raw) {
    if (!isRecord(raw)) {
        return undefined;
    }
 ...
 */
function readReferenceSettingFlag(referenceSettings, keys) {
...
function workflowManualReferenceLabel(workflow) {
    switch (workflow) {
        case "sales_invoice":
        case "sales_credit_note":
            return "sales";
...
function autoGenerateSettingForWorkflow(settings, workflow) {
    switch (workflow) {
        case "sales_invoice":
        case "sales_credit_note":
...
function isPlaceholderReference(value) {
    ...
    return (normalised.startsWith("mcp_quote_") ||
        normalised.startsWith("mcp_po_") ||
        normalised.startsWith("mcp_dd_"));
}
function readPayloadReferenceValues(payload) {
    if (!isRecord(payload)) {
        return [];
    }
    const keys = [
...
export async function getCompanyReferenceSettings(companyName) {
...
export function formatCompanyReferenceSettings(settings) {
    return [
        "Reference settings:",
...
export function isQuoteReferenceSettingUnknown(settings) {
    return settings.quotesAutoGenerateReference === undefined;
}
export function enforceReferenceSettingsOrThrow(settings, workflow, payload, endpointKind = "manual", options) {
    ...
    if (autoGenerate === true) {
        if (endpointKind === "manual") {
            warnings.push(`This company is configured to auto-generate ${workflowLabel(workflow)} references. Prefer the auto-generated reference workflow where available.`);
        }
        if (hasManualReference) {
            warnings.push(`This company is configured to auto-generate ${workflowLabel(workflow)} references in Big Red Cloud, but a manual reference was supplied. BRC may ignore or override the supplied reference.`);
        }
        return { warnings };
    }
    if (endpointKind === "generated") {
...
export async function loadAndEnforceReferenceSettings(companyName, workflow, payload, endpointKind = "manual", options) {
    const settings = await getCompanyReferenceSettings(companyName);
    const { warnings } = enforceReferenceSettingsOrThrow(settings, workflow, payload, endpointKind, options);
