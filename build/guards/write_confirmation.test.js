...
test("2. analysed cash receipt with acEntries does not require confirmCounterpartyExplicit", async () => {
    ...
    assert.equal(String(body.message).includes("which customer"), false, "analysed cash receipt must not ask the user to choose a customer");
    assert.equal(handlerCalled, false);
});
...
test("5. batch analysed cash receipt does not require confirmCounterpartyExplicit", async () => {
    ...
    const preview = parseBody(await wrapped({
        companyName: "Company C",
        items: [
            {
                item: {
...
test("5b. batch customer cash receipt still requires confirmCounterpartyExplicit", async () => {
    ...
    const body = parseBody(await wrapped({
        companyName: "Company C",
...
test("6. sales invoice counterparty confirmation remains unchanged", async () => {
    ...
    const missing = parseBody(await wrapped({
        total: 10,
        note: "Invoice without customer",
    }));
    assert.equal(missing.status, "counterparty_missing");
    const needsConfirm = parseBody(await wrapped({
        customerId: 1,
        acCode: "CUST",
        total: 10,
    }));
...
test("mcp instructions: analysed cash receipts do not always require a customer", () => {
    ...
    assert.match(instructions, /only require explicit customer confirmation when the cash receipt actually uses a customer/i);
    assert.match(instructions, /Analysed cash receipts may instead use an explicitly supplied analysis allocation/i);
