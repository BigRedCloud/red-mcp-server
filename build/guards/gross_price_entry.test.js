...
import { enforceTransactionSettingsOrThrow, } from "./company_processing_settings.js";
import { applySalesPriceBasisToRawPayload, buildSalesInvoicePayload, } from "../tools/general/payloads_tools.js";
function grossPriceSettings() {
    return {
        raw: {},
...
test("Gross Price Entry enabled with no priceBasis is blocked", () => {
    assert.throws(() => enforceTransactionSettingsOrThrow(grossPriceSettings(), "sales_invoice", BARE_INVOICE_PAYLOAD), /Gross Price Entry is enabled[\s\S]*priceBasis: "gross"[\s\S]*priceBasis: "net"/);
});
test("Gross Price Entry enabled with top-level priceBasis gross is allowed", () => {
    assert.doesNotThrow(() => enforceTransactionSettingsOrThrow(grossPriceSettings(), "sales_invoice", BARE_INVOICE_PAYLOAD, { priceBasis: "gross" }));
});
test("Gross Price Entry enabled with top-level priceBasis net is allowed", () => {
    assert.doesNotThrow(() => enforceTransactionSettingsOrThrow(grossPriceSettings(), "sales_invoice", BARE_INVOICE_PAYLOAD, { priceBasis: "net" }));
});
test("Gross Price Entry enabled is allowed when payload productTrans line states price basis", () => {
    ...
    assert.doesNotThrow(() => enforceTransactionSettingsOrThrow(grossPriceSettings(), "sales_invoice", payload));
});
function structuredInvoiceArgs(priceBasis) {
    return {
        priceBasis,
        customerId: 1,
        acCode: "CUST",
...
test("Structured invoice with gross unitPrice 123 and VAT 23% creates net 100, VAT 23, gross 123", () => {
    const payload = buildSalesInvoicePayload(structuredInvoiceArgs("gross"));
...
test("Raw gen_ref priceBasis gross adds useTaxInclusiveUnitPrice true to payload and productTrans lines", () => {
    ...
    for (const line of result.productTrans) {
        assert.equal(line.useTaxInclusiveUnitPrice, true);
    }
});
