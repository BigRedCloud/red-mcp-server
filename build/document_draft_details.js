...
export function isQuoteOrSalesInvoiceCreateTool(toolName) {
    return QUOTE_OR_INVOICE_CREATE_TOOLS.has(toolName);
}
function getCustomerIdFromPayload(toolName, payload) {
...
function buildMissingOrNotProvidedSection(missingLabels) {
    ...
    return [
        ...
        "",
        "These contact fields are warnings only for create/post. Red Connect must not invent missing phone or email values.",
...
function buildContactWarning(label) {
    if (label === "Customer email") {
        return [
            `${label} is missing or not provided.`,
            "Red Connect will not invent this value.",
    ...
    return `${label} is missing or not provided. Red Connect will not invent this value.`;
}
export async function buildQuoteOrSalesInvoiceDraftDetails(toolName, companyName, payload) {
    ...
    if (customerId) {
        try {
            const customer = await brcFetch(companyName, `/v1/customers/${encodeURIComponent(String(customerId))}`);
            if (customer && typeof customer === "object" && !Array.isArray(customer)) {
                const record = customer;
                customerDetails.phone = detailField("Customer phone", record.phone ?? record.mobile);
    ...
    const missingOrNotProvidedSection = buildMissingOrNotProvidedSection(missingOrNotProvided);
...
export async function fetchCustomerEmailById(companyName, customerId) {
    try {
        const customer = await brcFetch(companyName, `/v1/customers/${encodeURIComponent(String(customerId))}`);
        if (!customer || typeof customer !== "object" || Array.isArray(customer)) {
            return null;
...
export async function resolveCustomerEmailForEmailDocument(args) {
    ...
    if (path === "/v1/email/sendSalesInvoice") {
        ...
        const invoice = await brcFetch(companyName, `/v1/salesInvoices/${encodeURIComponent(String(salesInvoiceId))}`);
    ...
    if (path === "/v1/email/sendQuote") {
        ...
        const quote = await brcFetch(companyName, `/v1/quotes/${encodeURIComponent(String(quoteId))}`);
        if (!quote || typeof quote !== "object" || Array.isArray(quote)) {
            return null;
        }
        const customerId = Number(quote.customerOwnerId ??
            quote.customerId);
        if (!Number.isFinite(customerId) || customerId <= 0) {
            return null;
        }
...
export function buildRecipientEmailRequiredResponse(args) {
    return {
        ...
        message: [
            `Red Connect cannot send this ${args.documentTypeLabel} email because no recipient email address is available.`,
            "",
