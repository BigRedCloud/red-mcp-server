...
function verbNoun(verbs, nouns, window = 60) {
    return new RegExp(`\\b(?:${verbs})\\b.{0,${window}}\\b(?:${nouns})\\b|\\b(?:${nouns})\\b.{0,${window}}\\b(?:${verbs})\\b`, "i");
}
 ...
 */
function hasExplicitBatchIntent(text) {
    ...
    if (/\b(?:add|create|prepare|record|raise|post)\s+(?:[2-9]|\d{2,})\b/i.test(text)) {
        return true;
    }
    return false;
}
 ...
 */
function batchNounMatch(nouns, window = 60) {
    return new RegExp([
 ...
 */
export const ACTION_WORKFLOW_REGISTRY = [
    // --- Quotes (special verbs) ---
    {
        workflowId: "generate_sales_invoice_from_quote",
        description: "Generate a sales invoice from an existing quote after confirming the quote and previewing the invoice.",
        ...
        match: verbNoun("generate|convert|create", "invoice\\s+from\\s+quote|quote\\s+to\\s+invoice|invoices?\\s+from\\s+(?:a\\s+)?quotes?"),
    },
    {
    ...
    },
    {
        ...
        description: "Create a product. Ask for required details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a sales rep. Ask for required details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a sales credit note. Confirm customer and lines, then show a preview before posting.",
        allowedTools: [
        ...
        permissionFlag: "update",
        match: verbNoun(CREATE_VERBS, "sales\\s+credit\\s+notes?|credit\\s+notes?"),
    },
    {
    ...
    },
    {
        ...
        description: "Create a sales entry. Confirm details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a quote. Confirm customer and lines, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a bank account. Confirm details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a cash payment. Confirm details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a cash receipt. Confirm details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a payment. Confirm details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create an accrual. Confirm details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a prepayment. Confirm details, then show a preview before posting.",
    ...
    },
    {
        ...
        description: "Create a nominal journal batch. Confirm lines, then show a preview before posting.",
        ...
        permissionFlag: "update",
        match: verbNoun(CREATE_VERBS + "|post", "nominal\\s+journal(?:\\s+batch(?:es)?)?|journal\\s+batch(?:es)?|journals?"),
    },
    {
        workflowId: "process_vat_category_rates",
        description: "Process VAT category rates after confirming the change and previewing.",
        allowedTools: ["brc_process_vat_category_rates"],
        actionVerbs: ["process", "update", "apply"],
        businessNouns: ["vat category", "vat rate"],
        permissionFlag: "update",
        match: verbNoun("process|update|apply", "vat\\s+category\\s+rates?|vat\\s+rates?|vat\\s+categor(?:y|ies)"),
    },
    // --- Update ---
    {
        workflowId: "update_customer",
        description: "Update a customer after confirming which record and fields to change, with preview before posting.",
    ...
    },
    {
        ...
        description: "Update a product after confirming which record and fields to change.",
    ...
    },
    {
        ...
        permissionFlag: "update",
        match: verbNoun(UPDATE_VERBS, "sales\\s+credit\\s+notes?|credit\\s+notes?"),
    },
    {
    ...
    },
    {
        ...
        description: "Update a quote's manual reference after confirming the record. Does not update Quote notes.",
    ...
    },
    {
        ...
        description: "Update allocations after confirming the records and amounts.",
    ...
    },
    {
        ...
        permissionFlag: "update",
        match: verbNoun(UPDATE_VERBS, "nominal\\s+journal(?:\\s+batch(?:es)?)?|journal\\s+batch(?:es)?"),
    },
    // --- Delete ---
    {
        workflowId: "delete_customer",
        description: "Delete a customer after identifying the record and obtaining explicit confirmation.",
    ...
    },
    {
        ...
        description: "Delete a supplier after identifying the record and obtaining explicit confirmation.",
    ...
    },
    {
        ...
        description: "Delete a product after identifying the record and obtaining explicit confirmation.",
    ...
    },
    {
        ...
        businessNouns: ["credit note"],
        permissionFlag: "delete",
        match: verbNoun(DELETE_VERBS, "sales\\s+credit\\s+notes?|credit\\s+notes?"),
    },
    {
        workflowId: "delete_sales_invoice",
        description: "Delete a sales invoice after identifying the record and obtaining explicit confirmation.",
    ...
    },
    {
        ...
        permissionFlag: "delete",
        match: verbNoun(DELETE_VERBS, "allocation\\s+resolvers?|allocations?", 80),
    },
    {
        ...
        businessNouns: ["nominal journal", "journal batch"],
        permissionFlag: "delete",
        match: verbNoun(DELETE_VERBS, "nominal\\s+journal(?:\\s+batch(?:es)?)?|journal\\s+batch(?:es)?"),
    },
    // --- Batch ---
    {
...
const TRANSACTIONAL_GROUPS = new Set([
    "update",
    "delete",
    "batch",
    "email",
]);
export function listActionWorkflows() {
...
export function getWorkflowForTool(toolName) {
    const name = toolName.trim();
    return ACTION_WORKFLOW_REGISTRY.find((entry) => entry.allowedTools.includes(name));
}
export function isTransactionalToolName(toolName) {
    return TRANSACTIONAL_GROUPS.has(getToolSkillGroup(toolName));
}
/** Tools currently enabled that require a routeToken. */
export function listEnabledTransactionalTools() {
    const tools = new Set();
 ...
 */
export function enabledToolsForWorkflow(definition) {
    return definition.allowedTools.filter((tool) => isToolEnabled(tool) && isTransactionalToolName(tool));
}
export function toActionWorkflow(definition, allowedTools) {
 ...
 */
function detectPrimaryAction(text) {
    const regex = /\b(add|create|set\s+up|setup|new|raise|prepare|record|post|update|change|edit|amend|modify|correct|delete|remove|erase|batch|bulk|import|email|send|mail)\b/i;
...
function earliestBusinessNounIndex(text, workflow, afterIndex) {
    const lower = text.toLowerCase();
    const indexes = workflow.businessNouns
        .map((noun) => lower.indexOf(noun.toLowerCase(), afterIndex))
        .filter((index) => index >= 0);
    return indexes.length > 0
        ? Math.min(...indexes)
        : Number.MAX_SAFE_INTEGER;
}
export function resolveWorkflowFromMessage(cleanedQuery) {
    const text = cleanedQuery.trim();
    if (!text) {
        return null;
    }
    const matches = ACTION_WORKFLOW_REGISTRY.filter((entry) => entry.match.test(text));
    if (matches.length === 0) {
        return null;
    }
    if (matches.length === 1) {
    ...
    const toolMention = matches.find((entry) => entry.allowedTools.some((tool) => new RegExp(`\\b${tool.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(text)));
    if (toolMention) {
        return toolMention;
    }
    // Explicit batch intent wins over an earlier create verb such as "prepare".
    if (hasExplicitBatchIntent(text)) {
        const batchMatches = matches.filter((entry) => entry.workflowId.startsWith("batch_"));
        ...
        if (batchMatches.length > 1) {
            ...
            batchMatches.sort((a, b) => earliestBusinessNounIndex(text, a, afterIndex) -
    ...
    const sameAction = matches.filter((entry) => entry.workflowId.startsWith(`${primary.action}_`));
    if (sameAction.length === 0) {
        return matches[0];
    }
    sameAction.sort((a, b) => earliestBusinessNounIndex(text, a, primary.index) -
...
export function sampleUtteranceForWorkflow(definition) {
