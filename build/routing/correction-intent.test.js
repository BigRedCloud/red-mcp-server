...
import { CORRECTION_ASSISTANT_GUIDANCE, CORRECTION_CUSTOMER_LANGUAGE_RULES, correctionGuidanceClaimsDeleteIsOnlyCorrection, correctionGuidanceContainsJargon, correctionGuidanceMakesUnverifiedAccountingEffectClaim, correctionGuidanceProposesUnverifiedOpposite, customerFacingReversalContainsInventedMethod, explainDeletedRecordUndo, explainFinancialReverse, explainLinkedQuoteInvoice, explainQuoteReferenceUndo, isCorrectionIntent, } from "./correction-intent.js";
...
test("reverse Cash Payment does not suggest creating a Cash Receipt", async () => {
    ...
    assert.equal(correctionGuidanceProposesUnverifiedOpposite(CORRECTION_CUSTOMER_LANGUAGE_RULES), false);
    assert.match(routed.guidance, /Do not assume Cash Payment is reversed with a Cash Receipt/i);
    assert.match(routed.guidance, /Do not invent negative or opposite transactions/i);
});
test("reverse Cash Payment does not claim unverified accounting effects", async () => {
    ...
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim(CORRECTION_CUSTOMER_LANGUAGE_RULES), false);
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim("Deleting the payment puts €12.30 back as outstanding on the supplier's account."), true);
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim("After deletion, the supplier will show €12.30 as still owed."), true);
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim("It'll just look like the payment never existed."), true);
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim("The safest concrete option I can actually execute is delete."), true);
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim("Deletion will restore the supplier allocation and ledger balance."), true);
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim("Removing it will clear the VAT position."), true);
});
...
test("staging: Reverse Cash Payment does not say delete is the only supported correction", async () => {
    ...
    assert.equal(routed.routeToken, undefined);
    assert.equal(routed.blockTransactionalTools, true);
    assert.match(routed.guidance, /Do not say deletion or removal is the only supported correction/i);
    assert.match(routed.guidance, /both removing the existing record and changing supported details/i);
    ...
    const stagingDeleteOnly = "Red can delete this cash payment record. That's the only supported correction action available for a cash payment.";
    assert.equal(correctionGuidanceClaimsDeleteIsOnlyCorrection(stagingDeleteOnly), true);
    assert.equal(correctionGuidanceClaimsDeleteIsOnlyCorrection("Red has a supported action to permanently delete this cash payment record. That's the only supported operation for this record type"), true);
});
test("assembled runtime guidance for Reverse Cash Payment is not delete-only", async () => {
    ...
    assert.equal(routed.preferredTools.includes("brc_delete_cash_payment"), false);
    assert.equal(routed.preferredTools.includes("brc_update_cash_payment"), false);
    ...
    assert.match(routed.guidance, /Neither is automatically the correct accounting treatment/i);
    assert.match(routed.guidance, /Do not say deletion is the only supported operation for this record type/i);
    assert.equal(/\bbrc_/i.test(routed.guidance), false);
    assert.equal(correctionGuidanceClaimsDeleteIsOnlyCorrection(routed.guidance), false);
    assert.equal(correctionGuidanceProposesUnverifiedOpposite(routed.guidance), false);
    assert.equal(correctionGuidanceMakesUnverifiedAccountingEffectClaim(routed.guidance), false);
