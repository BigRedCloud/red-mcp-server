 ...
 * reference to QT0003").
 */
export function isCorrectionIntent(message) {
...
export function explainQuoteReferenceUndo(args) {
    ...
    return (`I can change that back. The quote reference was changed from ${args.fromReference} to ${args.toReference}. ` +
        `I would change it back to ${args.fromReference} and leave the rest of the quote unchanged. Would you like me to do that?`);
}
export function explainDeletedRecordUndo(recordLabel = "record") {
    return (`That ${recordLabel} was deleted, so I can't simply switch the deletion off. ` +
...
export function explainFinancialReverse(recordLabel = "payment") {
    if (/cash payment/i.test(recordLabel)) {
        return ("I have checked the Cash Payment and have not changed anything. " +
    ...
    return (`I have checked the ${recordLabel} and have not changed anything. ` +
        `"Reverse" can mean different things depending on what needs correcting. ` +
...
function clauseLooksLikeProhibition(text, matchIndex) {
    const start = text.lastIndexOf(".", matchIndex);
    const clause = text.slice(start === -1 ? 0 : start + 1, matchIndex).trimStart();
    return /^(?:do not|don't|never|will not|won't|i will not|i have not|i haven't)\b/i.test(clause);
}
function hasUnprohibitedMatch(text, pattern) {
 ...
 */
export function correctionGuidanceProposesUnverifiedOpposite(text) {
    return [
 ...
 */
export function correctionGuidanceMakesUnverifiedAccountingEffectClaim(text) {
    ...
    if (/\bsafest (?:concrete )?option\b/i.test(text) &&
        /\b(?:delete|deleting|remove|removing)\b/i.test(text) &&
        !/\b(?:not|don't|do not)\b.{0,60}\bsafest\b/i.test(text)) {
        return true;
    }
    if (/\bwill (?:restore|increase|decrease|clear|change|become|show)\b.{0,80}\b(?:outstanding|allocation|ledger|VAT|history)\b/i.test(text)) {
        return true;
    }
    if (/\b(?:outstanding balance|allocation|ledger balance|VAT position).{0,40}\bwill\b/i.test(text) &&
        !/\b(?:have not verified|will not assume|won't assume|do not claim)\b/i.test(text)) {
        return true;
    }
    return false;
}
export function explainLinkedQuoteInvoice() {
    return ("That quote and the sales invoice created from it are now linked. " +
...
const FORBIDDEN_CUSTOMER_JARGON = /\b(?:rollback|mutation|inverse payload|compensating API call|endpoint|route token|routeToken|object clone|database rollback|HTTP|JSON|MCP)\b/i;
const FORBIDDEN_INTERNAL_TOOL_NAME = /\bbrc_[a-z0-9_]+\b/i;
const FORBIDDEN_HTTP_METHODS = /\b(?:POST|PUT|DELETE|GET|PATCH)\b/;
export function correctionGuidanceContainsJargon(text) {
 ...
 */
export function customerFacingReversalContainsInventedMethod(text) {
    return CUSTOMER_FACING_INVENTED_REVERSAL_PATTERNS.some((pattern) => pattern.test(text));
}
/**
 ...
 */
export function correctionGuidanceClaimsDeleteIsOnlyCorrection(text) {
    return [
