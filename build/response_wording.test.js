...
import { describeWriteStatusForUser, getCredentialForCompany, RED_ACTIVITY_SCOPE_INSTRUCTION, } from "./shared.js";
...
test("403 reuses the connection-no-longer-valid wording", () => {
    assert.match(describeWriteStatusForUser(403), /connection was no longer valid|needed to be refreshed/);
});
...
test("Red activity scope instruction excludes unrelated chat history", () => {
    ...
    assert.match(RED_ACTIVITY_SCOPE_INSTRUCTION, /unless the user explicitly asks for broader chat history/);
});
