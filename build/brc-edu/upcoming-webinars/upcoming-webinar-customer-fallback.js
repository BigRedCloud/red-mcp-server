 ...
 */
export function isUpcomingWebinarScheduleQuery(question) {
    const q = question.trim();
    if (!q) {
        return false;
    }
    if (/\brecorded\b/i.test(q) &&
        !/\b(upcoming|coming\s+up|scheduled|next|soon)\b/i.test(q)) {
        return false;
    }
    return (/\b(upcoming|coming\s+up|scheduled|next|soon)\b[\s\S]{0,40}\bwebinars?\b/i.test(q) ||
        /\bwebinars?\b[\s\S]{0,40}\b(upcoming|coming\s+up|scheduled|next|soon)\b/i.test(q) ||
        /\bare there any (upcoming\s+)?webinars?\b/i.test(q) ||
        /\bany upcoming webinars?\b/i.test(q));
}
export function buildEmptyUpcomingWebinarCustomerMarkdown(scheduleUrl = UPCOMING_WEBINAR_SCHEDULE_URL) {
    return [
