...
import { buildEmptyUpcomingWebinarCustomerMarkdown, isUpcomingWebinarScheduleQuery, } from "./upcoming-webinar-customer-fallback.js";
...
test("isUpcomingWebinarScheduleQuery detects schedule questions", () => {
    assert.equal(isUpcomingWebinarScheduleQuery("What webinars are coming up?"), true);
    assert.equal(isUpcomingWebinarScheduleQuery("Are there any upcoming webinars?"), true);
    assert.equal(isUpcomingWebinarScheduleQuery("Find a recorded webinar about sales invoices"), false);
});
test("empty upcoming webinar markdown points to schedule page and email without claiming none exist", () => {
    ...
    assert.match(text, /couldn’t find any upcoming webinars in Red’s currently available webinar listings/i);
