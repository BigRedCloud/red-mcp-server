...
import { parseUpcomingWebinarsFromHtml, shouldIgnoreRecordedWebinarCard, } from "./upcoming-webinar-parser.js";
const UPCOMING_HTML = `
<section>
...
test("parseUpcomingWebinarsFromHtml indexes only upcoming webinars", () => {
    const webinars = parseUpcomingWebinarsFromHtml(UPCOMING_HTML, "https://bigredcloud.com/webinar-series/", "2026-07-15T10:00:00.000Z");
    ...
    assert.equal(webinars.some((item) => item.title.includes("Getting Started with Big Red Cloud")), false);
});
test("parseUpcomingWebinarsFromHtml preserves weekday and Zoom registration URL", () => {
    ...
    assert.equal(bankFeeds?.registrationUrl, "https://zoom.us/meeting/register/test-bank-feeds");
...
test("shouldIgnoreRecordedWebinarCard ignores recorded cards and YouTube links", () => {
    assert.equal(shouldIgnoreRecordedWebinarCard("Recorded Webinar Watch Now on YouTube"), true);
    assert.equal(shouldIgnoreRecordedWebinarCard("Feature Training: Bank Feeds"), false);
});
