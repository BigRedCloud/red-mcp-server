...
import { mergeFreshdeskCatalogWithOverrides, toFreshdeskAdminArticle, upsertFreshdeskArticleOverride, visibleFreshdeskArticlesFromEffective, } from "./freshdesk-catalog-merge.js";
function article(id, title, folderName = "Sales Invoices") {
    return {
        id: `freshdesk-${id}`,
        source: "freshdesk",
...
test("mergeFreshdeskCatalogWithOverrides applies exclusions by articleId", () => {
    const overrides = upsertFreshdeskArticleOverride({}, "2", { excluded: true, reason: "Staff only", excludedBy: "staff@example.com" });
    ...
    assert.equal(visible.some((item) => item.freshdeskArticleId === 2), false);
});
...
test("exclusions survive a fresh raw catalogue merge", () => {
    ...
    assert.equal(afterSync.items.find((item) => item.articleId === "5")?.excluded, true);
    assert.equal(afterSync.items.find((item) => item.articleId === "6")?.excluded, false);
