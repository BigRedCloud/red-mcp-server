...
import { buildFreshdeskPublicArticleUrl, extractSlugFromLegacyBigredcloudSupportUrl, FRESHDESK_LINK_RESPONSE_GUIDANCE, FRESHDESK_PUBLIC_ARTICLES_BASE_URL, getSyncedFreshdeskArticlePublicUrl, isFreshdeskPublicArticleUrl, isLegacyBigredcloudSupportArticleUrl, readFreshdeskArticleUrlFields, repairStoredFreshdeskArticlePublicUrl, resolveFreshdeskArticlePublicUrl, slugifyFreshdeskArticleTitle, } from "./freshdesk-article-url.js";
...
const BANK_RECON_LEGACY_URL = "https://bigredcloud.com/support/how-do-i-do-the-bank-reconciliation-bank-rec/";
...
test("slugifyFreshdeskArticleTitle matches bank reconciliation regression slug", () => {
    assert.equal(slugifyFreshdeskArticleTitle("How do I do the Bank Reconciliation (Bank Rec)?"), "how-do-i-do-the-bank-reconciliation-bank-rec-");
});
test("slugifyFreshdeskArticleTitle matches add-a-customer regression slug", () => {
    assert.equal(slugifyFreshdeskArticleTitle("How do I add a Customer?"), "how-do-i-add-a-customer-");
});
...
test("add-a-customer regression returns canonical URL pattern from title", () => {
    ...
    assert.equal(url, `${FRESHDESK_PUBLIC_ARTICLES_BASE_URL}${articleId}-how-do-i-add-a-customer-`);
});
test("canonical URL is built from article ID and API slug field", () => {
    ...
    assert.equal(url, `${FRESHDESK_PUBLIC_ARTICLES_BASE_URL}1001-complete-a-bank-reconciliation`);
});
test("API path under /support/solutions/articles/ is accepted", () => {
    const url = resolveFreshdeskArticlePublicUrl({
        freshdeskArticleId: 1001,
        title: "Fallback title",
        apiPath: "/support/solutions/articles/1001-complete-a-bank-reconciliation",
    });
    assert.equal(url, "https://bigredcloud.freshdesk.com/support/solutions/articles/1001-complete-a-bank-reconciliation");
});
...
test("article ID and title always produce a publicUrl", () => {
    ...
    assert.equal(url, "https://bigredcloud.freshdesk.com/support/solutions/articles/42-create-a-sales-invoice");
});
...
test("legacy wrong URL is repaired using slug extracted from legacy path when title is absent", () => {
    const legacySlug = extractSlugFromLegacyBigredcloudSupportUrl(BANK_RECON_LEGACY_URL);
    assert.equal(legacySlug, "how-do-i-do-the-bank-reconciliation-bank-rec");
    const repaired = repairStoredFreshdeskArticlePublicUrl({
        freshdeskArticleId: BANK_RECON_FRESHDESK_ARTICLE.freshdeskArticleId,
        publicUrl: BANK_RECON_LEGACY_URL,
    });
    assert.equal(repaired.publicUrl, buildFreshdeskPublicArticleUrl(BANK_RECON_FRESHDESK_ARTICLE.freshdeskArticleId, "how-do-i-do-the-bank-reconciliation-bank-rec"));
});
...
test("invalid host is rejected", () => {
    assert.equal(isFreshdeskPublicArticleUrl("https://evil.example.com/support/solutions/articles/1001-slug"), false);
});
test("HTTP URL is rejected", () => {
    assert.equal(isFreshdeskPublicArticleUrl("http://bigredcloud.freshdesk.com/support/solutions/articles/1001-slug"), false);
});
test("malformed URL is rejected", () => {
    assert.equal(isFreshdeskPublicArticleUrl("not-a-url"), false);
    assert.equal(isFreshdeskPublicArticleUrl("javascript:alert(1)"), false);
});
