...
import { createConfiguredFreshdeskIndexContainer, saveFreshdeskArticlesIndex, } from "./freshdesk-index-store.js";
import { createFreshdeskImageContainer, } from "./image-sync.js";
import { getFreshdeskKbImageContainerName, getFreshdeskKbStorageConnectionString, } from "./freshdesk-kb-storage.js";
import { syncFreshdeskKnowledgeBase, } from "./freshdesk-sync-service.js";
import { mergeFreshdeskCatalogWithOverrides, summarizeFreshdeskEffectiveCatalog, toFreshdeskAdminArticle, upsertFreshdeskArticleOverride, } from "./freshdesk-catalog-merge.js";
import { createConfiguredFreshdeskCatalogContainer, emptyFreshdeskSyncStatus, loadFreshdeskEffectiveCatalog, loadFreshdeskOverrides, loadFreshdeskRawArticles, loadFreshdeskSyncStatus, saveFreshdeskEffectiveCatalog, saveFreshdeskOverrides, saveFreshdeskSyncStatus, toSafeFreshdeskCatalogStorageError, } from "./freshdesk-catalog-store.js";
import { invalidateFreshdeskHelpIndexCache } from "./freshdesk-help-search.js";
function getFreshdeskClientFromEnv() {
    const apiKey = process.env.FRESHDESK_API_KEY?.trim();
    if (!apiKey) {
        return null;
    }
    const baseUrl = process.env.FRESHDESK_BASE_URL?.trim() ||
        "https://bigredcloud.freshdesk.com";
    return new FreshdeskClient(baseUrl, apiKey);
}
function getImageContainerFromEnv() {
    const connection = getFreshdeskKbStorageConnectionString();
    if (!connection) {
        return null;
    }
    return createFreshdeskImageContainer(connection, getFreshdeskKbImageContainerName());
}
...
export async function runFreshdeskCatalogSync(source, deps = {}) {
    const now = deps.now ?? new Date();
    const nowIso = now.toISOString();
    const catalogContainer = deps.catalogContainer === undefined
        ? createConfiguredFreshdeskCatalogContainer()
        : deps.catalogContainer;
    const imageContainer = deps.imageContainer === undefined
        ? getImageContainerFromEnv()
        : deps.imageContainer;
    const client = deps.client === undefined ? getFreshdeskClientFromEnv() : deps.client;
    ...
    if (!client) {
        const error = "Freshdesk sync is not configured. Set FRESHDESK_API_KEY.";
    ...
    if (!imageContainer) {
        const error = "Freshdesk image storage is not configured. Set BRC_EDU_KB_STORAGE_CONNECTION or BRC_EDU_UPLOAD_STORAGE_CONNECTION_STRING.";
        status = { ...status, lastErrorSummary: error };
        await saveFreshdeskSyncStatus(catalogContainer, status);
        return {
            ok: false,
    ...
    try {
        ...
        if (syncResult.articles.length === 0) {
            const error = "Freshdesk returned no published articles. The previous catalogue was preserved.";
            status = { ...status, lastErrorSummary: error };
            await saveFreshdeskSyncStatus(catalogContainer, status);
            return {
                ok: false,
...
export async function updateFreshdeskArticleVisibility(params) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt += 1) {
        ...
        const rawArticle = raw.articles.find((item) => String(item.freshdeskArticleId) === articleId);
        if (!rawArticle) {
            return {
                ok: false,
                status: 404,
        ...
        const nextOverrides = upsertFreshdeskArticleOverride(overridesLoad.document.overrides, articleId, {
        ...
        try {
            ...
            if (statusCode === 409) {
                return {
                    ok: false,
                    status: 409,
                    error: "Another administrator updated article visibility at the same time. Please retry.",
                };
            }
            return {
                ok: false,
...
export async function loadFreshdeskArticlesForAdmin(container = createConfiguredFreshdeskCatalogContainer()) {
    if (!container) {
        return {
            ok: false,
            status: 503,
