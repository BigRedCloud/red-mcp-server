...
const SECRET_CONNECTION_STRING = "DefaultEndpointsProtocol=https;AccountName=secret;AccountKey=super-secret-key;EndpointSuffix=core.windows.net";
function sha256Hex(buffer) {
    return createHash("sha256").update(buffer).digest("hex");
}
function createImageResponse(buffer, contentType, status = 200) {
...
function createMockContainer(options = {}) {
    ...
    const container = {
        ...
        getBlockBlobClient: (blobName) => ({
            ...
            uploadData: async (buffer, uploadOptions) => {
...
function imageRef(sourceUrl) {
    return { sourceUrl, altText: null };
}
test("syncFreshdeskImages rejects non-HTTPS URLs", async () => {
    const { container } = createMockContainer();
    await assert.rejects(() => syncFreshdeskImages(ARTICLE_ID, [imageRef("http://cdn.freshdesk.com/image.png")], container), /Freshdesk image URL must use HTTPS/);
});
test("syncFreshdeskImages rejects unexpected hosts", async () => {
    const { container } = createMockContainer();
    await assert.rejects(() => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://evil.example.com/image.png")], container), /Freshdesk image host is not allowed: evil.example.com/);
});
test("syncFreshdeskImages rejects unsupported content types", async () => {
    const buffer = Buffer.from("not-an-image");
    const { container } = createMockContainer();
    await withMockFetch(() => createImageResponse(buffer, "text/plain"), async () => {
        await assert.rejects(() => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/file.bin")], container), /Unsupported Freshdesk image type: text\/plain/);
    });
});
test("syncFreshdeskImages rejects images over the configured size limit", async () => {
    ...
    const { container } = createMockContainer();
    await withMockFetch(() => createImageResponse(oversized, "image/png"), async () => {
        await assert.rejects(() => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/huge.png")], container), new RegExp(`Freshdesk image exceeds ${MAX_IMAGE_BYTES} bytes`));
    });
});
test("syncFreshdeskImages calculates a stable SHA-256 hash", async () => {
    const buffer = Buffer.from("fake-png-bytes");
    const { container } = createMockContainer();
    const synced = await withMockFetch(() => createImageResponse(buffer, "image/png"), () => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/stable.png")], container));
...
test("syncFreshdeskImages creates blob names under freshdesk/{articleId}/{sha256}.{extension}", async () => {
    ...
    const { container } = createMockContainer();
    const synced = await withMockFetch(() => createImageResponse(buffer, "image/png"), () => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/path.png")], container));
...
test("syncFreshdeskImages maps png, jpeg, gif and webp to correct extensions", async () => {
    ...
    for (const { contentType, extension } of cases) {
        ...
        const { container } = createMockContainer();
        const synced = await withMockFetch(() => createImageResponse(buffer, contentType), () => syncFreshdeskImages(ARTICLE_ID, [
            imageRef(`https://s3.amazonaws.com/freshdesk/${contentType.replace("/", "-")}`),
        ], container));
        assert.equal(synced[0]?.blobName, `freshdesk/${ARTICLE_ID}/${sha256Hex(buffer)}${extension}`, contentType);
    }
});
test("syncFreshdeskImages uploads when the blob does not exist", async () => {
    ...
    await withMockFetch(() => createImageResponse(buffer, "image/png"), () => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/new.png")], container));
...
test("syncFreshdeskImages skips upload when the blob already exists", async () => {
    ...
    await withMockFetch(() => createImageResponse(buffer, "image/png"), () => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/existing.png")], container));
    assert.equal(uploads.length, 0);
});
test("syncFreshdeskImages sets content type, cache control and metadata", async () => {
    const buffer = Buffer.from("metadata-test");
    const { container, uploads } = createMockContainer();
    await withMockFetch(() => createImageResponse(buffer, "image/jpeg"), () => syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/meta.jpg")], container));
    const upload = uploads[0];
    assert.equal(upload?.options.blobHTTPHeaders?.blobContentType, "image/jpeg");
    assert.equal(upload?.options.blobHTTPHeaders?.blobCacheControl, "private, max-age=31536000, immutable");
...
test("syncFreshdeskImages returns the expected synced-image metadata", async () => {
    ...
    const { container } = createMockContainer();
    const synced = await withMockFetch(() => createImageResponse(buffer, "image/png"), () => syncFreshdeskImages(ARTICLE_ID, [{ sourceUrl, altText: "ignored by sync" }], container));
...
test("syncFreshdeskImages does not expose storage connection strings in errors", async () => {
    ...
    const { container } = createMockContainer({
        uploadError: new Error(`Upload failed for ${SECRET_CONNECTION_STRING}`),
    });
    await withMockFetch(() => createImageResponse(buffer, "image/png"), async () => {
        try {
            await syncFreshdeskImages(ARTICLE_ID, [imageRef("https://cdn.freshdesk.com/fail.png")], container);
...
test("syncFreshdeskImages is idempotent for identical image bytes", async () => {
    ...
    const first = await withMockFetch(fetchHandler, () => syncFreshdeskImages(ARTICLE_ID, [imageRef(sourceUrl)], container));
    const second = await withMockFetch(fetchHandler, () => syncFreshdeskImages(ARTICLE_ID, [imageRef(sourceUrl)], container));
    assert.equal(uploads.length, 1);
