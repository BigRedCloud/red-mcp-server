...
export function isFreshdeskPublicArticleUrl(value) {
    ...
    try {
        ...
        const segment = parsed.pathname.slice(FRESHDESK_PUBLIC_ARTICLES_PATH_PREFIX.length);
...
export function isLegacyBigredcloudSupportArticleUrl(value) {
    try {
        const parsed = new URL(value.trim());
        if (parsed.protocol !== "https:") {
            return false;
        }
        return (parsed.hostname.toLowerCase() === LEGACY_BIGREDCLOUD_SUPPORT_HOST &&
...
export function extractSlugFromLegacyBigredcloudSupportUrl(value) {
...
export function buildFreshdeskPublicArticleUrl(freshdeskArticleId, slug) {
...
export function readFreshdeskArticleUrlFields(article) {
    return {
...
function parseArticleSegmentFromPath(path) {
...
function resolveFromApiPath(freshdeskArticleId, apiPath) {
    ...
    if (parsedSegment) {
        ...
        const url = buildFreshdeskPublicArticleUrl(freshdeskArticleId, parsedSegment.slug);
...
export function resolveFreshdeskArticlePublicUrl(input) {
    ...
    if (input.apiSlug) {
        const fromSlug = buildFreshdeskPublicArticleUrl(input.freshdeskArticleId, input.apiSlug);
