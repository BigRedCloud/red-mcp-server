...
import { buildCustomerFacingInstructionMarkdown, buildCustomerFacingScreenshotMarkdown, buildScreenshotLinkLabel, buildScreenshotLinkMarkdown, buildScreenshotLinksMarkdown, buildScreenshotMarkdownTextBlock, resolveHelpImagePresentation, SCREENSHOT_MARKDOWN_COPY_INSTRUCTION, toScreenshotMarkdownLink, } from "./screenshot-markdown.js";
...
const SIGNED_A = "https://red.example.com/public/brc-edu/freshdesk-images/1/token-a";
const SIGNED_B = "https://red.example.com/public/brc-edu/freshdesk-images/1/token-b";
...
test("buildScreenshotLinksMarkdown uses View image with exact URL", () => {
    const longCaption = "Click Bank Accounts > select the relevant Bank Account > click Ledger";
...
test("buildCustomerFacingScreenshotMarkdown joins View image links", () => {
    ...
    assert.equal(markdown, `[View image](${SIGNED_A})\n\n[View image](${SIGNED_B})`);
});
test("buildCustomerFacingInstructionMarkdown uses View image beside steps", () => {
    ...
    assert.match(markdown ?? "", /^1\. Click Bank Accounts → select the relevant Bank Account → click Ledger\./);
    assert.match(markdown ?? "", /\[View image\]\(https:\/\/red\.example\.com/);
    assert.equal(markdown?.includes(longCaption), false);
    assert.equal(markdown?.includes("Changing a customer: Open O/Balance"), false);
    assert.match(markdown ?? "", /2\. Click O\/Balance\./);
    assert.match(markdown ?? "", /\[View image\]\(https:\/\/red\.example\.com\/public\/brc-edu\/freshdesk-images\/1\/token-b\)/);
});
...
test("internal caption remains available while link uses View image", () => {
    ...
    assert.equal(screenshot.caption, "Bank reconciliation: Select the account and click Ledger");
    assert.equal(buildScreenshotLinkMarkdown(screenshot), `[View image](${SIGNED_A})`);
});
test("rejected descriptive captions still emit View image links", () => {
    ...
    assert.equal(toScreenshotMarkdownLink("Show Image", "https://red.example.com/public/brc-edu/freshdesk-images/1/t"), "[View image](https://red.example.com/public/brc-edu/freshdesk-images/1/t)");
