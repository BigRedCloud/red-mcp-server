...
import { buildFreshdeskArticlesIndex, createReadableStreamFromBuffer, FRESHDESK_ARTICLES_INDEX_BLOB_PATH, FRESHDESK_ARTICLES_INDEX_CACHE_CONTROL, FRESHDESK_ARTICLES_INDEX_CONTENT_TYPE, loadFreshdeskArticlesIndex, normalizeLoadedFreshdeskArticlesIndex, saveFreshdeskArticlesIndex, serializeFreshdeskArticlesIndex, toSafeIndexStorageErrorMessage, } from "./freshdesk-index-store.js";
...
function createSyncedArticle(overrides = {}) {
    return {
        id: "freshdesk-101",
        source: "freshdesk",
...
function createMockContainer(initialBlobs = new Map()) {
...
test("serializeFreshdeskArticlesIndex uses deterministic formatting", () => {
    const index = buildFreshdeskArticlesIndex(createSyncResult(), new Date("2026-07-14T12:00:00.000Z"));
...
test("loadFreshdeskArticlesIndex loads an existing index", async () => {
    ...
    const index = buildFreshdeskArticlesIndex(syncResult, new Date("2026-07-14T13:00:00.000Z"));
    const body = Buffer.from(serializeFreshdeskArticlesIndex(index), "utf8");
    const { container } = createMockContainer(new Map([[FRESHDESK_ARTICLES_INDEX_BLOB_PATH, { body }]]));
...
test("loadFreshdeskArticlesIndex throws a safe error for malformed JSON", async () => {
    ...
    const { container } = createMockContainer(new Map([[FRESHDESK_ARTICLES_INDEX_BLOB_PATH, { body }]]));
    await assert.rejects(() => loadFreshdeskArticlesIndex(container), /Freshdesk articles index JSON is malformed/);
});
test("loadFreshdeskArticlesIndex storage errors do not expose secrets", async () => {
    ...
    try {
        ...
        assert.equal(message, "Freshdesk articles index storage operation failed.");
...
test("buildFreshdeskArticlesIndex sets article and failure counts correctly", () => {
    ...
    const index = buildFreshdeskArticlesIndex(syncResult, new Date("2026-07-14T14:00:00.000Z"));
...
test("toSafeIndexStorageErrorMessage redacts SAS tokens and authorization values", () => {
    assert.equal(toSafeIndexStorageErrorMessage(new Error("https://account.blob.core.windows.net/c?sv=2021&sig=abc123")), "Freshdesk articles index storage operation failed.");
    assert.equal(toSafeIndexStorageErrorMessage(new Error("Authorization: Basic dXNlcjpwYXNzd29yZA==")), "Freshdesk articles index storage operation failed.");
});
...
test("loadFreshdeskArticlesIndex repairs legacy articles missing publicUrl", async () => {
    const legacyIndex = {
        ...
        articles: [
            ...
            createSyncedArticle({
                ...
                publicUrl: "https://bigredcloud.com/support/how-do-i-add-a-customer/",
                slug: null,
            }),
        ],
        failures: [],
    ...
    assert.equal(loaded?.articles[0]?.publicUrl, "https://bigredcloud.freshdesk.com/support/solutions/articles/157000368991-how-do-i-do-the-bank-reconciliation-bank-rec-");
    assert.equal(loaded?.articles[1]?.publicUrl, "https://bigredcloud.freshdesk.com/support/solutions/articles/157000123456-how-do-i-add-a-customer-");
});
    ...
    const legacyIndex = {
        ...
        articles: [
            createSyncedArticle({
                ...
                title: "How do I do the Bank Reconciliation (Bank Rec)?",
                slug: "how-do-i-do-the-bank-reconciliation-bank-rec-",
                publicUrl: "https://bigredcloud.com/support/how-do-i-do-the-bank-reconciliation-bank-rec/",
            }),
        ],
        failures: [],
    };
...
test("normalizeLoadedFreshdeskArticlesIndex assigns publicUrl to every article with ID and title", () => {
    ...
    assert.equal(index.articles.filter((article) => article.publicUrl).length, 3);
    for (const article of index.articles) {
        assert.match(article.publicUrl ?? "", /^https:\/\/bigredcloud\.freshdesk\.com\/support\/solutions\/articles\//);
        assert.equal(isLegacyBigredcloudSupportArticleUrl(article.publicUrl ?? ""), false);
    }
});
