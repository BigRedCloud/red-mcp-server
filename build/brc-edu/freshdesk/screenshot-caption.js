...
const GENERIC_ALT_PATTERN = /^(image|images|img|screenshot|screenshots|photo|picture|graphic|diagram|untitled|show image|a screenshot of a computer|screenshot\s*\d+|image\s*\d+|relevant article section|freshdesk screenshot)$/i;
...
export function instructionTextToCaption(text, nearbyHeading) {
    ...
    if (clickThenClick) {
        return truncateCaption(`${titleCaseWords(clickThenClick[1] ?? "")} — click ${clickThenClick[2]}`);
    }
...
function headingToCaption(heading) {
