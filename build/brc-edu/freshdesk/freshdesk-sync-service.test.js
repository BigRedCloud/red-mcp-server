...
import { FRESHDESK_EXCLUDED_FOLDER_IDS, FRESHDESK_SYNC_CATEGORY_ID, syncFreshdeskKnowledgeBase, toSafeSyncErrorMessage, } from "./freshdesk-sync-service.js";
...
function createMockClient(options) {
    return {
        ...
        getArticles: async (folderId) => options.articlesByFolder?.get(folderId) ?? [],
...
function createMockSyncImages(handler) {
    return async (articleId, images, _container) => {
        if (handler) {
...
test("syncFreshdeskKnowledgeBase stores canonical Freshdesk publicUrl from API slug", async () => {
    ...
    assert.equal(result.articles[0]?.publicUrl, "https://bigredcloud.freshdesk.com/support/solutions/articles/3001-how-do-i-do-the-bank-reconciliation-bank-rec-");
});
...
test("syncFreshdeskKnowledgeBase syncs multiple folders and articles", async () => {
    ...
    assert.deepEqual(result.articles.map((article) => article.title).sort(), ["Article A1", "Article A2", "Article B1"]);
    assert.deepEqual(result.failures, []);
});
...
test("syncFreshdeskKnowledgeBase attaches synced image metadata", async () => {
    ...
    const article = createArticle({
        id: 6001,
        folder_id: 600,
        description: '<img src="https://cdn.freshdesk.com/one.png" alt="One" />' +
...
test("syncFreshdeskKnowledgeBase continues when one article fails", async () => {
    ...
    const client = createMockClient({
        ...
        getArticleErrors: new Map([
            [
                7002,
                new Error(`Freshdesk request failed with Authorization: Basic ${Buffer.from(`${API_KEY}:X`, "utf8").toString("base64")}`),
            ],
        ]),
    });
    const result = await syncFreshdeskKnowledgeBase(client, MOCK_CONTAINER, {
...
test("toSafeSyncErrorMessage removes API keys and Azure connection strings", () => {
    ...
    assert.equal(toSafeSyncErrorMessage(new Error(`Authorization: Basic ${encodedAuth}`)), "Freshdesk article sync failed.");
    assert.equal(toSafeSyncErrorMessage(new Error(`Upload failed: ${CONNECTION_STRING}`)), "Freshdesk article sync failed.");
    assert.equal(toSafeSyncErrorMessage(new Error("Freshdesk image download failed with status 404.")), "Freshdesk image download failed with status 404.");
});
