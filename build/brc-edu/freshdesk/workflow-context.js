...
const CLICK_TARGET_PATTERN = /\b(?:click|open|press|tap)\s+["']?([A-Za-z0-9][A-Za-z0-9 /&-]{0,40}?)["']?(?=\s*(?:[.!?,;]|on\s+the\b|then\b|and\b|$))/gi;
const MUTUALLY_EXCLUSIVE_PAIRS = [
    ["add_customer", "existing_customer"],
    ["manual_allocations", "non_manual_allocations"],
];
export function extractNearbyActions(...texts) {
    const joined = texts
 ...
 */
export function classifyFreshdeskWorkflows(...texts) {
    const joined = texts
        .map((text) => normalizeText(text ?? ""))
        .filter(Boolean)
        .join(" ")
    ...
    const tags = new Set();
    if (/\bemail\s+preferences\b/.test(joined) ||
        /\bemail\s+settings\b/.test(joined)) {
        tags.add("email_preferences");
    }
    if (/\bbank\s+reconciliation\b/.test(joined) ||
        /\breconcile\b/.test(joined) ||
        /\bstatement\s+balance\b/.test(joined)) {
        tags.add("bank_reconciliation");
    }
    if (/\bmanual\s+allocations?\b/.test(joined) &&
        !/\b(without|if you (do )?not|when manual allocations (are|is) (not |off|disabled))\b/.test(joined)) {
        tags.add("manual_allocations");
    }
    if (/\b(without|not using|do not use|if you (do )?not)\b.{0,40}\bmanual\s+allocations?\b/.test(joined) ||
        (/\b(current|1\s+month|2\s+months?|3\s+months?\s+plus|age(?:d|ing)?)\b/.test(joined) &&
            /\bopening\s+balance\b/.test(joined))) {
        tags.add("non_manual_allocations");
    }
    if (/\bo\/\s*balance\b/.test(joined) ||
        /\bopening\s+balance\b/.test(joined) ||
    ...
    const mentionsAdd = /\bclick\s+add\b/.test(joined) ||
        /\badd\s+(a\s+)?(new\s+)?customer\b/.test(joined) ||
        /\badding\s+(a\s+)?(new\s+)?customer\b/.test(joined);
    const mentionsChange = /\bclick\s+change\b/.test(joined) ||
    ...
    if (/\bclick\s+save\b/.test(joined) ||
 ...
 */
export function primaryWorkflow(workflows, nearbyActions = []) {
    ...
    if (actionSet.has("bank reconciliation") ||
 ...
 */
export function selectWorkflowsFromQuestion(question) {
    ...
    const wantsExisting = /\balready\b/.test(q) ||
        /\bexisting\b/.test(q) ||
        /\bwho (already |)?owes\b/.test(q) ||
        /\bi('ve| have) added\b/.test(q) ||
        /\bchange\b/.test(q);
    const wantsNew = /\bbrand[- ]?new\b/.test(q) ||
    ...
    const wantsEmail = /\bemail\s+preferences\b/.test(q) || /\bemail\s+settings\b/.test(q);
    const wantsBank = /\bbank\s+reconciliation\b/.test(q) ||
        /\breconcile\b/.test(q) ||
    ...
    else if (/\bopening\s+balance\b/.test(q) ||
        /\bage(?:d|ing)?\b/.test(q) ||
        /\bowes?\b/.test(q)) {
...
export function blockMatchesSelectedWorkflows(blockWorkflows, selected) {
    const workflows = blockWorkflows && blockWorkflows.length > 0
        ? blockWorkflows
        : ["generic"];
    if (workflows.some((tag) => selected.exclude.has(tag))) {
        ...
        const exclusiveOnly = workflows.filter((tag) => tag === "add_customer" ||
    ...
    return workflows.some((tag) => tag === "generic" ||
        tag === "final_save" ||
        selected.include.has(tag));
}
export function workflowDisplayLabel(workflow) {
    switch (workflow) {
        case "add_customer":
...
export function actionsOverlapScore(left, right) {
...
export function textTokenOverlap(left, right) {
    const tokenize = (value) => new Set(normalizeText(value)
        .toLowerCase()
        .split(/[^a-z0-9/]+/)
...
export function resolveMutuallyExclusiveConflicts(selected) {
