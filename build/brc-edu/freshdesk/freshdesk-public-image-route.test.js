...
import { FRESHDESK_PUBLIC_IMAGE_ROUTE, handleFreshdeskPublicImageRequest, } from "./freshdesk-public-image-route.js";
...
function createIndexContainer(article) {
    const index = {
...
function createImageContainer(png) {
    return {
...
test("public image endpoint crops large black canvas before streaming", async () => {
    ...
    await handleFreshdeskPublicImageRequest({
        ...
        freshdeskIndexContainer: createIndexContainer(freshdeskArticle()),
        freshdeskImageContainer: createImageContainer(source),
    });
...
test("valid token streams the expected image with inline headers", async () => {
    ...
    await handleFreshdeskPublicImageRequest({
        ...
        freshdeskIndexContainer: createIndexContainer(freshdeskArticle()),
        freshdeskImageContainer: createImageContainer(png),
    });
    assert.equal(res.statusCode, 200);
...
test("HEAD returns headers without body", async () => {
    ...
    await handleFreshdeskPublicImageRequest({
        ...
        freshdeskIndexContainer: createIndexContainer(freshdeskArticle()),
        freshdeskImageContainer: createImageContainer(Buffer.from("png")),
    });
    assert.equal(res.statusCode, 200);
...
test("missing blob returns safe 404 without Azure details", async () => {
    ...
    await handleFreshdeskPublicImageRequest({
        ...
        freshdeskIndexContainer: createIndexContainer(freshdeskArticle()),
        freshdeskImageContainer: emptyImageContainer,
    });
...
test("unsupported MIME type is rejected", async () => {
    ...
    await handleFreshdeskPublicImageRequest({
        ...
        freshdeskIndexContainer: createIndexContainer(badArticle),
        freshdeskImageContainer: createImageContainer(Buffer.from("svg")),
    });
    assert.equal(res.statusCode, 404);
});
...
test("route path is customer-safe", () => {
    assert.equal(FRESHDESK_PUBLIC_IMAGE_ROUTE, "/public/brc-edu/freshdesk-images/:articleId/:imageToken");
});
test("accepted PNG JPEG WebP GIF content types are served", async () => {
    ...
    for (const [mimeType, blobName, imageKey] of [
        ...
        await handleFreshdeskPublicImageRequest({
            ...
            freshdeskIndexContainer: createIndexContainer(article),
