...
import { applyOverrideToRawVideo, mergeYouTubeCatalogWithOverrides, upsertYouTubeVideoOverride, } from "./youtube-catalog-merge.js";
...
test("restored video becomes visible after the next sync merge", () => {
    ...
    }, "vid-1", { excluded: false, updatedAt: "2024-01-04T00:00:00Z" });
    const merged = mergeYouTubeCatalogWithOverrides({
...
test("title and description updates do not clear exclusion", () => {
    const previous = applyOverrideToRawVideo(rawVideo({ videoId: "vid-1", category: "youtube_video" }), {
        excluded: true,
        excludedAt: "2024-01-03T00:00:00Z",
        excludedBy: "staff@example.com",
        reason: "Staff only",
