...
import { mergeYouTubeCatalogWithOverrides, upsertYouTubeVideoOverride, } from "./youtube-catalog-merge.js";
import { parseYouTubeEffectiveCatalog, parseYouTubeOverridesDocument, parseYouTubeRawCatalog, } from "./youtube-catalog-store.js";
...
test("failed catalogue writes refuse empty YouTube catalogues", async () => {
    ...
    await assert.rejects(() => saveYouTubeRawCatalog({}, {
        generatedAt: "2024-01-01T00:00:00Z",
        channelId: "channel-1",
        webinarPlaylistId: "PL",
        itemCount: 0,
...
test("manual YouTube sync succeeds without generating a workbook", async () => {
    ...
    assert.equal([...blobs.keys()].some((key) => key.includes("webinar_video_routing_index")), false);
    const overrides = parseYouTubeOverridesDocument(JSON.parse(blobs.get("brc-edu/youtube/video-overrides.json")?.body ?? "null"));
