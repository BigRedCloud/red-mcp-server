...
import { BIGREDCLOUD_DOCS_RULE, normalizeApprovedWebUrl, safeWebFetchText, } from "../help/safe-web-fetch.js";
...
function extractSummary(bodyText, maxLength = 220) {
...
function extractKeywords(title, category, headings) {
...
export function extractCustomerDocsLinks(html, baseUrl) {
...
export function isCustomerDocsArticleUrl(url) {
    try {
        const pathname = new URL(url).pathname.replace(/\/+$/, "");
...
export function parseCustomerDocsArticlePage(html, url, syncedAt) {
    const $ = cheerio.load(html);
    ...
    const category = cleanText($(".breadcrumb a").last().text()) ||
        cleanText($("h2").first().text()) ||
        "Documentation";
    const main = $("main").first().length > 0
        ? $("main").first()
...
export async function crawlCustomerDocumentation(options = {}) {
