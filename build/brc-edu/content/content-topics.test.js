...
import { compareContentTopics, resolveContentTopic, topicLabel, } from "./content-topics.js";
...
test("resolveContentTopic prefers folder aliases then inference", () => {
    assert.equal(resolveContentTopic({ folderName: "Sales Invoices", title: "Anything" })
        .topic, "sales_invoices");
    assert.equal(resolveContentTopic({ folderName: "Cash Book", title: "Guide" }).topic, "cash_book");
    assert.equal(resolveContentTopic({
...
test("buildContentOverview groups visible Freshdesk and YouTube by topic", async () => {
    ...
    const freshdeskItems = [
        {
            ...
            enabled: true,
            slug: "how-to",
            publicUrl: "https://bigredcloud.freshdesk.com/support/solutions/articles/1",
            syncedImages: [],
            articleId: "1",
            topic: "sales_invoices",
    ...
    const salesTopic = overview.topics.find((topic) => topic.topic === "sales_invoices" || topic.topic === "sales");
    assert.ok(salesTopic);
    assert.ok(salesTopic.items.length >= 1);
    assert.ok(salesTopic.items.every((item) => item.type !== undefined));
    assert.equal(overview.topics.some((topic) => topic.items.some((item) => item.id === "hidden")), false);
    const manageUrls = salesTopic.items.map((item) => item.manageUrl);
    assert.ok(manageUrls.some((url) => url?.includes("view=freshdesk")) ||
