...
import { findFreshdeskArticleById, getHelpResourceDetails, helpResourceDetailResponse, } from "./help-resource-details.js";
import { DEFAULT_BRC_FRESHDESK_EFFECTIVE_CATALOG_BLOB, DEFAULT_BRC_FRESHDESK_OVERRIDES_BLOB, freshdeskArticleIdsMatch, } from "../freshdesk/freshdesk-catalog-types.js";
...
test("getHelpResourceDetails selects existing-customer screenshots from question", async () => {
    ...
    if (result.ok) {
        ...
        assert.equal(/Email Preferences/i.test(result.payload.customerFacingScreenshotMarkdown ?? ""), false);
        ...
        assert.equal(result.payload.customerFacingInstructionMarkdown?.includes("blob.core.windows.net"), false);
    }
});
...
test("links mode returns signed screenshot links without downloading blobs", async () => {
    ...
    if (result.ok) {
        ...
        assert.equal(result.payload.imageCount, 1);
        assert.equal(result.payload.screenshotUrls?.length, 1);
        assert.match(result.payload.screenshotUrls?.[0]?.url ?? "", /^https:\/\/red\.example\.com\/public\/brc-edu\/freshdesk-images\//);
        assert.equal(result.images.length, 0);
        assert.ok(result.payload.customerFacingScreenshotMarkdown);
    }
});
...
function mockBlobContainer(blobs) {
    return {
...
test("visible article from effective catalogue returns instructionBlocks and syncedImages", async () => {
    ...
    const result = await getHelpResourceDetails("freshdesk:1001", {
        freshdeskIndexContainer: container,
        freshdeskImageContainer: null,
    });
    assert.equal(result.ok, true);
    if (result.ok) {
...
test("raw catalogue fallback returns rich fields when effective catalogue is missing", async () => {
    ...
    const found = await findFreshdeskArticleById("1001", container);
    assert.ok(found);
...
test("string and numeric Freshdesk article IDs match safely", async () => {
    ...
    assert.equal(await findFreshdeskArticleById("9999", container), null);
});
