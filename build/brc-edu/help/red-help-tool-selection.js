...
const TRANSACTIONAL_TOOL_PATTERN = /^brc_(?:create|update|delete|batch|send|post)_/i;
/**
 ...
 */
export function simulateRedHelpToolSelection(userMessage, tools, instructions) {
    const { isHelpMode } = detectHelpMode(userMessage);
    if (!isHelpMode) {
        return null;
    }
    const hasRoutingOverride = (/REQUEST ROUTING|RED-HELP SHORTCUT|RED-HELP ROUTING OVERRIDE/i.test(instructions) &&
    ...
    for (const tool of tools) {
        ...
        let score = 0;
        if (tool.name === "brc_red_help")
            score += 100;
        if (tool.name === "brc_find_help_resources")
            score += 40;
...
export function isTransactionalAccountingToolName(toolName) {
