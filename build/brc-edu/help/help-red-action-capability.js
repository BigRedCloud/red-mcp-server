...
export const RED_ACTION_MARKDOWN_COPY_INSTRUCTION = "When redActionAvailable is true, include the following Markdown after Sources and before any support section. Do not start the Red action unless the user asks. Do not claim any data has already been changed.";
/**
 ...
 */
const RED_ACTION_DEFINITIONS = [
    {
        redActionName: "create_customer",
        toolNames: ["brc_create_customer"],
        match: /\b(?:add|create|set\s+up|setup)\b.{0,40}\bcustomers?\b|\bcustomers?\b.{0,40}\b(?:add|create|set\s+up|setup)\b/i,
        markdown: [
            "Do this through Red",
            "",
    ...
    },
    {
        ...
        match: /\b(?:add|create|set\s+up|setup)\b.{0,40}\bsuppliers?\b|\bsuppliers?\b.{0,40}\b(?:add|create|set\s+up|setup)\b/i,
        markdown: [
            "Do this through Red",
            "",
...
export function resolveHelpRedActionCapability(question, options = {}) {
    ...
    for (const definition of RED_ACTION_DEFINITIONS) {
        ...
        const enabledTool = definition.toolNames.find((toolName) => checkEnabled(toolName));
        if (!enabledTool) {
            continue;
        }
        return {
...
export function buildRedActionMarkdownTextBlock(redActionMarkdown) {
