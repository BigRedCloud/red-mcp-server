...
import { CUSTOMER_FACING_SUPPORT_MARKDOWN, SUPPORT_CONTACT_URL, resolveSupportFallback, } from "./help-support-fallback.js";
import { resolveHelpRedActionCapability } from "./help-red-action-capability.js";
import { AUTO_SCREENSHOT_RETRIEVAL_GUIDANCE, HELP_ANSWER_LAYOUT_GUIDANCE, HELP_ANSWER_SECTION_ORDER, } from "./help-answer-layout.js";
import { FIND_HELP_RESOURCES_TOOL_DESCRIPTION, GET_HELP_RESOURCE_DETAILS_TOOL_DESCRIPTION, } from "../../tools/edu/help_resources_tools.js";
...
test("how-to question guidance retrieves details with includeImages=true", () => {
    ...
    assert.match(response.responseGuidance.format.join(" "), /includeImages=true/i);
    assert.match(response.responseGuidance.autoScreenshots ?? "", /did not explicitly ask for images/i);
});
test("screenshots guidance applies even when the user did not ask for images", () => {
    ...
    assert.match(getBrcMcpServerInstructions(50, false), /even when the user did not explicitly ask for images/i);
});
test("screenshots remain beside steps and are not moved into Sources", () => {
    ...
    assert.match(buildSourcesMarkdownTextBlock(sourcesMarkdown) ?? "", /Do not move screenshot links into Sources/i);
    assert.match(HELP_ANSWER_LAYOUT_GUIDANCE, /beside steps/i);
});
test("no screenshots are claimed when none exist", () => {
    assert.match(GET_HELP_RESOURCE_DETAILS_TOOL_DESCRIPTION, /Do not claim screenshots were supplied when imageCount is 0/i);
    assert.match(AUTO_SCREENSHOT_RETRIEVAL_GUIDANCE, /Do not claim screenshots were shown when imageCount is 0/i);
});
test("used Freshdesk article appears under Sources with exact publicUrl", () => {
    ...
    assert.equal(response.customerFacingSourcesMarkdown?.includes("Getting Started"), false);
});
...
test("Do this through Red appears for customer creation when tool is enabled", () => {
    const capability = resolveHelpRedActionCapability("How do I add a customer in Big Red Cloud?", { isToolEnabled: (name) => name === "brc_create_customer" });
    assert.equal(capability.redActionAvailable, true);
    assert.equal(capability.redActionName, "create_customer");
    assert.match(capability.customerFacingRedActionMarkdown ?? "", /Do this through Red/);
    assert.match(capability.customerFacingRedActionMarkdown ?? "", /I'll collect the required details and show you a preview before anything is saved/i);
    assert.equal(/already (created|saved|posted|changed)/i.test(capability.customerFacingRedActionMarkdown ?? ""), false);
});
test("Do this through Red appears for supplier and invoice when tools are enabled", () => {
    ...
    const invoice = resolveHelpRedActionCapability("Show me how to create a sales invoice", {
        isToolEnabled: (name) => name === "brc_create_sales_invoice" ||
...
test("Do this through Red is omitted when no matching operational tool exists", () => {
    const bankRec = resolveHelpRedActionCapability("How do I reconcile my bank?", { isToolEnabled: () => true });
    assert.equal(bankRec.redActionAvailable, false);
    assert.equal(bankRec.customerFacingRedActionMarkdown, undefined);
    const disabledCustomer = resolveHelpRedActionCapability("How do I add a customer?", { isToolEnabled: () => false });
...
test("support appears after Sources and Red-action with exact contact URL", () => {
    ...
    const response = buildUnifiedFindHelpResourcesResponse("How do I add a customer in Big Red Cloud?", { freshdeskArticles: [freshdeskArticle()] }, undefined);
    ...
    const withRedAction = {
        ...
        supportFallbackReason: "incomplete_answer",
    ...
    const sourcesIndex = texts.findIndex((text) => text.includes("Include the following exact Sources Markdown"));
    const redIndex = texts.findIndex((text) => text.includes("When redActionAvailable is true"));
    const supportIndex = texts.findIndex((text) => text.includes("Always include the following support Markdown last"));
    ...
    assert.match(response.customerFacingAnswerSectionsMarkdown ?? "", /Still need help\?/);
    assert.ok((response.customerFacingAnswerSectionsMarkdown ?? "").endsWith("[Contact Big Red Cloud Support](https://bigredcloud.com/contact/)"));
});
...
test("support section is always included on every help answer", () => {
    ...
    assert.equal(fallback.customerFacingSupportMarkdown, CUSTOMER_FACING_SUPPORT_MARKDOWN);
    const response = buildUnifiedFindHelpResourcesResponse("add a customer", {
        freshdeskArticles: [freshdeskArticle()],
    });
    ...
    assert.match(response.customerFacingAnswerSectionsMarkdown ?? "", /Still need help\?[\s\S]*Contact Big Red Cloud Support/);
...
test("interaction-mode clarification feature is not restored", () => {
    ...
    assert.equal(/Would you like instructions for doing this in Big Red Cloud, or would you like to do it here through Red/i.test(instructions), false);
    assert.equal(/ask once.*tutorial versus Red/i.test(FIND_HELP_RESOURCES_TOOL_DESCRIPTION + GET_HELP_RESOURCE_DETAILS_TOOL_DESCRIPTION), false);
});
