import { brcFetch, extractListItems, normaliseCompanyName, round2, toNumber, } from "./shared.js";
const vatOnCashCache = new Map();
...
const EXPLICIT_KEY_PATTERN = /^(vocrSettingValue|vatOnCashReceipts?|useVatOnCashReceipts?|enableVatOnCashReceipts?|vatOnCash)$/i;
...
function findExplicitVatOnCashSetting(value) {
    ...
    if (typeof value === "string") {
        for (const pattern of XML_TAG_PATTERNS) {
            ...
            if (match)
                return parseBooleanSetting(match[1]);
        }
        const jsonMatch = value.match(/"vatOnCash(?:Receipts?)?"\s*:\s*(true|false)/i);
        if (jsonMatch)
            return parseBooleanSetting(jsonMatch[1]);
    }
...
function findVocrSettingValue(...sources) {
    for (const source of sources) {
        const options = optionsRoot(source);
        if (!options || !(VOCR_OPTION_KEY in options))
            continue;
...
function findVocrFromCompanySettingsXml(companySettings, regionDescription) {
...
async function loadVatOnCashReceiptEnabled(companyName) {
    ...
    const regionDescription = typeof setup?.generalDetails === "object" && setup.generalDetails !== null
        ? String(setup.generalDetails.regionDescription || "")
        : undefined;
    const vocrFromOptions = findVocrSettingValue(companyOptions, setup, setup?.options);
    if (vocrFromOptions !== undefined)
        return vocrFromOptions;
    const vocrFromXml = findVocrFromCompanySettingsXml(companySettings, regionDescription);
    if (vocrFromXml !== undefined)
        return vocrFromXml;
...
export function clearVatOnCashReceiptCache(companyName) {
    if (companyName) {
...
export async function isVatOnCashReceiptEnabled(companyName) {
...
export function sanitizeCashReceiptInput(args, vatOnCashEnabled) {
    if (vatOnCashEnabled)
        return args;
    const next = { ...args };
