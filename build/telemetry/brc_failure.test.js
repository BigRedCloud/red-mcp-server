...
import { __getLastBrcFailureTelemetryForTests, __resetBrcFailureTelemetryForTests, buildBrcFailureTelemetryDimensions, recordBrcDownstreamFailureTelemetry, } from "./brc_failure.js";
import { buildTelemetryCustomDimensions, runWithRedTelemetryContext, } from "./identity.js";
...
test("downstream BRC failure telemetry uses the same Azure correlation IDs as customDimensions", () => {
    ...
    runWithRedTelemetryContext({
        ...
        recordBrcDownstreamFailureTelemetry({
            ...
            recordId: 999999999,
            toolName: "brc_update_quote",
            errorSummary: "Write did not complete (500 Internal Server Error). Unknown error occurred.",
        });
        const recorded = __getLastBrcFailureTelemetryForTests();
        assert.ok(recorded);
        assert.equal(recorded.dimensions["red.connection_session_id"], azure["red.connection_session_id"]);
        assert.equal(recorded.dimensions["red.telemetry_client_id"], azure["red.telemetry_client_id"]);
        assert.equal(recorded.dimensions["red.connection_session_id"], STAGING_CONNECTION_SESSION_ID);
        assert.equal(recorded.dimensions["red.telemetry_client_id"], STAGING_TELEMETRY_CLIENT_ID);
...
test("confirmed Quote preflight GET failure records searchable BRC telemetry", async () => {
    ...
    await runWithRedTelemetryContext({
        ...
        await assert.rejects(() => Promise.resolve(wrapped({
...
test("failure telemetry omits sensitive values and request payloads", () => {
    ...
    const dimensions = buildBrcFailureTelemetryDimensions({
        ...
        stage: "write",
        toolName: "brc_update_quote",
        errorSummary: "Authorization: Bearer secret-token apiKey=super-secret-company-key",
    });
