...
import { buildConnectTelemetryFlowDiagnostics, hashConnectionCodeForDiagnostics, resolveAndPersistConnectTelemetryClientId, resolveCanonicalTelemetryClientId, resolveTelemetryClientIdFromRequest, } from "./context.js";
import { buildTelemetryClientIdPageScript, generateTelemetryUuid, isValidTelemetryUuid, TELEMETRY_CLIENT_ID_COOKIE, TELEMETRY_CLIENT_ID_FORM_FIELD, } from "./identity.js";
...
test("GET and POST retain one client ID for the same connection", async () => {
    const { ensureConnectionStoreInitialized } = await import("../auth/connection_store.js");
    await ensureConnectionStoreInitialized();
...
test("fallback UUID is generated only once per connection", async () => {
    const { ensureConnectionStoreInitialized, getConnectionStore } = await import("../auth/connection_store.js");
    await ensureConnectionStoreInitialized();
