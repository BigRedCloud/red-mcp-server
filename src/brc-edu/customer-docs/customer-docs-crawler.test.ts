...
import {
  extractCustomerDocsLinks,
  isCustomerDocsArticleUrl,
  parseCustomerDocsArticlePage,
...
test("extractCustomerDocsLinks discovers docs category and article links", () => {
  const links = extractCustomerDocsLinks(
    DOCS_HOME_HTML,
    "https://bigredcloud.com/docs/",
  );
...
test("isCustomerDocsArticleUrl identifies article pages", () => {
  assert.equal(
    isCustomerDocsArticleUrl("https://bigredcloud.com/docs/sales/invoices/"),
    true,
  );
  assert.equal(
    isCustomerDocsArticleUrl("https://bigredcloud.com/docs/sales/"),
    false,
  );
});

test("parseCustomerDocsArticlePage preserves canonical URL and removes navigation text", () => {
  const article = parseCustomerDocsArticlePage(
    ARTICLE_HTML,
    "https://bigredcloud.com/docs/cash-book/bank-reconciliation/",
