import type {
  YouTubeCatalogVideo,
  YouTubeEffectiveCatalog,
  YouTubeRawCatalog,
  YouTubeRawCatalogVideo,
  YouTubeVideoOverride,
  YouTubeVideoOverridesMap,
...
export function mergeYouTubeCatalogWithOverrides(params: {
  rawVideos: YouTubeRawCatalogVideo[];
  ...
  const items: YouTubeCatalogVideo[] = params.rawVideos.map((video) => {
...
export function applyOverrideToRawVideo(
  video: YouTubeRawCatalogVideo,
  override: YouTubeVideoOverride | undefined,
): YouTubeCatalogVideo {
...
export function upsertYouTubeVideoOverride(
  current: YouTubeVideoOverridesMap,
  videoId: string,
  next: {
    excluded: boolean;
    reason?: string;
  ...
  const copy: YouTubeVideoOverridesMap = { ...current };
...
export function summarizeEffectiveCatalog(catalog: YouTubeEffectiveCatalog): {
  total: number;
  visible: number;
  excluded: number;
