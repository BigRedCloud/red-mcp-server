...
const FOLDER_TOPIC_ALIASES: Array<{ pattern: RegExp; topic: string }> = [
...
const KNOWN_ROUTING = new Set<string>(HELP_ROUTING_CATEGORIES);

export type ResolveContentTopicInput = {
  title?: string;
  folderName?: string;
  helpRoutingCategory?: string;
};

export type ResolvedContentTopic = {
  topic: string;
  label: string;
};
...
function humanizeTopicKey(topic: string): string {
  return topic
    .split("_")
    .filter(Boolean)
...
function mapFolderToTopic(folderName: string): string | null {
 ...
 */
export function resolveContentTopic(
  input: ResolveContentTopicInput,
): ResolvedContentTopic {
  ...
  const inferred = inferHelpRoutingCategory(
    input.title ?? "",
    input.folderName,
  );
...
export function compareContentTopics(left: string, right: string): number {
  const rank = (topic: string): number => {
    const key = normalizeTopicKey(topic);
    if (key === "general_help") return 1;
...
export function compareOverviewItemsByDateThenTitle(
  left: { updatedAt?: string; title: string },
  right: { updatedAt?: string; title: string },
): number {
