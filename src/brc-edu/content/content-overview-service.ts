...
import {
  ...
  compareOverviewItemsByDateThenTitle,
  resolveContentTopic,
  topicLabel,
...
} from "../youtube/youtube-catalog-store.js";
...
export type ContentOverviewItemType =
  | "freshdesk_article"
  | "youtube_video"
  | "recorded_webinar";
...
export type ContentOverviewItem = {
  id: string;
  source: "freshdesk" | "youtube";
  type: ContentOverviewItemType;
  title: string;
  description?: string;
  url?: string;
  topic: string;
...
export type ContentOverviewTopic = {
  topic: string;
  label: string;
  counts: {
    total: number;
    freshdeskArticles: number;
    youtubeVideos: number;
    recordedWebinars: number;
  };
  items: ContentOverviewItem[];
};

export type ContentOverviewPayload = {
  counts: {
    totalVisible: number;
    freshdeskArticles: number;
    youtubeVideos: number;
  ...
  lastContentRefreshAt?: string;
  topics: ContentOverviewTopic[];
};

export type ContentOverviewDeps = {
  youtubeContainer?: ContainerClient | null;
  freshdeskContainer?: ContainerClient | null;
  adminBasePath?: string;
  loadYouTube?: (
    container: ContainerClient | null,
  ) => Promise<{ items: YouTubeCatalogVideo[]; excluded: number }>;
  loadFreshdesk?: (
    container: ContainerClient | null,
  ) => Promise<{ items: FreshdeskCatalogArticle[]; excluded: number }>;
  loadYouTubeStatus?: (
    container: ContainerClient | null,
  ) => Promise<{ lastSuccessAt: string | null } | null>;
  loadFreshdeskStatus?: (
    container: ContainerClient | null,
  ) => Promise<{ lastSuccessAt: string | null } | null>;
};
...
function youtubeManageUrl(basePath: string): string {
  return `${basePath}?view=youtube`;
}

function freshdeskManageUrl(basePath: string): string {
  return `${basePath}?view=freshdesk`;
}

function youtubeTopic(video: YouTubeCatalogVideo): string {
...
function toYouTubeOverviewItem(
  ...
  return {
    ...
    source: "youtube",
    type:
      video.category === "recorded_webinar"
        ? "recorded_webinar"
        : "youtube_video",
    title: video.title,
...
function toFreshdeskOverviewItem(
...
async function loadVisibleYouTubeItems(
  container: ContainerClient | null,
): Promise<{ items: YouTubeCatalogVideo[]; excluded: number }> {
  if (!container) {
...
async function loadVisibleFreshdeskItems(
  container: ContainerClient | null,
): Promise<{ items: FreshdeskCatalogArticle[]; excluded: number }> {
  if (!container) {
    return { items: [], excluded: 0 };
...
export async function buildContentOverview(
  deps: ContentOverviewDeps = {},
): Promise<ContentOverviewPayload> {
  const adminBasePath = deps.adminBasePath ?? "/internal/brc-edu/admin";
  const youtubeContainer =
    deps.youtubeContainer === undefined
      ? createConfiguredYouTubeBlobContainer()
      : deps.youtubeContainer;
  const freshdeskContainer =
    deps.freshdeskContainer === undefined
      ? createConfiguredFreshdeskCatalogContainer()
      : deps.freshdeskContainer;

  const [youtube, freshdesk, youtubeStatus, freshdeskStatus] =
    await Promise.all([
  ...
  const overviewItems: ContentOverviewItem[] = [
    ...freshdesk.items.map((article) =>
      toFreshdeskOverviewItem(article, adminBasePath),
    ),
    ...youtube.items.map((video) =>
      toYouTubeOverviewItem(video, adminBasePath),
    ),
  ];

  const topicMap = new Map<string, ContentOverviewTopic>();
  ...
  for (const item of overviewItems) {
    ...
    const topic =
      ...
      ({
        ...
        items: [],
      } satisfies ContentOverviewTopic);
  ...
  const youtubeVideos = youtube.items.filter(
    (item) => item.category === "youtube_video",
  ).length;
  const recordedWebinars = youtube.items.filter(
    (item) => item.category === "recorded_webinar",
  ).length;
  ...
  return {
    ...
    lastContentRefreshAt: maxIsoDate(
      youtubeStatus?.lastSuccessAt,
      freshdeskStatus?.lastSuccessAt,
    ),
    topics,
  };
}
