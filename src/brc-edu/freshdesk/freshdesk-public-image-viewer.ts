...
export type FreshdeskPublicImageDisplaySize = {
  width: number;
  scale: number;
};
...
export function prefersFreshdeskPublicImageViewer(req: Request): boolean {
...
export function computeFreshdeskPublicImageDisplaySize(options: {
...
}): FreshdeskPublicImageDisplaySize {
  const {
    naturalWidth,
    naturalHeight,
    viewportWidth,
    viewportHeight,
    maxScale = FRESHDESK_PUBLIC_IMAGE_VIEWER_MAX_SCALE,
    maxWidthRatio = FRESHDESK_PUBLIC_IMAGE_VIEWER_MAX_WIDTH_RATIO,
    maxHeightRatio = FRESHDESK_PUBLIC_IMAGE_VIEWER_MAX_HEIGHT_RATIO,
  } = options;
  ...
  const scale = Math.min(
    maxScale,
    (viewportWidth * maxWidthRatio) / naturalWidth,
    (viewportHeight * maxHeightRatio) / naturalHeight,
  );
...
export function buildFreshdeskPublicImageViewerResizeScript(): string {
  return `(function () {
