...
import {
  freshdeskArticleHasSyncedImages,
...
export type FreshdeskImageSkipReason =
  | "missing_blob"
  | "unsupported_mime"
  | "oversized"
  | "total_limit"
...
export type FreshdeskImageLoadResult = {
  blocks: HelpResourceImageBlock[];
  imageAvailable: boolean;
  imageCount: number;
  requestedImageCount: number;
  skippedImageCount: number;
  skippedByReason: Partial<Record<FreshdeskImageSkipReason, number>>;
  storageWarning?: string;
};

export type FreshdeskImageLoadDiagnostics = {
  freshdeskArticleId: number;
  storedImageReferences: number;
  successfulDownloads: number;
  skippedDownloads: number;
  configuredContainerPresent: boolean;
};
...
function incrementSkip(
  skippedByReason: Partial<Record<FreshdeskImageSkipReason, number>>,
  reason: FreshdeskImageSkipReason,
) {
...
export function buildFreshdeskImageLoadDiagnostics(
  article: SyncedFreshdeskArticle,
  result: FreshdeskImageLoadResult,
  configuredContainerPresent: boolean,
...
export function logFreshdeskImageLoadDiagnostics(
  diagnostics: FreshdeskImageLoadDiagnostics,
): void {
  console.info(
    "Freshdesk help image diagnostics:",
...
export async function loadFreshdeskImageBlocks(
  ...
  const emptyResult = (
    overrides: Partial<FreshdeskImageLoadResult> = {},
  ): FreshdeskImageLoadResult => ({
    blocks: [],
    imageAvailable,
    imageCount: 0,
  ...
  if (!container) {
    return emptyResult({
      storageWarning:
        "Freshdesk screenshots are temporarily unavailable. The article text is still available.",
    });
  }
  ...
  const maxImages = Math.min(
    options.maxImages ?? FRESHDESK_IMAGE_LOAD_MAX_IMAGES_DEFAULT,
    FRESHDESK_IMAGE_LOAD_MAX_IMAGES_HARD,
  );
  const maxImageBytes =
    options.maxImageBytes ?? FRESHDESK_IMAGE_LOAD_MAX_IMAGE_BYTES;
  const maxTotalBytes =
    options.maxTotalBytes ?? FRESHDESK_IMAGE_LOAD_MAX_TOTAL_BYTES;
