...
import {
  ...
  resolveFreshdeskContentBlocksForMatching,
  SCREENSHOT_MATCH_MIN_SCORE,
...
test("caption combines heading with fill instruction for A/C Code", () => {
  ...
  assert.equal(
    caption,
    "Adding a customer: Enter the required A/C Code",
  );
});
...
test("existing-customer opening balance excludes Add screenshot", () => {
  ...
  const urls = screenshots.map((block) =>
    block.type === "screenshot" ? block.url : "",
  );
...
test("opening balance screenshots match steps by meaning not raw image order", () => {
  ...
  const paired: Array<{ text: string; url?: string; caption?: string }> = [];
...
test("weak screenshot matches are omitted", () => {
  const blocks = buildFreshdeskInstructionBlocks(
    ...
    mockScreenshotUrls(1),
    { question: "How do printers work?" },
  );

  assert.equal(
    blocks.some((block) => block.type === "screenshot"),
    false,
  );
...
test("exact signed URL is preserved unchanged", () => {
  const exactUrl =
    "https://red.example.com/public/brc-edu/freshdesk-images/1001/exact.token.value";
  const parsed = parseFreshdeskArticleContent(OPENING_BALANCE_HTML);
  const blocks = buildFreshdeskInstructionBlocks(
    parsed.contentBlocks,
    [
  ...
  const changeScreenshot = blocks.find(
    (block) =>
      block.type === "screenshot" && /Click Change/i.test(block.caption),
  );
...
test("bank reconciliation workflow uses generic matching without hard-coding", () => {
  ...
  const blocks = buildFreshdeskInstructionBlocks(
    parsed.contentBlocks,
    screenshotUrls,
    { question: "How do I complete a bank reconciliation?" },
  );
...
test("customer email preferences workflow uses generic matching", () => {
  ...
  const blocks = buildFreshdeskInstructionBlocks(
    parsed.contentBlocks,
    screenshotUrls,
    { question: "How do I open customer email preferences?" },
  );
...
test("new-customer question prefers Add workflow screenshots", () => {
  ...
  const blocks = buildFreshdeskInstructionBlocks(
    parsed.contentBlocks,
    screenshotUrls,
    { question: "How do I add a brand-new customer?" },
  );
  ...
  assert.ok(
    blocks.some(
      (block) =>
        block.type === "screenshot" && /Click Add/i.test(block.caption),
    ),
  );
});
...
test("extractNearbyActions finds O/Balance and ageing buckets", () => {
  const actions = extractNearbyActions(
    "Click O/Balance then enter Current, 1 Month, 2 Months and 3 Months Plus",
  );
...
test("legacy articles without contentBlocks still enrich captions safely", () => {
  ...
  assert.equal(
    enriched[0]?.url,
    "https://red.example.com/public/brc-edu/freshdesk-images/9/token",
  );
});
...
test("text-only contentBlocks synthesize image refs for semantic matching", () => {
  const textBlocks = [
    {
      ...
      workflow: "add_customer" as const,
  ...
  assert.ok(screenshots.length >= 2);
  assert.equal(
    new Set(screenshots.map((block) => (block.type === "screenshot" ? block.url : ""))).size,
    screenshots.length,
  );

  const lookupStep = blocks.findIndex(
    (block) =>
      block.type === "text" && block.text.includes("Click Lookup or Setup"),
  );
  const addStep = blocks.findIndex(
    (block) =>
      block.type === "text" && block.text.includes("Click Customers, then click Add"),
  );
  const saveStep = blocks.findIndex(
    (block) => block.type === "text" && block.text.includes("Click Save"),
  );
  ...
  if (
    ...
    assert.notEqual(
      lookupShot.url,
      screenshotUrls[0]!.url,
      "Lookup step must not take the Save screenshot merely because it is first",
    );
