...
test("generic captions are rejected from Markdown links", () => {
  ...
  assert.equal(
    isRejectedFreshdeskCaption("AI-generated content may be incorrect"),
    true,
  );
  assert.equal(
    toScreenshotMarkdownLink(
...
test("Markdown text block includes copy instruction", () => {
  const text = buildScreenshotMarkdownTextBlock({
    screenshotMarkdown:
      "[Changing a customer: Click Change](https://red.example.com/public/brc-edu/freshdesk-images/1/t)",
  });
