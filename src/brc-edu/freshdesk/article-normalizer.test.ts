...
function createArticle(
  ...
  return {
    ...
    description:
      '<p>See <img src="https://cdn.freshdesk.com/guide.png" alt="Guide screenshot" /></p>',
...
test("normalizeFreshdeskArticle includes extracted image references", () => {
  const normalized = normalizeFreshdeskArticle(
    createArticle({
      description:
        '<img src="https://cdn.freshdesk.com/a.png" alt="First" />' +
  ...
  assert.equal(
    normalized.images[0]?.sourceUrl,
    "https://cdn.freshdesk.com/a.png",
  );
...
test("normalizeFreshdeskArticle preserves ordered content blocks", () => {
  ...
  if (normalized.contentBlocks?.[1]?.type === "image") {
    assert.equal(
      normalized.contentBlocks[1].precedingText,
