...
import {
  extractSafeBlobNameFromLegacyAzureUrl,
...
test("extractSafeBlobNameFromLegacyAzureUrl keeps only safe freshdesk blob paths", () => {
  assert.equal(
    extractSafeBlobNameFromLegacyAzureUrl(
      "https://account.blob.core.windows.net/brc-edu-images/freshdesk/1001/abc.png",
      { allowedContainerName: "brc-edu-images" },
    ),
    "freshdesk/1001/abc.png",
  );
  assert.equal(
    extractSafeBlobNameFromLegacyAzureUrl(
      "https://account.blob.core.windows.net/other/freshdesk/1001/abc.png",
      { allowedContainerName: "brc-edu-images" },
    ),
    null,
  );
...
test("Freshdesk article with one stored PNG returns one MCP image block", async () => {
  ...
  const result = await loadFreshdeskImageBlocks(createArticle(), container as never, {
...
test("multiple images return in article order", async () => {
  ...
  const result = await loadFreshdeskImageBlocks(
    createArticle({
      syncedImages: [
        {
...
test("PNG, JPEG, WebP, and GIF MIME types are preserved correctly", async () => {
  ...
  const result = await loadFreshdeskImageBlocks(
    ...
    container as never,
    { maxImages: 4, maxImageBytes: 10, maxTotalBytes: 40 },
  );
...
test("maxImages is enforced", async () => {
  ...
  const result = await loadFreshdeskImageBlocks(
    ...
    container as never,
    { maxImages: 2, maxImageBytes: 10, maxTotalBytes: 30 },
  );
...
test("total byte limit is enforced", async () => {
  ...
  const result = await loadFreshdeskImageBlocks(
    ...
    container as never,
    { maxImages: 2, maxImageBytes: 10, maxTotalBytes: 15 },
  );
...
test("one Azure failure does not prevent later valid images where safe", async () => {
  ...
  const result = await loadFreshdeskImageBlocks(
    ...
    container as never,
    { maxImageBytes: 10, maxTotalBytes: 10 },
  );
