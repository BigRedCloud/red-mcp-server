...
import {
  ...
  mergeHelpSearchResults,
  parseEnrichedEduCsv,
} from "../../edu/brc_edu_resources.js";

import type { FreshdeskArticlesIndex } from "./freshdesk-index-store.js";

import {
  createFreshdeskBodyExcerpt,
  findFreshdeskHelpArticles,
...
function createFreshdeskArticle(
  ...
  return {
    ...
    bodyText:
      "Follow these steps to create a sales invoice in Big Red Cloud accounting software.",
    images: [
      {
...
function freshdeskArticles(
  overrides: Partial<SyncedFreshdeskArticle>[] = [createFreshdeskArticle()],
): SyncedFreshdeskArticle[] {
...
test("scoreFreshdeskHelpArticle ranks exact title above body matches", () => {
  ...
  const exactScore = scoreFreshdeskHelpArticle(
    exactArticle,
    "bank feeds",
    tokens,
  );
  const bodyScore = scoreFreshdeskHelpArticle(
    bodyArticle,
    "bank feeds",
    tokens,
  );
...
test("mergeHelpSearchResults combines webinar and Freshdesk results by relevance", () => {
  const results = mergeHelpSearchResults(
    "bank feeds",
    webinarResources(),
    freshdeskArticles([
      {
...
test("mergeHelpSearchResults removes duplicate titles", () => {
  ...
  assert.equal(
    titles.filter((title) => title === "sales invoices cash book").length,
    1,
  );
});
...
test("buildFindHelpResourcesResponse falls back to webinar results when Freshdesk index is unavailable", async () => {
  ...
  assert.equal(
    response.resources.every((resource) => resource.source === "Big Red Cloud"),
    true,
  );
...
test("loadFreshdeskArticlesForHelpSearch falls back safely when index load fails", async () => {
  ...
  const mockContainer = {} as ContainerClient;
  const loaded = await loadFreshdeskArticlesForHelpSearch({
    now: 1_700_000_000_000,
...
test("toFreshdeskHelpResourceResult does not return the full article body", () => {
  ...
  const result = toFreshdeskHelpResourceResult(
    createFreshdeskArticle({ bodyText: longBody }),
  );
...
test("createFreshdeskBodyExcerpt normalizes whitespace", () => {
  assert.equal(
    createFreshdeskBodyExcerpt("  Line one.   Line   two.  "),
    "Line one. Line two.",
  );
});
...
test("loadFreshdeskArticlesForHelpSearch loads articles from a valid index", async () => {
  ...
  const index: FreshdeskArticlesIndex = {
...
test("buildFindHelpResourcesResponse with Freshdesk results keeps support fallback when nothing matches", () => {
  const response = buildFindHelpResourcesResponse(
