...
test("FreshdeskClient rejects missing base URL", () => {
  assert.throws(
    () => new FreshdeskClient("   ", API_KEY),
    /Freshdesk base URL is required/,
  );
});

test("FreshdeskClient rejects missing API key", () => {
  assert.throws(
    () => new FreshdeskClient(BASE_URL, "  "),
...
test("FreshdeskClient builds the correct folders URL", async () => {
  await withMockFetch(
    ...
    async (calls) => {
      ...
      assert.equal(
        calls[0]?.url,
        `${BASE_URL}/api/v2/solutions/categories/12/folders`,
      );
    },
  );
});
...
test("FreshdeskClient builds the correct articles URL", async () => {
  await withMockFetch(
    ...
    async (calls) => {
      ...
      assert.equal(
        calls[0]?.url,
        `${BASE_URL}/api/v2/solutions/folders/34/articles`,
      );
    },
  );
});
...
test("FreshdeskClient builds the correct single article URL", async () => {
  await withMockFetch(
    ...
    async (calls) => {
      ...
      assert.equal(
        calls[0]?.url,
        `${BASE_URL}/api/v2/solutions/articles/56`,
      );
    },
  );
});
  ...
  await withMockFetch(
    ...
    async () => {
      ...
      await assert.rejects(
        () => client.getArticle(99),
        /Freshdesk request failed with status 401/,
      );
    },
  );
});
...
test("FreshdeskClient never includes the API key or Authorization header in errors", async () => {
  const encodedCredentials = Buffer.from(`${API_KEY}:X`, "utf8").toString(
    "base64",
  );

  await withMockFetch(
    () =>
      new Response(`Authorization: Basic ${encodedCredentials}`, {
        status: 403,
      }),
    async () => {
      const client = new FreshdeskClient(BASE_URL, API_KEY);
