...
export type FreshdeskPublicImageCropResult = {
  buffer: Buffer;
  cropped: boolean;
};
...
function isContentPixel(rgb: Rgb, background: Rgb): boolean {
  const channelDistance = Math.max(
    Math.abs(rgb.r - background.r),
    Math.abs(rgb.g - background.g),
    Math.abs(rgb.b - background.b),
  );
...
function selectSignificantComponents(
  ...
  const minArea = Math.max(
    40,
    Math.floor(analysisWidth * analysisHeight * MIN_COMPONENT_AREA_RATIO),
  );
  ...
  const smallAreaCutoff = Math.max(
    minArea,
    Math.floor(largest.area * SMALL_COMPONENT_AREA_RATIO),
  );
  ...
  return significant.filter((component) => {
    ...
    const distance = Math.hypot(
      component.centroidX - largest.centroidX,
      component.centroidY - largest.centroidY,
    );
...
function unionComponentBounds(components: ContentComponent[]): CropBounds | null {
  if (components.length === 0) {
    return null;
  }
...
function shouldSkipCrop(
  ...
  return (
    widthReduction < MIN_AXIS_REDUCTION_RATIO &&
...
export function detectFreshdeskPublicImageCropBounds(
  analysisWidth: number,
  analysisHeight: number,
  mask: boolean[],
  imageWidth: number,
  ...
  const significant = selectSignificantComponents(
    components,
    analysisWidth,
    analysisHeight,
  );
...
export async function analyzeFreshdeskPublicImageCropBounds(
  buffer: Buffer,
  ...
  const analysisScale = Math.min(
    1,
    ANALYSIS_MAX_DIMENSION / Math.max(imageWidth, imageHeight),
  );
  ...
  return detectFreshdeskPublicImageCropBounds(
    info.width,
    info.height,
    mask,
    imageWidth,
...
export async function normalizeFreshdeskPublicImageBuffer(
  buffer: Buffer,
  mimeType: string,
): Promise<FreshdeskPublicImageCropResult> {
