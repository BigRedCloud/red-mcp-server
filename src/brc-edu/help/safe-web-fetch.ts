...
export type AllowedWebPathRule = {
  hostname: string;
  pathPrefix: string;
};
...
export function validateApprovedWebUrl(
  rawUrl: string,
  rules: AllowedWebPathRule[],
): URL {
  let parsed: URL;
  ...
  const allowed = rules.some(
    (rule) =>
      hostname === rule.hostname.toLowerCase() &&
...
export function normalizeApprovedWebUrl(
  rawUrl: string,
  rules: AllowedWebPathRule[],
  baseUrl?: string,
): string {
...
export async function safeWebFetchText(
  rawUrl: string,
  rules: AllowedWebPathRule[],
...
export const BIGREDCLOUD_DOCS_RULE: AllowedWebPathRule = {
  hostname: "bigredcloud.com",
  pathPrefix: "/docs",
};

export const BIGREDCLOUD_WEBINAR_SERIES_RULE: AllowedWebPathRule = {
  hostname: "bigredcloud.com",
