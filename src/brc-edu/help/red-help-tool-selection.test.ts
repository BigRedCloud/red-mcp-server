...
import {
  ensureRedHelpQueryForUnifiedSearch,
  RED_HELP_TOOL_DESCRIPTION,
  RED_HELP_TOOL_TITLE,
} from "../../tools/edu/help_resources_tools.js";
import {
  isTransactionalAccountingToolName,
  simulateRedHelpToolSelection,
} from "./red-help-tool-selection.js";
...
test("brc_red_help description includes sales invoice, customer, and manual-help terms", () => {
  ...
  assert.match(
    tool.description,
    /never replace a red-help request with create, update, delete or post tools/i,
  );
  assert.match(
    tool.description,
    /brc_red_help\(\{ query: "how do I add a sales invoice" \}\)/,
  );
});
...
test("brc_red_help delegates to unified help pipeline with blockTransactionalTools", () => {
  const question = ensureRedHelpQueryForUnifiedSearch(
    "how do I add a sales invoice",
  );
...
test("tool-selection harness: red-help how do I add a sales invoice → brc_red_help", () => {
  const selected = simulateRedHelpToolSelection(
  ...
  const createInvoice = discoverableTools.find((tool) =>
    /create.*sales.*invoice|sales_invoice/i.test(tool.name),
  );
  assert.ok(
    createInvoice,
    "expected a transactional sales invoice tool to exist for contrast",
  );
