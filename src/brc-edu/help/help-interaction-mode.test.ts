...
import {
  HELP_INTERACTION_CLARIFICATION_QUESTION,
  TUTORIAL_MODE_NO_DATA_CHANGE_GUIDANCE,
  UNSUPPORTED_RED_ACTION_GUIDANCE,
  createInitialHelpInteractionModeState,
  detectHelpModeSwitch,
  isAmbiguousHelpActionQuestion,
  isClearRedActionRequest,
  isClearTutorialRequest,
  redActionLikelySupported,
  resolveHelpInteractionMode,
  unsupportedRedActionCustomerMessage,
} from "./help-interaction-mode.js";

test("first ambiguous How do I add a customer asks tutorial versus Red action", () => {
  const resolution = resolveHelpInteractionMode(
    createInitialHelpInteractionModeState(),
...
test("clarification is asked only once in the conversation", () => {
  const first = resolveHelpInteractionMode(
    createInitialHelpInteractionModeState(),
    "How do I add a customer?",
  );
  ...
  const later = resolveHelpInteractionMode(
    afterChoice.state,
...
test("Show me how in Big Red Cloud changes back to tutorial mode", () => {
  let state = createInitialHelpInteractionModeState();
  state = resolveHelpInteractionMode(
    state,
    "Add this customer through Red.",
  ).state;
  assert.equal(state.helpInteractionMode, "red_action");

  const switched = resolveHelpInteractionMode(
    state,
    "Show me how in Big Red Cloud",
  );
...
test("clear tutorial requests do not trigger clarification", () => {
  assert.equal(
    isClearTutorialRequest(
      "Show me how to add a customer in Big Red Cloud.",
    ),
    true,
  );
  assert.equal(
  ...
  const resolution = resolveHelpInteractionMode(
    createInitialHelpInteractionModeState(),
    "Give me a tutorial with screenshots.",
  );
...
test("clear Red-action requests do not trigger clarification", () => {
  assert.equal(
    isClearRedActionRequest("Create the invoice for me through Red."),
    true,
  );
  assert.equal(
    isAmbiguousHelpActionQuestion("Create the invoice for me through Red."),
    false,
  );

  const resolution = resolveHelpInteractionMode(
    createInitialHelpInteractionModeState(),
    "Please post this invoice.",
  );
  assert.equal(resolution.shouldAskClarification, false);
