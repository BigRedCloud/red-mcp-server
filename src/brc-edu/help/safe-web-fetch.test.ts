...
test("validateApprovedWebUrl rejects non-docs paths", () => {
  assert.throws(
    () =>
      validateApprovedWebUrl("https://bigredcloud.com/webinar-series/", [
        BIGREDCLOUD_DOCS_RULE,
      ]),
...
test("validateApprovedWebUrl rejects javascript URLs", () => {
  assert.throws(
    () =>
      validateApprovedWebUrl("javascript:alert(1)", [BIGREDCLOUD_DOCS_RULE]),
    /Only HTTPS URLs are allowed|malformed/i,
  );
});
...
test("validateApprovedWebUrl rejects localhost", () => {
  assert.throws(
    () =>
      validateApprovedWebUrl("https://localhost/docs/test", [
        BIGREDCLOUD_DOCS_RULE,
      ]),
    /not allowed/i,
  );
...
test("normalizeApprovedWebUrl resolves relative docs links", () => {
  const resolved = normalizeApprovedWebUrl(
    "/docs/sales/invoices/",
    [BIGREDCLOUD_DOCS_RULE],
