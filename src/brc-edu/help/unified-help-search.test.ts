...
test("empty upcoming webinar search does not claim none are scheduled", () => {
  ...
  assert.match(
    response.customerFacingEmptyUpcomingWebinarMarkdown ?? "",
    /currently available webinar listings/i,
  );
  assert.match(
    response.customerFacingEmptyUpcomingWebinarMarkdown ?? "",
    /#Upcoming-Webinar/,
  ...
  assert.equal(
    /no webinars are scheduled|nothing scheduled|nothing is coming up/i.test(
      response.customerFacingEmptyUpcomingWebinarMarkdown ?? "",
    ),
    false,
  );
  assert.match(
    response.responseGuidance.format.join(" "),
    /Do not claim that no webinars are scheduled/i,
  );
  assert.match(
  ...
  assert.equal(
    response.resources.some((resource) => resource.source === "recorded_webinar"),
    false,
  );
});
...
test("returned upcoming webinars use registration links and take priority", () => {
  ...
  assert.equal(
    response.resources.every((resource) => resource.source === "upcoming_webinar"),
    true,
  );
  assert.equal(response.customerFacingEmptyUpcomingWebinarMarkdown, undefined);
  assert.match(
    response.customerFacingSourcesMarkdown ?? "",
    /zoom\.us\/meeting\/register\/onboarding-3/,
  );
  assert.equal(
    /recorded_webinar/.test(response.usedResourceIds.join(",")),
    false,
  );
});
