...
import {
  ...
  BRC_EDU_ADMIN_DEFAULT_PROTECTED_PATH,
  BRC_EDU_ADMIN_STAFF_DENIED_MESSAGE,
  buildEasyAuthLoginRedirectUrl,
  buildOpenEduAdminToolPayload,
  EASY_AUTH_CLIENT_PRINCIPAL_HEADER,
  EASY_AUTH_CLIENT_PRINCIPAL_NAME_HEADER,
  evaluateEntraStaffAccess,
  getBrcEduAdminProtectedPath,
  getBrcEduAdminPublicUrl,
  parseEasyAuthClientPrincipal,
  type EasyAuthClientPrincipal,
} from "./brc_edu_admin_auth.js";
...
function encodePrincipal(principal: EasyAuthClientPrincipal): string {
...
function staffPrincipal(options: {
  ...
  const claims = [
    ...
    },
    {
      ...
      val: options.email ?? "staff_member@example.com",
    },
    {
      typ: "oid",
...
test("approved staff can access the admin page via Easy Auth principal", () => {
  ...
  const result = authorizeBrcEduAdminRequest({
    headers: {
      ...
      [EASY_AUTH_CLIENT_PRINCIPAL_NAME_HEADER]: "staff_member@example.com",
    },
  });
  ...
  if (result.ok) {
    ...
    assert.equal(result.identity, "staff_member@example.com");
  }
});
...
test("existing admin page still works after secret authentication", () => {
  ...
  assert.match(
    html,
    new RegExp(
      `${BRC_EDU_ADMIN_UPLOAD_SECRET_QUERY}=${encodeURIComponent(secret)}`,
    ),
  );
});
