...
import {
  ...
  enrichSupportCsvText,
  getBrcEduSyncedResourcesPath,
  handleBrcEduResourcesSyncRequest,
  loadSyncedEduResources,
  syncEduResourcesFromCsvText,
} from "./brc_edu_synced_store.js";
...
function withSyncEnv(
  values: Partial<Record<(typeof SYNC_ENV_KEYS)[number], string | undefined>>,
  run: () => Promise<void> | void,
): Promise<void> {
  const previous: Partial<Record<(typeof SYNC_ENV_KEYS)[number], string | undefined>> = {};
...
test("missing configured sync secret returns 503", async () => {
  ...
  try {
    await withSyncEnv(
      ...
      () => {
        const result = handleBrcEduResourcesSyncRequest(
          { csvText: SUPPORT_CSV },
          "any-secret",
          fixture.baseDir,
        );
  ...
  try {
    await withSyncEnv(
      ...
      () => {
        ...
        const empty = handleBrcEduResourcesSyncRequest(
          { csvText: "   " },
          "configured-secret",
          fixture.baseDir,
        );
        const wrongType = handleBrcEduResourcesSyncRequest(
          { csvText: 123 },
          "configured-secret",
          fixture.baseDir,
        );
    ...
    await withSyncEnv(
      ...
      () => {
        ...
        const stored = JSON.parse(readFileSync(fixture.syncedPath, "utf8")) as {
          storedAt: string;
          resources: Array<{ title: string; url: string; helpRoutingCategory: string }>;
        };
