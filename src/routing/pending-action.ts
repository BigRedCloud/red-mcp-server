...
import type { PendingActionRecord } from "../auth/connection_store_types.js";
import { resolveHttpClientKey } from "../shared.js";
import { ROUTE_TOKEN_TTL_MS } from "./route-token.js";

export type PendingActionStatus = "routed" | "previewed";

export type PendingActionRejectionReason =
  | "no_pending"
  | "expired"
  | "connection_mismatch"
  | "scope_mismatch"
...
export function hashPendingActionScopeKey(
...
export function buildTargetRecordKey(
...
export function logPendingActionLookup(args: {
  ...
  rejectionReason?: PendingActionRejectionReason | null;
}): void {
  console.info(
    JSON.stringify({
...
export function logPendingActionRejected(args: {
  reason: PendingActionRejectionReason;
  workflowId?: string | null;
}): void {
  console.info(
    JSON.stringify({
...
export async function getPendingAction(args: {
  ...
  await ensureConnectionStoreInitialized();
  const record = await getConnectionStore().getPendingAction(
    args.connectionId.trim(),
    args.scopeKeyHash.trim(),
  );
  if (!record) {
    return null;
  }
...
export async function markPendingActionPreviewed(args: {
  ...
  const connectionId =
    args.connectionId?.trim() || getCurrentConnectionId()?.trim();
  if (!connectionId) {
    return;
  }
  const scopeKeyHash =
    args.scopeKeyHash ?? resolvePendingActionScopeKey();
...
export async function clearPendingAction(args: {
  ...
  await ensureConnectionStoreInitialized();
  await getConnectionStore().clearPendingAction(
    args.connectionId.trim(),
    args.scopeKeyHash.trim(),
  );
}
