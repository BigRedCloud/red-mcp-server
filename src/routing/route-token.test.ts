...
import {
  ...
  getRouteTokenSigningSecretSource,
  hashConnectionIdForBinding,
  hashRouteMessage,
  hashSessionIdForDiagnostics,
  ...
  ROUTE_TOKEN_SIGNING_SECRET_ENV,
  ROUTE_TOKEN_TTL_MS,
  validateRouteToken,
  validateRouteTokenForTool,
...
test("route and transactional calls succeed in the same session", async () => {
  ...
  const routed = await runWithMcpSessionContext(
    { sessionId, connectionId: "" },
    () => routeRequest("add a customer", { sessionId }),
  );
...
test("route and transactional calls succeed after HTTP session rehydration", async () => {
  ...
  const validation = await runWithMcpSessionContext(
    { sessionId: sessionB, connectionId },
    () =>
      validateRouteTokenForTool(issued.routeToken, {
        toolName: "brc_create_customer",
        sessionId: sessionB,
      }),
  );
...
test("route token issued while connected contains stable connection binding", async () => {
  ...
  assert.equal(
    validation.payload.connectionBinding,
    hashConnectionIdForBinding(connectionId),
  );
  assert.equal(
    validation.payload.connectionBinding.includes(connectionId),
    false,
  );
});
...
test("different Claude sessions with same connection succeed via connection binding", async () => {
  ...
  const result = await runWithMcpSessionContext(
    { sessionId: sessionB, connectionId },
    () => wrapped({ routeToken: issued.routeToken }),
  );
  assert.equal(handlerCalled, true);
  assert.deepEqual(result, { ok: true });
});
...
test("token issued before connection remains session-bound", async () => {
  ...
  assert.equal(
    validateRouteToken(issued.routeToken, {
      toolName: "brc_create_customer",
      sessionId: sessionA,
    }).ok,
    true,
...
test("validation resolves connection before route-token session check", async () => {
  ...
  await runWithMcpSessionContext(
    { sessionId: uniqueId("session-tx"), connectionId },
    async () => {
...
test("message-hash mismatch is rejected when a message is supplied to the validator", () => {
  ...
  assert.equal(
    issued.payload.messageHash,
    hashRouteMessage("add a customer"),
  );
});
...
test("preview reuse is allowed; consumption only after confirmed write", async () => {
  ...
  assert.equal(
    previewOneBody.status ?? previewOneBody.error,
  ...
  assert.equal(
    previewTwoBody.status ?? previewTwoBody.error,
    "confirmation_required",
  );
