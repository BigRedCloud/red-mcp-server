...
import {
  enabledToolsForWorkflow,
  getWorkflowForTool,
  listActionWorkflows,
  listEnabledTransactionalTools,
  resolveActionWorkflow,
  sampleUtteranceForWorkflow,
  ...
  for (const workflow of listActionWorkflows()) {
    ...
    for (const tool of workflow.allowedTools) {
      assert.equal(
        seen.has(tool),
        false,
        `tool ${tool} appears in both ${seen.get(tool)} and ${workflow.workflowId}`,
      );
      seen.set(tool, workflow.workflowId);
      assert.equal(
        requiresRouteToken(tool),
        true,
        `${tool} must require routeToken`,
      );
      assert.ok(
        ["update", "delete", "batch", "email"].includes(getToolSkillGroup(tool)),
        `${tool} skill group`,
      );
    }
  }
...
test("every enabled transactional tool can obtain a valid routeToken", async () => {
  ...
  for (const workflow of listActionWorkflows()) {
    ...
    assert.equal(
      routed.mode,
      "action",
      `${workflow.workflowId} utterance "${utterance}" → ${routed.mode}`,
    );
    assert.ok(routed.routeToken, workflow.workflowId);
    assert.ok(
      (routed.preferredTools?.length ?? 0) > 0,
      `${workflow.workflowId} preferredTools empty`,
    );
