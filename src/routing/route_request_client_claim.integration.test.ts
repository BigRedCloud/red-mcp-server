...
test("route_request inherits client claim without connectionRef then create validates across sessions", async () => {
  ...
  const confirmResolution = resolveHttpClientKeyFromHeaders(
    confirmHeaders,
    "198.51.100.42"
  );
  const routeResolution = resolveHttpClientKeyFromHeaders(
    routeHeaders,
    "203.0.113.88"
  );
  ...
  const issuedLine = issuedLogs.find((line) =>
    line.includes('"event":"route_token_issued"')
  );
  ...
  assert.ok(
    issued.clientClaimInherited || issued.sessionBindingFound,
    "expected claim inheritance or resulting session binding"
  );
  ...
  await runHttpToolSessionFromExtra(
    createSession,
    new Map(),
    { sessionId: createSession, requestInfo: { headers: routeHeaders } },
    async () => {
  ...
  const strangerScope = await (
    await loadModules()
  ).prepareHttpToolSessionScope(
    strangerSession,
    new Map(),
    resolveHttpClientKeyFromHeaders(unrelatedHeaders, "198.51.100.99")
  );
...
test("route_request recovers via connectionRef when client claim is unavailable", async () => {
  ...
  const forgedScope = await (
    await loadModules()
  ).prepareHttpToolSessionScope(
    forgedRefSession,
    new Map(),
    undefined,
    "redconn_forged_not_real"
  );
