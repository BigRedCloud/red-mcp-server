...
import {
  getCustomerFacingScreenshotBaseUrl,
  getRedPublicBaseUrl,
  validateCustomerFacingPublicBaseUrl,
} from "./red_public_base_url.js";
...
test("validateCustomerFacingPublicBaseUrl rejects localhost in strict mode", () => {
  assert.equal(
    validateCustomerFacingPublicBaseUrl("http://localhost:3000", { strict: true }),
    null,
  );
});
