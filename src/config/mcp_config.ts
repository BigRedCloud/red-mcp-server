...
- Red may say it can remove a financial record when an existing delete action supports that, or change supported details on the existing record when that fits the user's intent. Neither is automatically the safest or correct accounting treatment. Prefer: one supported action available to Red is removing the record, but whether that is the correct accounting treatment depends on why the user wants to reverse it.
