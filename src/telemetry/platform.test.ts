...
test("Vibe detected as mistral", () => {
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "vibe" } }).platform,
    "mistral"
  );
  assert.equal(
    resolveClientPlatform({
...
test("case-insensitive clientInfo matching", () => {
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "ChatGPT" } }).platform,
    "chatgpt"
  );
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "CLAUDE" } }).platform,
    "claude"
  );
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "OpenAI" } }).platform,
    "chatgpt"
  );
});
...
test("ambiguous client remains unknown", () => {
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "mcp-client" } }).platform,
    "unknown"
  );
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "ai-assistant" } }).platform,
    "unknown"
  );
...
test("unsupported clients remain unknown", () => {
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "cursor" } }).platform,
    "unknown"
  );
  assert.equal(
  ...
  assert.equal(
    resolveClientPlatform({ clientInfo: { name: "vscode-mcp" } }).platform,
    "unknown"
  );
});
...
test("User-Agent fallback still works for known platforms", () => {
  ...
  assert.equal(
    detectClientPlatform({ "user-agent": "mistral-vibe/1.0" }),
    "mistral"
  );
});
