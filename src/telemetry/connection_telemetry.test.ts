...
test("same client reconnecting retains client ID but gets a new session ID", async () => {
  ...
  const first = await claimConnectionCodeForSession(
    confirmationCode1,
    uniqueId("session")
  );
  ...
  const second = await claimConnectionCodeForSession(
    confirmationCode2,
    uniqueId("session")
  );
...
test("connection page HTML includes anonymous client id script and form field", async () => {
  ...
  assert.match(
    html,
    new RegExp(
      `name="telemetryClientId" value="${clientId.toLowerCase()}"`
    )
  );
...
test("memory save then session-only update preserves both telemetry ids", async () => {
  const { getConnectionStore, ensureConnectionStoreInitialized } = await import(
...
test("cosmos telemetry merge document builder never drops client id on session patch", async () => {
  ...
  const { mergeConnectionTelemetryRecord } = await import(
    "../auth/connection_telemetry_merge.js"
  );
...
test(
  "cosmos save/load preserves both telemetry ids across session update",
  { skip: !cosmosConnectionString },
  async () => {
    const previousStore = process.env.RED_CONNECT_CONNECTION_STORE;
    process.env.RED_CONNECT_CONNECTION_STORE = "cosmos";
    try {
      const { CosmosConnectionStore } = await import(
        "../auth/cosmos_connection_store.js"
      );
      const database =
        process.env.RED_CONNECT_COSMOS_DATABASE?.trim() || "red-connect";
      const container =
        process.env.RED_CONNECT_COSMOS_CONTAINER?.trim() || "connections";
      const store = new CosmosConnectionStore(
