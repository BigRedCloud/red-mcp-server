...
function adminUrl(port: number, secret?: string, view?: string): string {
  const path = view ? `${ADMIN_PATH}?view=${encodeURIComponent(view)}` : ADMIN_PATH;
  return withSecret(path, port, secret);
}

function legacyUploadUrl(port: number, secret?: string): string {
...
test("Freshdesk and overview APIs require admin authentication", async (t) => {
  ...
  const visibility = await fetch(
    `http://127.0.0.1:${port}/internal/brc-edu/freshdesk/articles/1/visibility`,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
