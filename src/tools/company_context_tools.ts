...
export function registerCompanyContextTools(server: ServerType) {
  ...
  server.tool(
    ...
    "Stores a BRC API key for a named company context in MCP server memory for this session. API keys are not returned in responses. The assistant must never echo or confirm the key value in chat after storing it.",
    {
  ...
  server.tool(
    ...
    async ({ companyName }) => {
      ...
      return jsonResponse({
        ...
        importantNote:
          "This clears the API key from MCP server memory. If the key was typed into the AI chat, it may still exist in the host chat history.",
      });
    }
  );
  ...
  server.tool(
    ...
    async () => {
      ...
      return jsonResponse({
        ...
        importantNote:
          "This clears all API keys from MCP server memory. If keys were typed into the AI chat, they may still exist in the host chat history.",
      });
    }
  );
}
