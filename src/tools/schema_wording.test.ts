...
test("quote create tools require companyId with no test-company default wording", () => {
  for (const name of ["brc_create_quote", "brc_create_quote_gen_ref"]) {
    ...
    assert.equal(
      companyId.isOptional(),
      false,
      `${name} companyId must be required`
    );
    assert.match(companyId.description ?? "", /required/i);
    assert.match(companyId.description ?? "", /connected company/i);
    assert.equal(
      /previous test company|defaults to/i.test(companyId.description ?? ""),
      false,
      `${name} companyId must not use test-environment default wording`
    );
  }
});
...
test("brc_update_quote advertises reference-only updates and does not expose note", () => {
  ...
  assert.equal(
    /staging has proven/i.test(description),
    false,
    "description must not use staging-test wording"
  );
});
...
test("quote list and get wording does not treat field differences as failures", () => {
  ...
  for (const tool of [list, get]) {
    ...
    assert.equal(
      /list and get return the same fields/i.test(tool.description),
      false
    );
  }
});
...
test("Quote and cash tool descriptions have no leftover staging or diagnostic wording", () => {
  ...
  for (const name of names) {
    const { description } = getTool(name);
    assert.equal(
      /staging has proven|temporary diagnostic|staging test|requestedUpdateKeys|currentRecordLedger/i.test(
        description
      ),
      false,
      `${name} description must not contain leftover staging or diagnostic wording`
    );
  }
});
