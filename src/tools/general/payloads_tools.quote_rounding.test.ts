...
function assertNoFloatArtifacts(values: number[]): void {
...
test("Quote 7.50 @ 23% uses Quote-specific rounding accepted by staging BRC", () => {
  ...
  assertNoFloatArtifacts([
    payload.totalNet as number,
    payload.totalVat as number,
    payload.total as number,
    line.vatAmount as number,
...
test("batch Quote arithmetic matches single create for 7.50 @ 23%", () => {
  ...
  const item = batch[0]!.item as Record<string, unknown>;

  assert.equal(item.totalNet, single.totalNet);
  assert.equal(item.totalVat, single.totalVat);
  assert.equal(item.total, single.total);

  const singleLine = (single.productTrans as Record<string, unknown>[])[0]!;
  const batchLine = (item.productTrans as Record<string, unknown>[])[0]!;
