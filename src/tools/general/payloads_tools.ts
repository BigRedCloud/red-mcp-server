...
export const SALES_DOCUMENT_RAW_PAYLOAD_STRUCTURE_DESCRIPTION =
  "Raw BRC payloads for multi-line sales documents must use the correct BRC structure: one object per product line in `productTrans`, with nested `acEntries` on each line where analysis posting applies. Prefer structured MCP tools (for example brc_create_sales_invoice) where possible — they build the correct line shape and reconcile totals. For raw payload tools, supply all required header fields (customerId, entryDate, procDate, saleRepId, saleRepCode, totals, vatTypeId, etc.) and ensure line amounts, VAT, and header totals reconcile where caller-supplied. Preview-before-posting shows what Red will post and waits for confirmation — it does not confirm a raw payload is structurally complete; missing productTrans lines, acEntries, or mismatched totals may still fail at post time.";
