...
function parseIsoDateParts(value: unknown): DateParts | undefined {
...
function formatMonthYear(parts: DateParts): string {
  return `${MONTH_NAMES[parts.month - 1]} ${parts.year}`;
}

export interface SalesDatePeriodSummary {
  /** Neutral processing-date line, for example "Processing date: 30/06/2026". */
  processingDate?: string;
  /** Neutral accounting-period line, for example "Period entered: June 2026". */
  periodEntered?: string;
  /** True only when the stored processing date differs from the requested one. */
  processingDateMismatch: boolean;
  /** Populated only on a genuine processing-date mismatch. */
  warning?: string;
}
...
export function buildSalesDatePeriodSummary(args: {
  ...
  returnedProcDate?: unknown;
  returnedEntryDate?: unknown;
}): SalesDatePeriodSummary {
  ...
  const processingDateMismatch = Boolean(
    requested &&
      returnedProc &&
  ...
  const summary: SalesDatePeriodSummary = {
    processingDate,
    periodEntered,
  ...
  if (processingDateMismatch && requested && returnedProc) {
    summary.warning = `The processing date was requested as ${formatDayMonthYear(
      requested
    )} but Big Red Cloud recorded ${formatDayMonthYear(
      returnedProc
