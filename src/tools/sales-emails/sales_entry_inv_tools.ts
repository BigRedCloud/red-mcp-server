  ...
  server.tool(
    ...
    async (args) =>
      createSalesInvoiceWithGeneratingReference({
        ...
        confirmCrAnalysisCategory:
          args.confirmCrAnalysisCategory,
  ...
  server.tool(
    ...
    ].join(" "),
    {
      ...
      productTrans: z
        ...
        .optional()
        .describe(
          "Complete BRC productTrans collection. Use this when changing invoice lines, quantities, unit prices, VAT or line amounts."
        ),
    },
    ...
    async ({
      ...
      const endpoint =
        `/v1/salesInvoices/${encodeURIComponent(String(id))}`;
      ...
      if (
        ...
        throw new Error(
          `Could not read sales invoice ${id} before update.`
        );
      }
      ...
      if (totalNet !== undefined) payload.totalNet = totalNet;
      if (totalVAT !== undefined) payload.totalVAT = totalVAT;
...
if (monetaryEditRequested) {
  if (
    ...
    throw new Error(
      "Monetary invoice updates require productTrans, totalNet, totalVAT and total together so Red does not send an inconsistent invoice to BRC."
    );
  }
}
      // Basic reconciliation before sending monetary changes.
      if (productTrans !== undefined) {
        const lines = productTrans as JsonRecord[];
  
        const calculatedNet = lines.reduce(
          (sum, line) =>
            sum +
        ...
        const calculatedVat = lines.reduce(
          (sum, line) =>
            sum +
        ...
        if (
          ...
          throw new Error(
            `totalNet ${totalNet} does not match productTrans amountNet total ${calculatedNet}.`
          );
        }
        ...
        if (
          ...
          throw new Error(
            `total ${total} does not match productTrans amount total ${calculatedTotal}.`
          );
        }
      }
      ...
      try {
        ...
        return jsonResponse({
          ...
          companyName,
          historicalDateAttempt:
            entryDate !== undefined || procDate !== undefined
              ? {
                  entryDate: payload.entryDate,
                  procDate: payload.procDate,
                }
        ...
        return jsonResponse({
          message:
            "BRC rejected the sales invoice update. The requested update was attempted rather than automatically blocked because of its financial year.",
          companyName,
          id,
          endpoint: `PUT ${endpoint}`,
