...
import type { CompanyProcessingSettings } from "../../guards/company_processing_settings.js";
import type { CompanyReferenceSettings } from "../../guards/company_reference_settings.js";
import {
  countActiveSalesVatRates,
  countSalesAnalysisCategories,
  evaluateCompanyReadiness,
  isSalesAnalysisCategory,
  type CompanyReadinessEvaluationInput,
...
function activeConnection(): ConnectionSnapshot {
  return {
    contextPresent: true,
...
function processing(overrides: Partial<CompanyProcessingSettings> = {}): CompanyProcessingSettings {
  return {
    raw: {},
...
function references(
  overrides: Partial<CompanyReferenceSettings> = {}
): CompanyReferenceSettings {
  return {
    raw: {},
...
function baseInput(
  overrides: Partial<CompanyReadinessEvaluationInput> = {}
): CompanyReadinessEvaluationInput {
  return {
    companyName: "Demo Co",
    today: TODAY,
    nowMs: NOW_MS,
...
test("ready with warnings when products are missing", () => {
  const report = evaluateCompanyReadiness(
    baseInput({ products: { ok: true, count: 0 } })
  );
...
test("no sales representatives is a warning", () => {
  const report = evaluateCompanyReadiness(
    baseInput({ salesReps: { ok: true, count: 0 } })
  );
  assert.equal(report.overallStatus, "ready_with_warnings");
...
test("absence of suppliers is not a sales-invoice blocker", () => {
  const report = evaluateCompanyReadiness(
    baseInput({ suppliers: { ok: true, count: 0 } })
  );
