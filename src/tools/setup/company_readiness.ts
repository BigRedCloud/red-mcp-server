...
export type HealthCheckStatus = "pass" | "warning" | "fail" | "unknown";
...
export type HealthChecks = {
  ...
  financialYear: HealthCheckResult;
  customers: HealthCheckResult;
  products: HealthCheckResult;
  suppliers: HealthCheckResult;
  salesReps: HealthCheckResult;
  salesVatRates: HealthCheckResult;
  salesAnalysisCategories: HealthCheckResult;
  processingSettings: HealthCheckResult;
  referenceSettings: HealthCheckResult;
};
...
export type ConnectionSnapshot = {
  ...
  expiresAtMs?: number;
  /** True when context exists, credential material is present, and not expired. */
  connectionActive: boolean;
  credentialsValid: boolean;
};

export type ListSample = {
  ok: boolean;
  count: number;
  errorMessage?: string;
};

export type SalesVatSample = {
  ok: boolean;
  salesActiveCount: number;
  totalRatesSampled: number;
  hasSalesCategory: boolean;
  errorMessage?: string;
};

export type SalesAnalysisSample = {
  ok: boolean;
  salesAnalysisCount: number;
  totalCategoriesSampled: number;
  errorMessage?: string;
};

export type SettingsSample<T> = {
  ok: boolean;
  value?: T;
  errorMessage?: string;
};

export type CompanyReadinessEvaluationInput = {
  companyName: string;
  today: string;
  nowMs: number;
  connection: ConnectionSnapshot;
  financialYear: FinancialYearInfo;
  customers: ListSample;
  products: ListSample;
  suppliers: ListSample;
  salesReps: ListSample;
  salesVatRates: SalesVatSample;
  salesAnalysisCategories: SalesAnalysisSample;
  processingSettings: SettingsSample<CompanyProcessingSettings>;
  referenceSettings: SettingsSample<CompanyReferenceSettings>;
};

export type CompanyReadinessReport = {
  companyName: string;
  connectedContextFound: boolean;
  today: string;
  financialYear: FinancialYearInfo;
  todayInFinancialYear: boolean | null;
  vatOnCashReceiptsEnabled: boolean | undefined;
  cashReceiptVatMode: CompanyProcessingSettings["cashReceiptVatMode"] | undefined;
  referenceSettings: {
    salesReferences: "Auto" | "Manual" | "Unknown";
    purchasesReferences: "Auto" | "Manual" | "Unknown";
    quotesReferences: "Auto" | "Manual" | "Unknown";
    debtorsJournalReferences: "Auto" | "Manual" | "Unknown";
    creditorsJournalReferences: "Auto" | "Manual" | "Unknown";
  };
  referenceDataSampleCounts: {
    customersOnFirstPage: number;
    productsOnFirstPage: number;
    suppliersOnFirstPage: number;
    vatRatesOnFirstPage: number;
    salesRepsOnFirstPage: number;
    salesVatRatesOnFirstPage: number;
    salesAnalysisCategoriesOnFirstPage: number;
  };
  readiness: {
    readOnlyReady: boolean;
    createCustomerSupplierProductReady: boolean;
    transactionReady: boolean;
    transactionReadyStatus: "Not ready" | "Ready with preflight checks" | "Ready";
    generatedDocumentReady: boolean;
  };
  ...
  deploymentCapabilities: ReturnType<typeof getCustomerDeploymentCapabilities>;
};
...
export function transactionNeedsPreflightChecks(
 ...
 */
export function isSalesAnalysisCategory(item: JsonRecord): boolean {
...
export function isActiveVatRate(rate: JsonRecord): boolean {
...
export function countActiveSalesVatRates(
  categories: unknown,
  rates: unknown
): { salesActiveCount: number; totalRatesSampled: number; hasSalesCategory: boolean } {
  ...
  for (const rate of rateItems) {
    ...
    if (!context.salesVatRateIds.has(rateId)) continue;
    if (!isActiveVatRate(rate)) continue;
    salesActiveCount += 1;
  }
...
export function countSalesAnalysisCategories(categories: unknown): {
  salesAnalysisCount: number;
  totalCategoriesSampled: number;
} {
...
export function inspectCompanyConnection(
  companyName: string,
...
function unknownCheck(message: string, details?: Record<string, unknown>): HealthCheckResult {
  return { status: "unknown", message, details };
}

function buildConnectionCheck(
  connection: ConnectionSnapshot,
  nowMs: number
): HealthCheckResult {
...
function buildFinancialYearCheck(
  financialYear: FinancialYearInfo,
  todayInFinancialYear: boolean | null
): { check: HealthCheckResult; blocker?: string } {
  const details = {
  ...
  if (todayInFinancialYear === false) {
    return {
      ...
      blocker:
        "Today is outside the company's current financial year, so transaction posting is not ready.",
    };
  }
...
function buildListPresenceCheck(args: {
  ...
  sample: ListSample;
  emptyMessage: string;
  emptyStatus?: "warning" | "fail";
  passMessage: string;
}): HealthCheckResult {
  if (!args.sample.ok) {
    return unknownCheck(
      `Could not check ${args.label}: ${args.sample.errorMessage ?? "lookup failed"}.`,
      { count: args.sample.count }
    );
  }
...
function buildProcessingCheck(
  sample: SettingsSample<CompanyProcessingSettings>,
  warnings: string[]
): HealthCheckResult {
  if (!sample.ok || !sample.value) {
    return unknownCheck(
      `Could not read processing settings: ${sample.errorMessage ?? "lookup failed"}.`
    );
  }
  ...
  if (settings.vatOnCashReceiptsEnabled === undefined) {
    pushUnique(
      warnings,
      "Could not confirm the VAT on Cash Receipts setting from company options."
    );
    return {
      status: "warning",
  ...
  if (settings.vatOnCashReceiptsEnabled === true) {
    if (settings.cashReceiptVatMode === "manual") {
      pushUnique(
        warnings,
        "VAT on Cash Receipts is enabled with manual VAT mode. Extra VAT details may be required before posting cash receipts."
      );
      return {
        status: "warning",
    ...
    if (settings.cashReceiptVatMode === "allocation") {
      pushUnique(
        warnings,
        "VAT on Cash Receipts is enabled with allocation mode. Allocation details may be required before posting cash receipts."
      );
      return {
        status: "warning",
    ...
    if (settings.cashReceiptVatMode === "unknown") {
      pushUnique(
        warnings,
        "VAT on Cash Receipts is enabled, but the cash receipt VAT mode could not be determined."
      );
      return {
        status: "warning",
  ...
  return {
    status: "pass",
    message:
      settings.vatOnCashReceiptsEnabled === false
...
function buildReferenceCheck(
  sample: SettingsSample<CompanyReferenceSettings>,
  warnings: string[]
): {
  check: HealthCheckResult;
  formatted: CompanyReadinessReport["referenceSettings"];
} {
  const formatted: CompanyReadinessReport["referenceSettings"] = {
  ...
  if (!sample.ok || !sample.value) {
    return {
      check: unknownCheck(
        `Could not read reference settings: ${sample.errorMessage ?? "lookup failed"}.`
      ),
      formatted,
    };
  }
  ...
  formatted.purchasesReferences = formatReferenceMode(
  ...
  formatted.debtorsJournalReferences = formatReferenceMode(
    settings.debtorsJournalAutoGenerateReference
  );
  formatted.creditorsJournalReferences = formatReferenceMode(
    settings.creditorsJournalAutoGenerateReference
  );

  const details: Record<string, unknown> = { ...formatted };
  ...
  if (settings.salesAutoGenerateReference === undefined) {
    ...
    pushUnique(
      warnings,
      "Sales references are manual. A reference will be required before posting sales invoices or credit notes."
    );
  }
  ...
  if (settings.purchasesAutoGenerateReference === undefined) {
    pushUnique(
      warnings,
      "Could not confirm whether purchase references are auto-generated or manual."
    );
  } else if (settings.purchasesAutoGenerateReference === false) {
    pushUnique(
      warnings,
      "Purchase references are manual. A reference will be required before posting purchases."
    );
  }
  ...
  if (settings.quotesAutoGenerateReference === undefined) {
    pushUnique(
      warnings,
      "Could not confirm whether quote references are auto-generated or manual. Ask for a quote reference before preparing a postable quote."
    );
  } else if (settings.quotesAutoGenerateReference === false) {
    pushUnique(
      warnings,
      "Quote references are manual. A reference will be required before posting quotes."
    );
  }
  ...
  if (settings.debtorsJournalAutoGenerateReference === undefined) {
    pushUnique(
      warnings,
      "Could not confirm whether debtors journal references are auto-generated or manual."
    );
  }
  ...
  if (settings.creditorsJournalAutoGenerateReference === undefined) {
    pushUnique(
      warnings,
      "Could not confirm whether creditors journal references are auto-generated or manual."
    );
  }
...
export function evaluateCompanyReadiness(
  input: CompanyReadinessEvaluationInput
): CompanyReadinessReport {
  const warnings: string[] = [];
  const blockers: string[] = [];
  const recommendedActions: string[] = [];

  const todayInFinancialYear = dateWithinRange(
    input.today,
  ...
  if (connectionProblem) {
    ...
    if (!input.connection.contextPresent) {
      pushUnique(
        recommendedActions,
        "Connect this company using the secure Red connection page, then run the readiness check again."
      );
    } else {
      pushUnique(
        recommendedActions,
        "Reconnect this company using the secure Red connection page."
      );
    }
  }
  ...
  const fy = connectionProblem
    ? {
        check: unknownCheck(
          "Skipped financial year check because the company connection is not active."
        ),
  ...
  const customers = connectionProblem
    ...
    : buildListPresenceCheck({
        ...
        emptyMessage:
          "No customers were found on the first page. Customer workflows may need setup data.",
  ...
  const products = connectionProblem
    ...
    : buildListPresenceCheck({
        ...
        emptyMessage:
          "No products were found on the first page. Product-based invoices and quotes are limited; non-product transactions may still be possible.",
  ...
  if (products.status === "warning") {
    ...
    pushUnique(
      recommendedActions,
      "Add products in Big Red Cloud if you need product lines on invoices or quotes."
    );
  ...
  const suppliers = connectionProblem
    ...
    : buildListPresenceCheck({
        ...
        emptyMessage:
          "No suppliers were found on the first page. Purchase workflows may need setup data. This does not block sales invoices.",
  ...
  const salesReps = connectionProblem
    ...
    : buildListPresenceCheck({
        ...
        emptyMessage:
          "No sales representatives were found on the first page. Some sales documents may need a sales rep before posting.",
  ...
  if (salesReps.status === "warning") {
    ...
    pushUnique(
      recommendedActions,
      "Add a sales representative in Big Red Cloud if invoices require one."
    );
  } else if (salesReps.status === "unknown") {
    pushUnique(warnings, salesReps.message);
  }

  let salesVatRates: HealthCheckResult;
  if (connectionProblem) {
    salesVatRates = unknownCheck(
      "Skipped Sales VAT check because the company connection is not active."
    );
  } else if (!input.salesVatRates.ok) {
    salesVatRates = unknownCheck(
      `Could not check Sales VAT rates: ${input.salesVatRates.errorMessage ?? "lookup failed"}.`,
      {
    ...
    pushUnique(
      recommendedActions,
      "Add or activate a Sales VAT rate in Big Red Cloud before posting sales invoices."
    );
  }

  let salesAnalysisCategories: HealthCheckResult;
  if (connectionProblem) {
    salesAnalysisCategories = unknownCheck(
      "Skipped Sales Analysis check because the company connection is not active."
    );
  } else if (!input.salesAnalysisCategories.ok) {
    salesAnalysisCategories = unknownCheck(
      `Could not check Sales Analysis categories: ${
        input.salesAnalysisCategories.errorMessage ?? "lookup failed"
      }.`,
      {
    ...
    pushUnique(
      blockers,
      "No Sales Analysis categories are available for sales document lines."
    );
    pushUnique(
      recommendedActions,
      "Add Sales Analysis categories (for example SA codes) in Big Red Cloud."
    );
  }

  let processingSettings: HealthCheckResult;
  if (connectionProblem) {
    processingSettings = unknownCheck(
      "Skipped processing settings check because the company connection is not active."
    );
  } else {
  ...
  let referenceFormatted: CompanyReadinessReport["referenceSettings"] = {
    salesReferences: "Unknown",
    purchasesReferences: "Unknown",
    quotesReferences: "Unknown",
    debtorsJournalReferences: "Unknown",
  ...
  let referenceSettings: HealthCheckResult;
  if (connectionProblem) {
    referenceSettings = unknownCheck(
      "Skipped reference settings check because the company connection is not active."
    );
  } else {
  ...
  const needsPreflight =
    processingValue && referenceValue
  ...
  if (needsPreflight && !connectionProblem && blockers.length === 0) {
    pushUnique(
      warnings,
      "Some transaction workflows may need extra VAT, allocation, or reference details before posting."
    );
  }
  ...
  if (connectionProblem) {
    ...
    const hasWarningOrUnknown = Object.values(checks).some(
  ...
  const readiness = {
    ...
    transactionReadyStatus: !transactionReadyBase
      ? ("Not ready" as const)
      : needsPreflight
        ? ("Ready with preflight checks" as const)
        : ("Ready" as const),
...
function buildSummary(args: {
  ...
  blockers: string[];
  warnings: string[];
  readiness: CompanyReadinessReport["readiness"];
}): string {
  ...
  switch (args.overallStatus) {
    ...
    case "ready_with_warnings":
      return `${name} can be used, but ${args.warnings.length} warning${
        args.warnings.length === 1 ? "" : "s"
      } should be reviewed before posting.`;
    case "not_ready":
      return `${name} is not ready for transactions yet (${args.blockers.length} blocker${
        args.blockers.length === 1 ? "" : "s"
      }).`;
...
async function safeSalesVatSample(companyName: string): Promise<SalesVatSample> {
  try {
...
async function safeSalesAnalysisSample(
  companyName: string
): Promise<SalesAnalysisSample> {
  try {
    const data = await brcFetch(
      companyName,
      `/v1/analysisCategories?page=1&pageSize=${LOOKUP_PAGE_SIZE}`
    );
...
async function safeSettingsSample<T>(
  loader: () => Promise<T>
): Promise<SettingsSample<T>> {
  try {
...
export type RunCompanyReadinessOptions = {
  nowMs?: number;
  today?: string;
  connection?: ConnectionSnapshot;
};
 ...
 */
export async function runCompanyReadinessCheck(
  companyName: string,
  options: RunCompanyReadinessOptions = {}
): Promise<CompanyReadinessReport> {
  ...
  const listPath = (resource: string) =>
    `/v1/${resource}?page=1&pageSize=${LIST_PAGE_SIZE}`;
  ...
  const financialYear =
    financialYearData.ok || setupData.ok
