...
/** List/get only — registered in production for payments and reference data. */
...
/** Full bank CRUD — under development; not registered in production. */
export function registerBankTools(server: ServerType) {
  registerBankListTools(server);

  server.tool(
    "brc_create_bank_account",
    "Creates a BRC bank account. Requires acCode, details, lastChq, nominalAcCode, and categoryId (Bank Payments category type from brc_list_category_types). nominalAcCode must reference an existing balance-sheet bank nominal account created in BRC setup.",
    {
  ...
  registerRawUpdateTool(
    ...
    "Updates a BRC bank account using merged fields.",
    "/v1/bankAccounts",
    "Bank account"
  );
  registerRawDeleteTool(
    ...
    "Deletes a BRC bank account by id.",
    "/v1/bankAccounts",
    "bank account"
  );
}
