 ...
 */
export const RED_HELP_TOOL_TITLE =
  "Red Help — Manual Instructions and Resources";
 ...
 */
export function ensureRedHelpQueryForUnifiedSearch(query: string): string {
...
export async function runUnifiedHelpSearchPipeline(
  question: string,
  options?: {
    category?: string;
    maxResults?: number;
    source?: (typeof HELP_RESOURCE_SOURCES)[number] | "all";
  },
) {
  const [
    recordedWebinars,
...
export function registerHelpResourcesTools(server: ServerType): void {
  server.tool(
    "brc_red_help",
    RED_HELP_TOOL_DESCRIPTION,
    {
      query: z
        .string()
        .min(1)
        .describe(
          "Text after the red-help command, for example how do I add a sales invoice or add a customer manually.",
        ),
    },
    async ({ query }) => {
      return runUnifiedHelpSearchPipeline(
        ensureRedHelpQueryForUnifiedSearch(query),
      );
    },
  );
  ...
  server.tool(
    ...
    GET_HELP_RESOURCE_DETAILS_TOOL_DESCRIPTION,
    {
      resourceId: z
        ...
        .min(1)
        .describe(
          "Resource ID from brc_red_help or brc_find_help_resources, for example customer_docs:bank-reconciliation or freshdesk:1001.",
        ),
      includeImages: z
        .boolean()
        .optional()
