...
async function seedClaimableAfterValidation(args: {
  ...
  const issued = await store.issueConfirmationCode(
    args.connectToken,
...
function mockValidator(
  outcomes: Record<string, "valid" | "invalid_or_expired_api_key">
) {
...
test("one invalid API key among multiple companies excludes only that company", async () => {
  ...
  assert.match(
    outcome.failedCompanies[0]?.message ?? "",
...
test("CompanyNotConnectedError is thrown when other companies are connected", async () => {
  ...
  await runWithSessionKeyStore(keyStore, async () => {
    ...
    assert.throws(
      ...
      (error: unknown) => error instanceof CompanyNotConnectedError
    );
  });
});
